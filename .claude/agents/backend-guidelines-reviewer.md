...
| DOM-24 | Kafka producer stubbed in tests that emit | (a) For each `*_test.go` file in the changed packages, grep for `AndEmit(`, `message.Emit(`, or `producer.Produce(` call sites. (b) If any are present, the package must have one of: a `TestMain` that calls `producertest.InstallNoop()` (from `github.com/Chronicle20/atlas/libs/atlas-kafka/producer/producertest`), OR per-test injection of a no-op `producer.Provider` via a `WithProducer(...)` builder method (see `atlas-marriages` for the canonical example). (c) Do NOT accept a service-local `noopWriter` / `testkafka` helper — the shared `producertest` package is the single source of truth. | Test packages with emit calls have a stub installed. An unstubbed emit path makes the test take ~42s per emit (10-retry × 100ms→10s backoff in `libs/atlas-kafka/producer/producer.go`) and may also cause tests to assert state that only holds while the producer is hanging. See `testing-guide.md` → "Stubbing the Kafka Producer in Tests". |
