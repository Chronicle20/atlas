...
| SCAFFOLD-03 | Grep `"atlas-<service>"` in `docker-bake.hcl`'s `go_services` list and `./services/atlas-<service>/atlas.com/<svc>` in `go.work`. | Both present. There is **no** per-service Dockerfile — the repo-root `Dockerfile` is shared and parameterized by `ARG SERVICE` (see [patterns-deploy.md](patterns-deploy.md)). A `services/atlas-<service>/Dockerfile` in a new Go service is itself a finding. `docker-bake.hcl` is hand-synced with services.json; adding to one does not add to the other. |
