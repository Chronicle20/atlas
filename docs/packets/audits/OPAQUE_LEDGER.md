...
| **mob move-path** | `MonsterMovement`, `MonsterMovementRequest` | `CMob::GenerateMovePath` sub-struct | No — element-loop over move fragments | **VERIFIED-EXCEPTION** · `model/movement_test.go`, `monster/clientbound/movement_test.go`, `monster/serverbound/movement_test.go` (byte-for-byte, task-065 `e32a3d809`). On GMS v87 the clientbound citation covers the packet framing and its blob linkage only — see the `model.Movement` row for the directional carve-out and where the v87 blob width is actually pinned. |
| **`model.Movement` (shared path)** | pet + monster movement, `*MovementRequest` | shared movement encoder reads as one `DecodeBuf` | No — element-loop | **VERIFIED-EXCEPTION** · `model/movement_test.go`, `model/version_bounds_test.go`. DIRECTIONAL on GMS v87: the client reads the per-element `XOffset`/`YOffset` pair but is never sent it back (`CMovePath::Encode` @0x6c70fe vs `CMovePath::Decode` @0x6c6e86), so encode is deliberately not decode's inverse there. The clientbound round-trip tests skip the identity assertion on v87 via `test.MovementRoundTrip` and assert the blob against this encoder instead; the per-version, per-direction blob WIDTH is pinned only by `model/version_bounds_test.go` (`TestNormalElementMovementVersionBoundary`, `TestNormalElementOffsetsAreDirectional`). |
