...
| 5 | int16 | int16 `packed: bit15=bLeft, low15=nAction (attackAction)` | ✅ |  |
| 6 | int16 | byte `nActionSpeed (unconditional in v83 — no nAction <= 0x110 guard)` | ❌ | width mismatch |
| 7 | byte | byte `nMastery (unconditional in v83)` | ✅ |  |
| 8 | byte | int32 `nBulletItemID (unconditional in v83)` | ❌ | width mismatch |
| 9 | int32 | int32 `monsterId per damage target (loop nDamagePerMob times)` | ✅ |  |
| 10 | int32 | byte `hitAction per target (if monsterId != 0)` | ❌ | width mismatch |
| 11 | byte | byte `damage count (only if meso explosion skill 4211006, per target)` | ✅ |  |
| 12 | byte | int32 `damage value per hit (loop nHits times, or damage-count for meso explosion)` | ❌ | width mismatch |
| 13 | int32 | int16 `ptBallStart.x (only if nType==212 ranged / opcode 0xBB)` | ❌ | width mismatch |
| 14 | int16 | int16 `ptBallStart.y (only if nType==212 ranged / opcode 0xBB)` | ✅ |  |
