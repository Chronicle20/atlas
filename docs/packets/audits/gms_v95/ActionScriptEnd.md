...
**IDA function:** `CQuest::StartQuest` @ 0x6b40a0, action=5 (`IsCompleteScriptLinkedQuest` branch)

When the quest has a complete script linked, the client sends action 5 via opcode 119. After
the `Action` header (action byte + questId), the script-end sub-fields are:

```
COutPacket::Encode1(&oPacket, 5u)            // action byte = 5 (in Action header)
...
`ActionScriptEnd.Encode/Decode` matches v95 exactly. The ✅ static-diff verdict is accurate.
Note: unlike `ActionComplete` (action=2), the script-end branch does NOT write a delivery-item
slot field. See `_pending.md` for the `ActionStart`/`ActionComplete` delivery-item gap.

Wire shape verified by `TestActionScriptEndWireShape` in
`libs/atlas-packet/quest/serverbound/action_script_end_test.go`:
