...
| 1 | int32 | bytes `uFlagSet: 16-byte UINT128 stat mask (DecodeBuffer 0x10) — atlas: MonsterTemporaryStat.Encode mask` | ❌ | width mismatch |
| 2 | int32 | bytes `per-stat body via CMob::ProcessStatSet (variable-length, mask-dispatched)` | ❌ | width mismatch |
