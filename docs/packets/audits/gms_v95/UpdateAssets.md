...
> ack: tool limitation (loop-flattened per-tab list + GW_ItemSlotBase/Asset
> sub-struct; row-3 width mismatch is the unmodellable conditional meso). NOT a
> wire bug — runtime callers set exactly one tab bit and filter assets to that
> type (flag&2 never set), so the client loop runs once and matches atlas's
> single-count emission. See `docs/packets/ida-exports/_pending.md` →
> "UpdateAssets clientbound — loop/sub-struct tool limitation (storage)".
