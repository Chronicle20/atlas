...
|---|---|---|---|---|
| 0 | (see byte-fixture) | `case 38 (@0xaef3af) — jms case-numbering diverges from GMS; alloc 0xD8h matches gms_v87's case-60 shape exactly: count(byte)+count x line(str)+whisper(bool), NO trailing update_time (updateTimeFirst=TRUE) — task-123 megaphone gap-fill pass.` | 🔍 | flat-diff-invalid: data-dependent branch — see byte-fixture test for the verified per-branch wire shape |
