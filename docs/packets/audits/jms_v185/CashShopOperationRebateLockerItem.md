...
| 1 | int64 | string `spw (secondary password). op-byte 0x1B (NOT GMS 0x1C). JMS has SPW string (atlas else-branch has int birthday)` | ❌ | width mismatch |
| 2 | byte | bytes `8-byte locker SN (Src). matches atlas unk long` | ❌ | atlas: short — missing trailing field |
