...
The companion project `home-hub` at `<home>/source/home-hub/frontend/` is a comparable internal admin dashboard (React 19, TypeScript, shadcn/ui, Tailwind 4, React Query, react-hook-form + Zod, multi-tenant context) built on **Vite 8 + react-router-dom 7 + Vitest**. It is simpler, smaller, and proves the template is sufficient for this class of application.
