...
| Kafka `expression.Event` missing `duration` + `byItemOption` | `kafka/message/expression/kafka.go` carries only `emote`; consumer calls `NewCharacterExpression(..., 0)` hard-coding `duration=0`. v95+/JMS clients always observe `duration=0` and `byItemOption=false` until the Kafka event is extended end-to-end (producer + consumer). TODO comment added in `consumer/expression/consumer.go`. | Follow-up: extend kafka/message/expression/kafka.go + producer side |
