# Misc-Domain Packet Audit — Product Requirements Document
...
After task-069 lands, this task also delivers the **complete-library audit ledger**: a `docs/packets/audits/gms_v95/TOTAL.md` cross-referencing every verdict from tasks 027, 028, 065, 066, 067, 068, and 069 — the canonical "what's wire-correct" source for `libs/atlas-packet/`.
...
- Produce `docs/packets/audits/gms_v95/TOTAL.md` — a consolidated ledger cross-referencing verdicts from tasks 027, 028, 065-069 — and confirm 100% coverage of `libs/atlas-packet/` top-level directories.
...
- Sub-op enum drift — defer to `_pending.md` per design §9.

## 3. User Stories

- As a **server operator**, I want every misc-domain packet to be IDA-verified against GMS v95 so that operationally critical paths (first-login TOS accept, socket handshake, quest action) cannot silently corrupt client state.
- As a **backend engineer touching these packets**, I want per-version `Region/MajorVersion` gates anchored to IDA evidence so that I can change behavior in one version without breaking the others.
- As a **release engineer**, I want a single `TOTAL.md` ledger after task-069 ships so that I can answer "is this packet wire-correct?" from one document instead of seven.
- As a **future audit-task author**, I want the post-069 sweep to confirm 100% domain coverage so that I know whether any `libs/atlas-packet/` directory still lacks an audit.
...
**account/serverbound (6):** `accept_tos`, `register_pin`, `set_gender`, plus any remaining files (enumerate at audit start).
...
`tool/` has no `.go` files under `clientbound/` or `serverbound/` in the current codebase — it contains only `uint128.go` and `uint128_test.go` (utility types) plus a `serverbound/` subdirectory. Confirm at audit time. `account/clientbound/` also has no files — the domain is serverbound-only.
...
### 4.7 Cross-version re-verification
...
### 4.9 TOTAL.md cross-task ledger

Produce `docs/packets/audits/gms_v95/TOTAL.md` containing:

- A table of every `libs/atlas-packet/<domain>/` directory with the audit task that owns it (027 login, 028 character, 065 combat, 066 social, 067 commerce, 068 world, 069 misc).
- A roll-up verdict count per domain (✅ / ⚠️ / ❌ counts pulled from each task's SUMMARY.md).
- A "gaps" section listing any domain not covered by tasks 027/028/065-069, with a recommendation (defer / new task / out-of-scope-permanently).
- Last-updated timestamp + commit refs for each contributing task's final audit.

### 4.10 Post-audit coverage sweep

As part of acceptance, run `find libs/atlas-packet -maxdepth 1 -type d | sort` and cross-reference against the 7-task coverage matrix. Every directory must either appear in a task's audit scope or be documented as out-of-scope (utility-only, deprecated, etc.) in `TOTAL.md`'s gaps section.
...
- New `docs/packets/audits/gms_v95/TOTAL.md` cross-task ledger.
...
| `services/atlas-login/`, `services/atlas-channel/`, `services/atlas-account/` | No code changes expected, but build-clean verification required after any encoder/decoder signature ripple. |
| `docs/packets/audits/gms_v95/` | New per-packet audit reports + TOTAL.md ledger. |
...
- `go build ./...` clean across the repo (catches encoder constructor-signature ripple into atlas-login, atlas-channel, atlas-account).
- Login (task-027) and character (task-028) verdicts unchanged.
- **Extra caution on socket packets**: `socket/clientbound/hello` and `socket/serverbound/pong` are on the critical path for every client connection — any wire change here must be verified against ALL version templates before commit, and atlas-login + atlas-channel must build clean before PR.
...
## 10. Key Risks / Patterns to Watch For

Drawn from task-027 / task-028 lessons:
...
- task-065, task-066, task-067, task-068 — sibling audit tasks. Their final SUMMARY.md verdict tables are inputs to TOTAL.md. If any are still in flight when task-069 reaches Phase B, coordinate on a TOTAL.md draft + final-revision plan.

## 12. Open Questions

- `tool/`: does `libs/atlas-packet/tool/serverbound/` contain packet files, or is the directory empty? (Resolved at audit start via the §4.8 enumeration step.)
- Are there any hidden domains under `libs/atlas-packet/` not captured in this PRD? (Resolved by §4.10 post-audit coverage sweep.)
- If a sibling task (065-068) hasn't shipped its final SUMMARY.md by the time TOTAL.md is being assembled, do we (a) draft TOTAL.md with placeholders and revise at PR time, or (b) wait? Default plan: draft with placeholders; revise before PR. Confirm at design phase.

## 13. Acceptance Criteria

- [ ] All ~49 listed packet files have audit reports under `docs/packets/audits/gms_v95/`.
- [ ] `tool/` domain is investigated and confirmed as utility-only or expanded if packet files are found; finding noted in `post-phase-b.md`.
...
- [ ] `post-phase-b.md` ledger written (includes tool-domain confirmation note, wire-bug fixes table, template fixes table).
- [ ] `docs/packets/audits/gms_v95/TOTAL.md` produced, cross-referencing verdicts from tasks 027, 028, 065, 066, 067, 068, and 069.
- [ ] `find libs/atlas-packet -maxdepth 1 -type d | sort` output cross-referenced against the 7-task coverage matrix in TOTAL.md; every directory is either claimed by a task or documented as out-of-scope.
- [ ] Cross-version pass (v83, v87, JMS v185) recorded in `post-phase-b.md`.
