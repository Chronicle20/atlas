...
- **No `go mod edit -replace` block.** Empirically tested per §2.3; expected to be unnecessary because `go.work` covers the local resolution.
  ...
  - **Recorded outcome:** [2026-05-21] First attempt copied the repo-root `go.work` directly into the build context. That failed because `go.work` lists ~50 sibling service modules + 2 `tools/*` modules whose `go.mod` files are not in any single-service build context, and workspace-load aborts when any `use (...)` entry cannot resolve. Mitigated per §3.3 by dropping `COPY go.work ./` and synthesizing a minimal `go.work` inline (17 libs + the target service) at build time, matching what the legacy per-service Dockerfiles did. Once synthesized, `docker build --build-arg SERVICE=atlas-account .` succeeded with no `go mod edit -replace` block. `go.work use(...)` alone is sufficient — no `-replace` reinstated, `go mod tidy` remains dropped. (Separate small fix dropped `go.sum` from the COPY lines for `atlas-retry` and `atlas-service`, which carry no external deps and therefore have no `go.sum` files.)
