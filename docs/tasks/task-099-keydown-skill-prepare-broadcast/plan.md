...
- [ ] **Step 3 (GREEN): handlers.** Implement `CharacterSkillPrepareHandleFunc` and `CharacterSkillCancelHandleFunc`: decode (`SkillPrepareInfo`/`SkillCancelInfo`), look up character with skill decorator, validate ownership (non-zero level) AND `skill.IsKeyDownSkill(id)` — on miss `l.Debugf(...)` + return (no Destroy), else `_map.NewProcessor(...).ForOtherSessionsInMap(...)` with the announce helper. Define consts `CharacterSkillPrepareHandle` / `CharacterSkillCancelHandle`. Run tests → PASS.
- [ ] **Step 4: register.** `main.go produceHandlers()`: add `handlerMap[handler.CharacterSkillPrepareHandle] = handler.CharacterSkillPrepareHandleFunc` (+ cancel). `produceWriters()`: append `charcb.CharacterSkillPrepareForeignWriter` and `charcb.CharacterSkillCancelForeignWriter`.
...
- [ ] **Step 1: add rows per version** using the **opcodes from `wire-spec.md`** (per-version — they differ): two `socket.handlers` rows (`CharacterSkillPrepareHandle`, `CharacterSkillCancelHandle`) each with `"validator": "LoggedInValidator"` (a missing validator silently drops the handler), and two `socket.writers` rows (`CharacterSkillPrepareForeign`, `CharacterSkillCancelForeign`). If Task 1 added MovingShoot, add its rows too.
