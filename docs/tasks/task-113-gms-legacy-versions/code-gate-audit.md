...
| buddy/operation_add (sb): trailing `group` ascii string | `libs/atlas-packet/buddy/serverbound/operation_add.go:47` (Encode; Decode :58) | `MajorVersion()>61` _(region-less; only GMS/JMS exist so JMS 185→present)_ | TRUE → group present (79>61) | TRUE → group present (72>61) | **FALSE** → group OMITTED | FALSE — = v61 | yes (CORRECTED) | **v48 pass corrected an unconditional-write false-pass:** base wrote `group` for ALL versions; v48/v61 `AddBuddy` send-site (`sub` @0x4e9c03, also v72 0x515575 / v79 0x558844) shows the buddy-group name entered with the buddy-group UI — v61 sends no trailing group string → gate `MajorVersion()>61`. v72/v79/v83+ unchanged (all >61 → present, matching base). Only v61 (and older) newly OMIT. Minor: region-less predicate (cosmetic per M1; correct because JMS 185>61). v48 commit `12aa5681`. |
| reactor/hit (sb): `isSkill` bool (v72+) and trailing `skillId` int (v79+) | `libs/atlas-packet/reactor/serverbound/hit.go:44` (isSkill; skillId :53) | isSkill: `(IsRegion("GMS") && MajorAtLeast(72)) \|\| JMS`; skillId: `(IsRegion("GMS") && MajorAtLeast(79)) \|\| JMS` | isSkill TRUE + skillId TRUE (79≥79) | isSkill TRUE + **skillId FALSE** (72<79) | **isSkill FALSE + skillId FALSE** | FALSE/FALSE — = v61 | yes (CORRECTED) | **v48 pass corrected an unconditional-write false-pass:** base wrote isSkill+skillId for ALL versions; IDA shows v48 `CReactorPool::FindHitReactor @0x5a5d1a` and v61 @0x633ac7 go oid→dwHitOption directly (no isSkill); v72 @0x6928bc inserts isSkill(Encode4 0); v79 @0x6b8077 + v83 @0x7356c7 append the trailing skillId. Gates `isSkill≥72` / `skillId≥79`. v83/84/87/95/JMS unchanged (all ≥79 → both present). v48 commit `daae358a`. |
| npc/start_conversation (sb): user x/y shorts after npc oid | `libs/atlas-packet/npc/serverbound/start_conversation.go:42` (`startConversationHasXY`; Encode :77, Decode :89) | `!IsRegion("GMS") \|\| MajorAtLeast(72) \|\| MajorVersion()==61 \|\| MajorVersion()==48` | TRUE → x/y present (79≥72) | **TRUE (CORRECTED)** → x/y present (72≥72) | **TRUE** → x/y present (`==61`) | **TRUE** → x/y present (`==48`) | yes (CORRECTED) | **Phase-5 corrected a v72 false-pass:** v48 close-B6 IDA-confirmed NPC_TALK sb = oid+x+y (`sub_568A2A`); Phase 5 then IDA-confirmed v72 `CUserLocal::TalkToNpc @0x63FD91` ALSO sends oid+Encode2(x)+Encode2(y) at all 3 send-sites (the old oid-only v72 fixture cited `0x70dd49` = a stale v48-copied symbol, `CUICharacterSaleDlg::OnCreate` in v72). Gate `MajorAtLeast(79)`→`MajorAtLeast(72)`; v72 fixture → oid+x+y; v79/v61/v48/v83+/JMS unchanged. Now monotone (all real versions ≥48 send x/y). v48 commit `a16262ea`; Phase-5 fix `a0b8614c`. |

> **Note on `startConversationHasXY` (RESOLVED in Phase 5):** the v61-pass gate
> `MajorAtLeast(79) \|\| ==61` left v72 FALSE — a documented v72 false-pass. Phase 5
> IDA-verified v72 `CUserLocal::TalkToNpc @0x63FD91` sends oid+x+y (all 3 send-sites),
> narrowed the gate to `MajorAtLeast(72)`, and re-fixtured v72 (commit `a0b8614c`). The
> gate is now monotone — every real version ≥48 carries x/y; only the ancient v28 test
> variant is oid-only. No outstanding follow-up.
