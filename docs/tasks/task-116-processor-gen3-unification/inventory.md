...
- `services/atlas-data/atlas.com/data/data/processor.go` (task 35): `RegisterAllData`'s worker-pool loop collects per-file registration errors into a local `errors []error` slice via a goroutine draining `errChan`, but the function always `return nil` regardless of `errors`' contents — every per-file registration failure inside the directory walk is silently swallowed (the individual `rf` errors never surface to the caller; only a directory-`Stat`/`WalkDir`-level failure propagates). Pre-existing in the original curried `RegisterAllData` (confirmed: the `errors` variable and its collector goroutine were present, unused for control flow, before this task's edits); left byte-identical per Global Constraint 1.
