...
| `services/atlas-data/atlas.com/data/characters/templates/processor.go` | Gen1 | R3 | 34 | done |
| `services/atlas-data/atlas.com/data/cosmetic/face/processor.go` | Gen1 | R3 | 34 | done |
| `services/atlas-data/atlas.com/data/cosmetic/hair/processor.go` | Gen1 | R3 | 34 | done |
| `services/atlas-data/atlas.com/data/equipment/processor.go` | Gen1 | R3 | 34 | done |
| `services/atlas-data/atlas.com/data/data/processor.go` | Gen1 | R3 | 35 | done |
...
- **OPEN — needs a design decision (not fixed):** `services/atlas-pets/atlas.com/pets/pet/processor.go` (`ProcessorImpl.Create`, task 5): pre-existing `// TODO this needs to generate a cashId if cashId == 0`. Implementing it is a genuine feature, not a mechanical fix, and it hinges on cash-serial ownership: `atlas-cashshop` already mints cash serials locally (`generateUniqueCashId`, a `rand.Int63()` + uniqueness-check loop against its own table); there is no central cashId authority. atlas-pets has no by-cashId lookup infra. The open question is whether a purchased pet's serial is minted by cashshop and passed to atlas-pets (making `cashId == 0` a rare non-purchase path safe to fill with an independently-minted value) or whether atlas-pets must coordinate with cashshop's namespace (independent minting would then diverge the pet's serial from the cash item's). Deferred pending that decision — see the branch summary.
- **RESOLVED (commit `2f2b3fe0e9`):** `services/atlas-login/atlas.com/login/socket/init.go:39` (task 16): pre-existing `go vet` finding `WaitGroup.Add called from inside new goroutine` (the `wg.Add(1)` call sits inside the `go func() { ... }()` closure it starts). Present on `main` before this task (confirmed unchanged since PR #738) and unrelated to `inventory`/`guild`. Fixed on user request post-conversion: `wg.Add(1)` moved ahead of the goroutine it tracks; `go vet` clean.
...
- **RESOLVED (commit `7a98d2f474`):** `services/atlas-rates/atlas.com/rates/session/processor.go` (task 30): pre-existing orphaned doc comment `// ComputePlaytimeSince computes total playtime for a character since the given time` sat directly above the `ComputePlaytimeInRange` method with no corresponding function — `ComputePlaytimeSince` does not exist anywhere in the package. Fixed on user request post-conversion: the orphaned comment (and its trailing blank line) was removed; `ComputePlaytimeInRange` keeps its own comment.
...
- `services/atlas-data/atlas.com/data/map/processor.go` (task 33): `calcDropPos` is a private curried helper (`func(s) func(ctx) func(mapId, initial, fallback) (...)`) whose sole caller is `map/resource.go` — a different file in the same package. Per R3 Step 3 ("private curried helpers... never enter the interface"), it could become an unexported `ProcessorImpl` method only if every caller could reach it through the `Processor` interface; since it's unexported, an interface-typed `Processor` value returned by `NewProcessor` cannot invoke it from `resource.go`. Rather than adding an unexported method to `Processor` (no precedent elsewhere in this task; `Processor` interfaces are exported-method-only per Global Constraint 4) or introducing a type assertion to `*ProcessorImpl`, `calcDropPos` was left as an unconverted package function in its original curried shape — a scoped, documented exception to full R3 conversion for this one helper.
- `services/atlas-data/atlas.com/data/npc/processor.go` (task 33): the task brief's higher-order exemplar text describes `data/workers/npc.go`'s `registerAllInDirectory(l, ctx, dir, npc.RegisterNpc(db))` call site rewiring to a bare method value `npc.NewProcessor(l, ctx, db).RegisterNpc` (type `func(path string) error`) with `registerAllInDirectory`'s parameter type simplified to match. That literal simplification is not possible yet: `registerAllInDirectory` (`data/workers/walk.go`) and `RegisterAllData`/`RegisterFileData` (`data/processor.go`) are single shared functions called from many sites beyond npc's — `mapw.go`, `mob.go`, `reactor.go`, `skill.go`, `item.go` (via a heterogeneous `[]struct{subdir string; rf RegisterFunc}` slice), and `character.go` (`face.RegisterFace`, `hair.RegisterHair`, `equipment.RegisterEquipment`, all still Gen1, Task 34) — all of which still pass `RegisterFunc`-curried closures at the time of npc's commit. Simplifying the shared parameter type now would break every one of those still-Gen1 call sites. Per the plan.md Tasks 32-35 preamble ("rewire that call site... into adapter closures matching the still-curried RegisterFunc/data.RegisterFunc shared types") and the established Task 32 precedent (cash/commodity/consumable/etc/pet/setup all used this same pattern), npc's two external call sites (`data/processor.go` `WorkerNPC` branch, `data/workers/npc.go`) were rewired with adapter closures — `func(l) func(ctx) func(filePath) error { return ... npc.NewProcessor(l, ctx, db).RegisterNpc(filePath) }` — identical in shape to map/mobskill/monster's adapters, not bare method values. The true method-value simplification of `registerAllInDirectory`'s own signature is deferred to Task 35, once every one of its callers (including Task 34's face/hair/equipment) has converted and can supply a bare `func(path string) error`.

- `services/atlas-data/atlas.com/data/data/processor.go` (task 35, the orchestrator `data/data` package): converted `ProcessData`, `InstructWorker`, `StartWorker`, `RegisterAllData`, `RegisterFileData` — all five exported curried functions in the file — into `Processor`/`ProcessorImpl{l, ctx, db}`/`NewProcessor(l, ctx, db) Processor` methods, collapsing only the `l`/`ctx`/`db` currying levels (matches R3 Step 1's "db where any converted function curries one in": only `StartWorker` curried a `db` level, so `NewProcessor` gained a `db` field shared by all five methods). `t := tenant.MustFromContext(ctx)` in `StartWorker` was kept inside the method body (not hoisted into `NewProcessor`) — in the original it was resolved when the `ctx` curry level was applied, one step before `db`/`(name,path)`, and since the whole curried chain evaluates eagerly in one caller expression either way, resolving it per-`StartWorker()`-call (via `p.ctx`) reproduces the exact same evaluation order without moving the acquisition point into construction. The sole external call site, `kafka/consumer/data/consumer.go`'s `handleStartWorker`, rewired `data.StartWorker(l)(ctx)(db)(c.Body.Name, c.Body.Path)` → `data.NewProcessor(l, ctx, db).StartWorker(c.Body.Name, c.Body.Path)`. `runwz.go`'s `RunWorkers` (also an exported curried function in the same `data` package, `(l, db, mc)(ctx, p)`) was intentionally left unconverted: it is not in a file named `processor.go`, is outside Task 35's stated file scope (`data/processor.go` + `data/workers/*.go`), and is not tracked by the FR-8 inventory scan (which only enumerates files literally named `processor.go`); converting it would be scope creep beyond this task's file list.
- **Workers plumbing simplification (task 35), resolving the task-33 npc deferral above:** now that every leaf package under `data/workers/*.go`'s call sites (map, monster, npc, skill, reactor, face, hair, equipment, consumable, cash, etc, setup, pet) is Gen3, both shared `RegisterFunc` types were simplified from the curried `func(l logrus.FieldLogger) func(ctx context.Context) func(path string) error` to the flat `func(path string) error`: `data/processor.go`'s own `RegisterFunc`/`RegisterAllData`/`RegisterFileData` (now `ProcessorImpl` methods, calling `rf(filePath)` instead of `rf(l)(ctx)(filePath)`), and `data/workers/walk.go`'s `RegisterFunc`/`registerAllInDirectory` (`rf(path)` instead of `rf(l)(ctx)(path)`). Every adapter-closure call site was replaced with a bare method value per the design's worked example (`npc.NewProcessor(l, ctx, db).RegisterNpc`): `data/processor.go`'s `StartWorker` (11 `RegisterAllData`/`RegisterFileData` call sites), `data/workers/mapw.go`, `mob.go`, `npc.go`, `skill.go`, `reactor.go` (one `registerAllInDirectory` call each), `character.go` (three: face/hair/equipment), and `item.go` (the `categories` slice's five `{subdir, rf}` entries). Verified behavior-identical, not just aesthetic: every affected leaf's `NewProcessor(l, ctx, db)` constructor is a pure struct literal (`return &ProcessorImpl{l: l, ctx: ctx, db: db}`, no side effects, confirmed by reading all 15 leaf `processor.go` files) — so constructing it once before a directory walk (new) versus fresh on every file inside the walk (old, since the innermost closure body evaluated `xxx.NewProcessor(l, ctx, db).RegisterXxx(path)` fresh per `rf(l)(ctx)(path)` call) uses identical `l`/`ctx`/`db` values throughout; only construction *count* drops, not any acquisition point or resolved value. `registerAllInDirectory`'s own signature keeps its `(l, ctx, dir, rf)` parameter list unchanged (matching the design's worked-example call shape `registerAllInDirectory(l, ctx, dir, npc.NewProcessor(l, ctx, db).RegisterNpc)`) even though `ctx` is now unused inside its body — Go permits unused function parameters, and keeping the signature avoids further call-site churn beyond the `rf` type.

- **RESOLVED (commit `3475187388`):** `services/atlas-data/atlas.com/data/data/processor.go` (task 35): `RegisterAllData`'s worker-pool loop collected per-file registration errors into a local slice via a goroutine draining `errChan`, but the function always `return nil` regardless — every per-file registration failure inside the directory walk was silently swallowed, and `errChan` was never closed (leaking the collector goroutine). Pre-existing in the original curried `RegisterAllData`. Fixed on user request post-conversion: after the workers drain and `wg.Wait()`, `errChan` is closed and the collector is joined, then `errors.Join(collected...)` (plus any `WalkDir` error) is returned and logged. Errors now surface through `StartWorker` → the Kafka `handleStartWorker` consumer (a log/report path — the MinIO `RunWorkers` ingest uses the separate `workers.Worker` interface and is unaffected).
