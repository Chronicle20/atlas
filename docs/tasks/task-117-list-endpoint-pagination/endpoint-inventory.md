...
- atlas-merchant per-character/per-instance forms; `GET /merchants/search/listings` — **Converted (task-14)**. Post-rebase note: task-127 (owl shop scanner) rewrote the search with criteria (`worldId`, `order`) and a 200-row game cap; the merge keeps the criteria and makes the game cap the route's default page size (max stays the repo-wide 250 so drain consumers are not rejected), so the page-param-less atlas-channel owl consumer still gets the capped top-N in one response.
- atlas-merchant `GET /merchants/{shopId}/blacklist`, `GET /merchants/{shopId}/visits` — added unpaged by task-127 (mini-room dialogs), pulled in when task-117 rebased over it — **Converted (rebase addendum)**: DB-paged (250/250, PagedQuery; visits keep `count DESC` + PK tiebreak), atlas-channel dialog consumers drained. The visit log grows with unique visitor names, so it is a genuine Group C member, not just convention hygiene.
- atlas-merchant `GET /worlds/{worldId}/shop-searches/top` — added by task-127 — **Converted (rebase addendum)**: bounded top-N (`LIMIT 10` at the query layer, `count DESC, item_id ASC` total order); envelope via materialize + `paginate.Slice`. The atlas-channel consumer deliberately stays a single-page fetch — page 1 at the route default is always the complete ranking.
