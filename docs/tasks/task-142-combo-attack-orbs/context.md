...
- **Original createdAt/expiresAt travel in the event**; the packet layer encodes duration as `expiresAt − now` (`libs/atlas-packet/model/character_temporary_stat.go:623`), so re-broadcast carries remaining duration and the buff never extends.
...
| `socket/handler/character_attack_common.go` | TODO at line ~404 replaced with melee-gated `comboOrbTryUpdate` call (fire-and-forget, post-broadcast) |
...
- Skill constants all exist in `libs/atlas-constants/skill/constants.go:2937-2947, 3302-3306` (exact names in plan Global Constraints).
