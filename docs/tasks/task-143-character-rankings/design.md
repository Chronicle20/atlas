...
4. **No atlas-character changes needed.** `GET /characters` exists today. It is unpaginated (no `page[...]` handling anywhere in the handlers), which is fine at these volumes; if/when task-117 lands pagination, atlas-rankings adopts it as a drop-in improvement. task-117 is explicitly **not a prerequisite**.
