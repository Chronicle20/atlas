...
4. **Version-agnostic:** `X` comes per-tenant from atlas-data via the `se` effect already fetched at :528. No `MajorVersion()` branches. The single path covers all 11 versions main exposes (gms 12/48/61/72/79/83/84/87/92/95, jms 185); validate across the supported set (§10 of the PRD), not just v83+v95. Per-version support requires atlas-data serving a nonzero `X` for 1311005 (else the cost silently no-ops) — jms 185 and v95 skill-presence are unverified.
