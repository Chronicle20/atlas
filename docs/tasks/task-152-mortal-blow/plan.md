...
- **Server-side data VERIFIED present (2026-07-27).** Queried the live `atlas-data` baseline (namespace `atlas-main`) via `GET /api/data/skills/{id}` with each legacy tenant's headers. Both **3110001 (Ranger)** and **3210001 (Sniper)** exist with full 20-level effects on **v48/v61/v72/v79** (and v83/84/87/92/95), with per-level `x` (20→50, HP threshold %) and `y` (1→10, kill chance %) **identical to v83**. So `se.X()`/`se.Y()` resolve correctly for every legacy tenant — no data gap. Tenant IDs: v48 `e1f06ae2…`, v61 `0d250dc9…`, v72 `48d415ca…`, v79 `92adbe47…`. (Only `prop` — the client-side shot-conversion roll, which the server never reads — differs: v48 0.32 vs 0.33 elsewhere.)
- **Only remaining unverified piece (client-data question, not a server blocker):** whether the v48/v61/v72/v79 *clients* actually tag a Mortal Blow proc with skill id 3110001/3210001 (the point-blank `prop` conversion) is not IDA-verified here — only v83/v84 were (PRD §4). The design is inert-safe either way, and the server data is confirmed present. Same status as PRD §4.6's v87/v92 note — surface it if a legacy tenant misbehaves; do **not** add version gates or hard-coded fallbacks to compensate.
