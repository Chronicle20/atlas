...
1. **Re-evaluate** if `dirtyAt` is set and `dirtyAt <= now`: atomically claim (clear `dirtyAt`), run the FR-1 computation (§5), write back `active`, `characterLevel`, `skillLevel`. On a state transition (or first evaluation) it **emits the new state inline** and sets `nextBroadcastAt = now + 3s`; an unchanged state emits nothing and leaves `nextBroadcastAt` alone (see the post-implementation revision below).
...
> **Post-implementation revision (deploy-env live test) — broadcast-schedule reset removed.** The step-1 rule "every re-evaluation replaces the schedule with `nextBroadcastAt = now + 5s`" (faithful Cosmic parity: `checkBerserk` cancels + reschedules the 5s task on every HP change) starves the aura in exactly the scenario Berserk is for. `EVENT_TOPIC_CHARACTER_STATUS` carries one `STAT_CHANGED[HP]` per damage/regen tick; each marks the entry dirty → re-evaluates → resets the deadline to `now + 5s`. When HP changes more often than every 5s (sustained combat), the deadline is pushed forward every pass and the broadcast **never comes due** until HP stops changing. Live logs (`atlas-pr-1006`, char 1, MaxHP 30000, x=50) show HP 13157→10164 over 11:14:30–43 with a re-eval on each event and no broadcast, then the aura firing only in the 12s quiet gap after the last hit — matching the reporter's "didn't notice berserk until I stopped taking damage" and "ticked 3–4 times after I potioned above 50%."
>
> **Fix (part 1) — schedule no longer reset on every re-eval:** decouple the broadcast schedule from re-evaluation. A re-evaluation now (re)starts the schedule **only on a state transition** (`active` flips) or the first evaluation; an **unchanged** state leaves `nextBroadcastAt` untouched so the steady 3s re-broadcast keeps its cadence. This drops the 5s initial delay entirely (`InitialBroadcastDelay` removed).
>
> **Fix (part 2) — transitions emitted inline:** the state emission used to happen only in the ticker's broadcast branch, so a detected flip waited for the *next* scan pass to actually broadcast. Live logs (`atlas-pr-1006`, char 1) of a pot above threshold: HP `14667`→`29667`, re-eval flip logged `11:42:02.453`, but the OFF aura event wasn't sent until `11:42:03.5` (the next broadcast pass) — the aura the prior `01.5` tick painted lingered ~1.5s past the pot. `reevaluate` now emits the transition **inline in the same pass it detects the flip**, and advances `nextBroadcastAt` one period so the periodic branch doesn't double-emit. The aura now flips within event+scan latency (~0.5s) of the crossing, down *and* up. `Model.evaluated` is a plain setter again; the schedule policy + inline emit live in `reevaluate`.
>
> Neither part affects the **damage** bonus, which is 100% client-side (`CWvsContext::CheckDarkForce` → `m_nDarkForceDamage`, driven by the client receiving `StatChanged(HP)`); the aura and the damage are independent mechanisms.
...
Consequences, all desirable: consumer lag can't build up behind slow REST calls (FR-5 resilience); re-evaluations per character are serialized by the atomic claim regardless of which topic triggered them (NFR ordering); a burst of HP changes collapses into one re-evaluation at the next pass (coalesced). Re-evaluation now updates state without disturbing the running broadcast cadence unless the state actually flips (see D2's post-implementation revision), so a sustained stream of HP changes no longer starves the aura.
