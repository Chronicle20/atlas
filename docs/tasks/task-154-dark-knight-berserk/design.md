...
1. **Re-evaluate** if `dirtyAt` is set and `dirtyAt <= now`: atomically claim (clear `dirtyAt`), run the FR-1 computation (§5), write back `active`, `characterLevel`, `skillLevel`. The schedule is (re)started at `nextBroadcastAt = now` only on a state transition (or first evaluation); an unchanged state leaves `nextBroadcastAt` alone (see the post-implementation revision below).
...
>
> **Fix:** decouple the broadcast schedule from re-evaluation (`Model.evaluated`). A re-evaluation now (re)starts the schedule at `now` **only on a state transition** (`active` flips) or the first evaluation; an **unchanged** state leaves `nextBroadcastAt` untouched so the steady 3s re-broadcast keeps its cadence. The aura therefore flips within ~1s of the threshold crossing (down *and* up) instead of waiting out a delay that combat kept resetting. This drops the 5s initial delay entirely (`InitialBroadcastDelay` removed). Note this does not affect the **damage** bonus, which is 100% client-side (`CWvsContext::CheckDarkForce` → `m_nDarkForceDamage`, driven by the client receiving `StatChanged(HP)`); the aura and the damage are independent mechanisms.
