...
| ≥15 | `0x5b7bb4` (via `LABEL_32`) | catch-all, same target as 6/7/11/12 | 777 | falls to `default:` | *(unmapped default)* |
...
Three of the thirteen `NPCShopOperation` names present in `template_gms_83_1.json`
have **no v48 case** and are omitted rather than guessed:

- `OVER_LEVEL_REQUIREMENT` (v83 case 15, `Decode4()`-driven dynamic level
  message) — v48 has no arm that calls `Decode4()` in this switch at all; the
  only dynamic-content arm in v48 (`v4==14`, `Decode1()!=0`) reads a *string*
  (`DecodeStr`), not a numeric level.
...
- `TRADE_LIMIT` (v83 case 16, fixed notice) — v48's `v4==16` falls through the
  same catch-all path as `v4==11/12/15/…`, with no distinct string id.

This does not mean v48's `CShopDlg` cannot express these conditions server-side
— only that the client shows no distinguishable notice for them; any v48
trigger of these states surfaces as `GENERIC_ERROR`.
...
(between the existing `0xD5` and `0xED` entries).
