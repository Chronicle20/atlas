...
| V3 | `templates` reads fall back to the baseline row. They are version-keyed, ~76 KB, and already treated as a shared read-only source by the PR bootstrap |
...
## 5. Four places where the obvious approach is wrong
...
- **Task 53 (flip the default) depends on every guard passing**, listed as
  executable commands in its Step 1.
...
| **54** (the proof) | It is one coherent measurement against one live environment. Splitting it would mean deploying the environment more than once |
...
`proof.md` (Task 54) and `query-scope-audit.md` §4 (Task 16). A green
...
| `docs/runbooks/sparse-environments.md` | 29, 45, 46, 49, 53 | The operator runbook: modes, the floor, the gate counters, the alert, the Loki selectors, the MetalLB pool ceiling |
| `proof.md` | 54 | The §17 acceptance proof, with pasted evidence |
