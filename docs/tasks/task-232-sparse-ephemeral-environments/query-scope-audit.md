...
| Login/channel ports + advertised IP | `tools/gen-lb-ports.sh`, `services/atlas-pr-bootstrap/scripts/version-ports.sh` | per-namespace LoadBalancer | Confirmed already scoped (Task 46): `derive_login_port`/`derive_channel_port` (`services/atlas-pr-bootstrap/scripts/version-ports.sh:10-24`) are a pure function of `majorVersion` alone — no environment/namespace input — so every environment on the same major version binds the identical port. Each PR namespace's `atlas-login-lb`/`atlas-channel-lb` requests a fresh dynamic IP from MetalLB's `pr-pool` (`deploy/k8s/overlays/pr/patches/lb-allocate.yaml`, mirrored byte-identical in `pr-sparse/patches/lb-allocate.yaml`), so isolation is same-port-different-IP, which is correct for a LoadBalancer Service. Pool capacity confirmed live (`kubectl -n metallb-system get ipaddresspool -o yaml`): `pr-pool` is `192.168.23.190-192.168.23.209` (20 addresses; the two checked-in comments previously said `.210-.229` and were wrong — corrected in this task), 2 addresses per sparse environment (login + channel) ⇒ ceiling of 10 concurrent environments, matching Task 29's metric-cardinality budget. |
