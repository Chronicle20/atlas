...
- **`$WZ_ARCHIVE` is an external input and is NOT on this machine.** Exhaustive search (`find / -xdev`, `/mnt/c`, `/mnt/d`, `/mnt/e`) found no `.wz` binary over 10 MB. The 51.6 MiB PKG1 `GMS/83.1/Reactor.wz` must be supplied before Tasks 5, 6, 7 and 15 can run. Every command in this plan that needs it writes `"$WZ_ARCHIVE"`.
