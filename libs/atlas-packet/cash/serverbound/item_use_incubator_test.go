...
func TestItemUseIncubatorRoundTrip(t *testing.T) {
	for _, first := range []bool{true, false} {
		for _, v := range pt.Variants {
			t.Run(v.Name, func(t *testing.T) {
				ctx := pt.CreateContext(v.Region, v.MajorVersion, v.MinorVersion)
				input := ItemUseIncubator{inventoryType: 1, slot: -5, updateTime: 1000, updateTimeFirst: first}
				output := *NewItemUseIncubator(first)
...
func TestItemUseIncubatorV83Bytes(t *testing.T) {
	l, _ := testlog.NewNullLogger()
	m := ItemUseIncubator{inventoryType: 1, slot: -5, updateTime: 1000, updateTimeFirst: false}
