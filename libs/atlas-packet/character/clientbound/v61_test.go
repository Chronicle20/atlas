...
// TestCharacterDamageByteOutputV61MobAttackIndex covers the boundary fixed in
...
// TestCharacterDamageByteOutputV61 — only the input attackIdx differs.
func TestCharacterDamageByteOutputV61MobAttackIndex(t *testing.T) {
	ctx := pt.CreateContext("GMS", 61, 1)
	...
	if !bytes.Equal(got, want) {
		t.Errorf("v61 CharacterDamage mob-attack-index wire:\n got %x\nwant %x", got, want)
	}
}

// TestCharacterDamageByteOutputV61CounterOmitsBlock covers the other side of
// the same boundary: DamageTypeCounter (-2) must still omit the block, since
// the guard is strictly `> -2`.
func TestCharacterDamageByteOutputV61CounterOmitsBlock(t *testing.T) {
	ctx := pt.CreateContext("GMS", 61, 1)
	...
	if !bytes.Equal(got, want) {
		t.Errorf("v61 CharacterDamage counter wire:\n got %x\nwant %x", got, want)
	}
}
