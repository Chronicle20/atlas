...
// TestWorldMessageMegaphoneByteOutputV48 — CWvsContext::OnBroadcastMsg
// @0x71c356: mode==2 has no extra pre-switch read (only `if v3==3` at
// 0x71c3cf reads extras); case 2 @0x71c5b1 does chatlog display only, no
// further wire. Wire: Decode1(mode) + DecodeStr(message), byte-identical to
// v61/v72/v83/v95.
// packet-audit:verify packet=chat/clientbound/ChatWorldMessageMegaphone version=gms_v48 ida=0x71c356
func TestWorldMessageMegaphoneByteOutputV48(t *testing.T) {
	ctx := pt.CreateContext("GMS", 48, 1)
	input := NewWorldMessageMegaphone(2, "hi")
	want := []byte{0x02, 0x02, 0x00, 0x68, 0x69}
	got := pt.Encode(t, ctx, input.Encode, nil)
	if !bytes.Equal(got, want) {
		t.Errorf("v48 megaphone: got % x want % x", got, want)
	}
}

// TestWorldMessageSuperMegaphoneByteOutputV48 — CWvsContext::OnBroadcastMsg
// @0x71c356, `if ( v3 == 3 ) { v54=Decode1(a2); v56=Decode1(a2); }`
// @0x71c3cf-0x71c3ea (channelId then whispersOn), AFTER the unconditional
// DecodeStr(message). Wire: mode(1) + message(str) + channelId(1) +
// whispersOn(1).
// packet-audit:verify packet=chat/clientbound/ChatWorldMessageSuperMegaphone version=gms_v48 ida=0x71c356
func TestWorldMessageSuperMegaphoneByteOutputV48(t *testing.T) {
	ctx := pt.CreateContext("GMS", 48, 1)
	...
	got := pt.Encode(t, ctx, input.Encode, nil)
	if !bytes.Equal(got, want) {
		t.Errorf("v48 super megaphone: got % x want % x", got, want)
	}
}
