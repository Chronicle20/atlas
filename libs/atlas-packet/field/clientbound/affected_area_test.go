...
//	[Decode4 tStart — gms_v92 and gms_v95], Decode4 tEnd.
//
...
// The tStart gate is IsRegion("GMS") && MajorAtLeast(92), so JMS185 does NOT
// carry it (the jms export notes confirm: "NO leading tStart"). gms_v92
...
// Decode1 (@0x421881) and the trailing +0x30 slot to Decode1 (@0x4218c4), so the
...
func TestAffectedAreaCreatedByteOutput(t *testing.T) {
	...
	tStart := []byte{0xD2, 0x04, 0x00, 0x00} // tStart = 1234 (gms_v92/gms_v95)
	tEnd := []byte{0x10, 0x27, 0x00, 0x00}   // tEnd = 10000

	// gms_v48 narrows nType to one byte and the trailing +0x30 slot to one byte
	...
	prefixV48 := []byte{
		...
		0x10, // trailing slot = low byte of tEnd (10000 & 0xFF)
	}
	// gms_v12 drops dwOwnerId entirely and reads no trailing time word
