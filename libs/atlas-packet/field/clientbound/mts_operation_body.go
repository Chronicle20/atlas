...
// order the matching CITC sub-handler performs on CInPacket). The notice-only
// "Empty-shape" arms (StringPool notice, zero wire body after the mode byte)
// now each have their OWN discrete per-mode struct in mts_result_empty_modes.go.
// The heavier list arms (Get*ListDone / Get*ItemDone / LoadWishSaleListDone)
// embed the ITCITEM / GW_ItemSlotBase item blob and are modeled in
// mts_operation_list.go (MtsItem).
