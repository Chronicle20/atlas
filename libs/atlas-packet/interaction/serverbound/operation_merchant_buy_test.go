...
// TestOperationMerchantBuyV72Bytes pins the GMS v72 legacy body (mode byte is
// dispatcher-framed, not part of this sub-struct). IDA v72 CPersonalShopDlg::BuyItem#Merchant (sub_665B58, merchant arm mode 0x20 @0x665f00): shared body Encode1(index),Encode2(qty); NO itemCRC (tradeCrcPresent gate). Body == v79.
// packet-audit:verify packet=interaction/serverbound/InteractionOperationMerchantBuy version=gms_v72 ida=0x665b58
func TestOperationMerchantBuyV72Bytes(t *testing.T) {
	l, _ := testlog.NewNullLogger()
