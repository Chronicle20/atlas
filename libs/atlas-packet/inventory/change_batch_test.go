...
func TestChangeBatchQuantityUpdateRoundTrip(t *testing.T) {
	for _, v := range test.Variants {
		t.Run(v.Name, func(t *testing.T) {
			...
			input := NewChangeBatch(false, NewQuantityUpdateEntry(2, 5, 100))
			output := ChangeBatch{}
			...
			qe, ok := output.Entries()[0].(QuantityUpdateEntry)
			if !ok {
...
func TestChangeBatchMoveRoundTrip(t *testing.T) {
	for _, v := range test.Variants {
		t.Run(v.Name, func(t *testing.T) {
			ctx := test.CreateContext(v.Region, v.MajorVersion, v.MinorVersion)
			input := NewChangeBatch(true, NewMoveEntry(2, 3, 7))
			output := ChangeBatch{}
			test.RoundTrip(t, ctx, input.Encode, output.Decode, nil)
			...
			me, ok := output.Entries()[0].(MoveEntry)
			if !ok {
...
func TestChangeBatchMoveEquipRoundTrip(t *testing.T) {
	for _, v := range test.Variants {
		t.Run(v.Name, func(t *testing.T) {
			...
			input := NewChangeBatch(false, NewMoveEntry(1, 5, -1))
			output := ChangeBatch{}
			test.RoundTrip(t, ctx, input.Encode, output.Decode, nil)
			me := output.Entries()[0].(MoveEntry)
			if me.OldSlot() != 5 {
...
func TestChangeBatchRemoveRoundTrip(t *testing.T) {
	for _, v := range test.Variants {
		t.Run(v.Name, func(t *testing.T) {
			ctx := test.CreateContext(v.Region, v.MajorVersion, v.MinorVersion)
			input := NewChangeBatch(false, NewRemoveEntry(2, 3))
			output := ChangeBatch{}
			test.RoundTrip(t, ctx, input.Encode, output.Decode, nil)
			re := output.Entries()[0].(RemoveEntry)
...
func TestChangeBatchAddStackableRoundTrip(t *testing.T) {
	...
	for _, v := range test.Variants {
		t.Run(v.Name, func(t *testing.T) {
			ctx := test.CreateContext(v.Region, v.MajorVersion, v.MinorVersion)
			input := NewChangeBatch(false, NewAddEntry(2, 5, asset))
			output := ChangeBatch{}
			test.RoundTrip(t, ctx, input.Encode, output.Decode, nil)
			ae := output.Entries()[0].(AddEntry)
	...
	for _, v := range test.Variants {
		t.Run(v.Name, func(t *testing.T) {
			...
			input := NewChangeBatch(false,
				NewAddEntry(2, 5, asset),
				NewQuantityUpdateEntry(2, 3, 50),
				NewRemoveEntry(2, 7),
			)
			output := ChangeBatch{}
			test.RoundTrip(t, ctx, input.Encode, output.Decode, nil)
			...
			if _, ok := output.Entries()[0].(AddEntry); !ok {
				t.Errorf("entry[0]: got %T, want AddEntry", output.Entries()[0])
			}
			if _, ok := output.Entries()[1].(QuantityUpdateEntry); !ok {
				t.Errorf("entry[1]: got %T, want QuantityUpdateEntry", output.Entries()[1])
			}
			if _, ok := output.Entries()[2].(RemoveEntry); !ok {
