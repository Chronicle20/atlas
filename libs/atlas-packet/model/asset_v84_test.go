...
// TestEquipableV84ExtraInt pins the v84+ equip durability field. v84's
// GW_ItemSlotEquip::RawDecode (a v95-era refactor v83 lacks) reads nDurability
// as an int between experience and hammersApplied; v83's older inline decode
// does not. atlas wrote the v83 layout for every version, so a v84 client
// under-ran each equipped item by 4 bytes (×4 starting equips on a fresh
// character) → ZException → silent disconnect entering the channel. The encoded
// equip must therefore be exactly 4 bytes longer for GMS v84+ than for v83.
