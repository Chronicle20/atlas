...
// SkillCancelInfo holds the body of a serverbound SendSkillCancelRequest packet.
// Wire order (all versions): skillId u32. Identical across all five versions.
type SkillCancelInfo struct {
	skillId uint32
}

func NewSkillCancelInfo() *SkillCancelInfo {
	return &SkillCancelInfo{}
}

// Getter

func (m *SkillCancelInfo) SkillId() uint32 { return m.skillId }

// Setter

func (m *SkillCancelInfo) SetSkillId(skillId uint32) *SkillCancelInfo {
	m.skillId = skillId
	return m
}

// Encode serializes the serverbound cancel body: skillId u32 (all versions).
func (m *SkillCancelInfo) Encode(l logrus.FieldLogger, ctx context.Context) func(options map[string]interface{}) []byte {
	return func(options map[string]interface{}) []byte {
		w := response.NewWriter(l)
		w.WriteInt(m.skillId)
		return w.Bytes()
...
// Decode deserializes the serverbound cancel body from r.
func (m *SkillCancelInfo) Decode(l logrus.FieldLogger, ctx context.Context) func(r *request.Reader, options map[string]interface{}) {
	return func(r *request.Reader, options map[string]interface{}) {
		m.skillId = r.ReadUint32()
	}
}
