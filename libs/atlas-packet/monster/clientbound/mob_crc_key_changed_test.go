...
// TestMobCrcKeyChangedBytesV72 pins the v72 wire. MOB_CRC_KEY_CHANGED (op 221) is
// NOT a per-mob OnMobPacket case; it is dispatched at the CMobPool top level to
// CMobPool::OnMobCrcKeyChanged @0x625a42 (GMS_v72.1_U_DEVM.exe, port 13339):
//
//	Decode4 @0x625a5a — m_dwMobCrcKey (the new mob CRC key)
//
// No uniqueId prefix (pool-level, no GetMob). Byte-identical to v79.
//
// packet-audit:verify packet=monster/clientbound/MonsterMobCrcKeyChanged version=gms_v72 ida=0x625a42
func TestMobCrcKeyChangedBytesV72(t *testing.T) {
	input := NewMobCrcKeyChanged(0x12345678)
	ctx := pt.CreateContext("GMS", 72, 1)
	want := []byte{
		0x78, 0x56, 0x34, 0x12, // crcKey uint32 LE = 0x12345678 (Decode4 @0x625a5a)
	}
	got := input.Encode(nil, ctx)(nil)
	if !bytes.Equal(got, want) {
		t.Errorf("v72 mobCrcKeyChanged bytes:\n got % x\nwant % x", got, want)
	}
}
