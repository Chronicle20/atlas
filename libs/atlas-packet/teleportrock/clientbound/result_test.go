...
// task-124 v61 verify pass (live GMS_v61.1_U_DEVM.exe, port 13338): already
// named in the v61 IDB. CWvsContext::OnMapTransferResult @0x846a0f —
// byte-identical read order to v83/v84/v87/v95/jms_v185:
// mode(Decode1)+targetList(Decode1) @0x846a2f/0x846a32, then for mode in
// {2,3} exactly 5 (targetList==0) or 10 (targetList==1) x Decode4 mapId
// @0x846a82 loop. Confirms the "identical v83 0xA25268 / v95 0x9F9F90" claim
// in the file-level comment above for v61 too.
//
// packet-audit:verify packet=teleportrock/clientbound/MapTransferList version=gms_v61 ida=0x846a0f
func TestMapTransferListRegularGoldenV61(t *testing.T) {
	l, _ := testlog.NewNullLogger()
...
// IDA (live GMS_v61.1_U_DEVM.exe v61, port 13338, task-124 verify pass):
// CWvsContext::OnMapTransferResult @0x846a0f, same function as the list form
// above — modes 5-11 (error/notice form, string ids resolved via
// sub_678022 with StringPool ids 499/2876/2911/2882/2879/2883 — a small
// offset from v83's ids that does not affect wire bytes) read only
// mode(Decode1)+targetList(Decode1) and never reach the Decode4 mapId loop
// (guarded on `case 2/3` in the v61 switch, byte-identical branch structure
// to v83's `v3 == 3 && !v4` guard). Same class of runtime/mode-guard
// tooling gap as v83/v84/v87/v95/jms (analyzer grades this candidate
// FlatInvalid even though the 2-byte body is exactly what v61 sends for the
// error modes). Evidence pinned to carry this cell via the linked-fixture
// path, mirroring the v83/v84/v87/v95/jms convention above.
//
// packet-audit:verify packet=teleportrock/clientbound/MapTransferError version=gms_v61 ida=0x846a0f
func TestMapTransferErrorGoldenV61(t *testing.T) {
	l, _ := testlog.NewNullLogger()
	ctx := pt.CreateContext("GMS", 61, 1)
	m := NewMapTransferError(5, false)
	got := m.Encode(l, ctx)(nil)
...
// task-124 v72 verify pass (live GMS_v72.1_U_DEVM.exe, port 13339): already
// named in the v72 IDB. CWvsContext::OnMapTransferResult @0x91d41a —
// byte-identical read order to v83/v84/v87/v95/jms_v185/v61:
// mode(Decode1)+targetList(Decode1) @0x91d43a/0x91d43d, then for mode in
// {2,3} exactly 5 (targetList==0) or 10 (targetList==1) x Decode4 mapId into
// this+1279 (regular) / this+1299 (VIP). Corrected ida.address in the
// registry from a shared batch-import placeholder to this confirmed
// function entry as part of this pass. Confirms the "identical v83 0xA25268
// / v95 0x9F9F90" claim in the file-level comment above for v72 too.
//
// packet-audit:verify packet=teleportrock/clientbound/MapTransferList version=gms_v72 ida=0x91d41a
func TestMapTransferListRegularGoldenV72(t *testing.T) {
	l, _ := testlog.NewNullLogger()
...
// IDA (live GMS_v72.1_U_DEVM.exe v72, port 13339, task-124 verify pass):
// CWvsContext::OnMapTransferResult @0x91d41a, same function as the list form
// above — modes 5-11 (error/notice form, StringPool ids
// 2960/2925/2960/2931/2928/2932) read only mode(Decode1)+targetList(Decode1)
// and never reach the Decode4 mapId loop (guarded on `case 2/3` in the v72
// switch, byte-identical branch structure to v83's `v3 == 3 && !v4` guard).
// Same class of runtime/mode-guard tooling gap as v83/v84/v87/v95/jms/v61
// (analyzer grades this candidate FlatInvalid even though the 2-byte body is
// exactly what v72 sends for the error modes). Evidence pinned to carry this
// cell via the linked-fixture path, mirroring the v83/v84/v87/v95/jms/v61
// convention above.
//
// packet-audit:verify packet=teleportrock/clientbound/MapTransferError version=gms_v72 ida=0x91d41a
func TestMapTransferErrorGoldenV72(t *testing.T) {
	l, _ := testlog.NewNullLogger()
	ctx := pt.CreateContext("GMS", 72, 1)
	m := NewMapTransferError(5, false)
	got := m.Encode(l, ctx)(nil)
...
// task-124 v79 verify pass (live GMS_v79_1_DEVM.exe, port 13340): already
// named in the v79 IDB. CWvsContext::OnMapTransferResult @0x96f362 —
// byte-identical read order to v61/v72/v83/v84/v87/v95/jms_v185:
// mode(Decode1)+targetList(Decode1) @0x96f382/0x96f385, then for mode in
// {2,3} exactly 5 (targetList==0) or 10 (targetList==1) x Decode4 mapId into
// this+1303 (regular) / this+1323 (VIP). Corrected ida.address in the
// registry from a stale shared batch-import placeholder (9779200 / 0x953400)
// to this confirmed function entry as part of this pass. Confirms the
// "identical v83 0xA25268 / v95 0x9F9F90" claim in the file-level comment
// above for v79 too.
//
// packet-audit:verify packet=teleportrock/clientbound/MapTransferList version=gms_v79 ida=0x96f362
func TestMapTransferListRegularGoldenV79(t *testing.T) {
	l, _ := testlog.NewNullLogger()
...
// IDA (live GMS_v79_1_DEVM.exe v79, port 13340, task-124 verify pass):
// CWvsContext::OnMapTransferResult @0x96f362, same function as the list form
// above — modes 5-11 (error/notice form, StringPool ids
// 2963/2928/2963/2934/2931/2935) read only mode(Decode1)+targetList(Decode1)
// and never reach the Decode4 mapId loop (guarded on `case 2/3` in the v79
// switch, byte-identical branch structure to v83's `v3 == 3 && !v4` guard).
// Same class of runtime/mode-guard tooling gap as v83/v84/v87/v95/jms/v61/v72
// (analyzer grades this candidate FlatInvalid even though the 2-byte body is
// exactly what v79 sends for the error modes). Evidence pinned to carry this
// cell via the linked-fixture path, mirroring the v83/v84/v87/v95/jms/v61/v72
// convention above.
//
// packet-audit:verify packet=teleportrock/clientbound/MapTransferError version=gms_v79 ida=0x96f362
func TestMapTransferErrorGoldenV79(t *testing.T) {
	l, _ := testlog.NewNullLogger()
	ctx := pt.CreateContext("GMS", 79, 1)
	m := NewMapTransferError(5, false)
	got := m.Encode(l, ctx)(nil)
