...
// encodeFilterPaeth unconditionally applies the encoder-side Paeth filter to
// cr[0] using pr as the previous row, writing the result into cr[ftPaeth].
// Unlike filter(), it does not early-exit on a running sum; the entire row
// must be filled because the caller uses cr[ftPaeth] verbatim.
//
// Distinct from filterPaeth in paeth.go which is the decoder-side reverse
// Paeth transform.
//
// Pinned for byte-identity (atlas determinism), not encode-size optimality.
func encodeFilterPaeth(cr *[nFilter][]byte, pr []byte, bpp int) {
	cdat0 := cr[0][1:]
	cdat4 := cr[ftPaeth][1:]
	pdat := pr[1:]
	n := len(cdat0)
...
func (e *encoder) writeImage(w io.Writer, m image.Image, cb int, level int) error {
	...
	for y := b.Min.Y; y < b.Max.Y; y++ {
		...
		//
		// Pinned for byte-identity (atlas determinism), not encode-size optimality.
		// Upstream Go 1.21 tries all five PNG filter types per scanline and picks
		// min-sum-of-absolute-differences; that heuristic could shift between Go
		// versions. We always emit ftPaeth (already a strong default for the
		// natural-image sprite sheets this package targets) so the bytes are stable.
		f := ftNone
		if level != zlib.NoCompression && cb != cbP8 && cb != cbP4 && cb != cbP2 && cb != cbP1 {
			// Since we skip paletted images we don't have to worry about
			// bitsPerPixel not being a multiple of 8
			bpp := bitsPerPixel / 8
			encodeFilterPaeth(&cr, pr, bpp)
			f = ftPaeth
		}
...
// Encoder.CompressionLevel to map to zlib.BestCompression.
//
// Pinned for byte-identity (atlas determinism), not encode-size optimality.
// Upstream Go 1.21 maps DefaultCompression to zlib.DefaultCompression; we map
// it to zlib.BestCompression so that callers passing the zero value always get
// the same deterministic output across host Go versions.
