...
func InitResource(si jsonapi.ServerInformation) func(db *gorm.DB) server.RouteInitializer {
	return func(db *gorm.DB) server.RouteInitializer {
		return func(router *mux.Router, l logrus.FieldLogger) {
			...
			r.HandleFunc("/", register("get_history", handleGetHistory(db))).Methods(http.MethodGet)
			r.HandleFunc("/accounts/{accountId}", register("get_history_by_account", handleGetHistoryByAccountId(db))).Methods(http.MethodGet)
		}
	}
}

func handleGetHistory(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			if ip != "" || hwid != "" {
				...
				if ip != "" {
					paged, err = NewProcessor(d.Logger(), d.Context(), db).ByIPPagedProvider(ip, page)()
				} else {
					paged, err = NewProcessor(d.Logger(), d.Context(), db).ByHWIDPagedProvider(hwid, page)()
				}
				if err != nil {
...
func handleGetHistoryByAccountId(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseAccountId(d.Logger(), func(accountId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				...
				paged, err := NewProcessor(d.Logger(), d.Context(), db).ByAccountIdProvider(accountId, page)()
				if err != nil {
