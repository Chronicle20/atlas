...
// allEntityProvider lists the tenant's coupons, narrowed by f.
//
...
func allEntityProvider(t tenant.Model, f Filters) database.EntityProvider[[]Entity] {
	return func(db *gorm.DB) model.Provider[[]Entity] {
		...
		var results []Entity
		if err := tx.Order("created_at").Find(&results).Error; err != nil {
			return model.ErrorProvider[[]Entity](err)
		}
