...
// returning the updated count. atlas-account enforces the 12-per-world cap
// and returns 409 (surfaced here as requests.ErrConflict) when the pair is
// already at the cap; the caller must check the cap itself before deciding
// to increment (task-246 bug-b-type-must-add-a-slot.md).
func (p *ProcessorImpl) IncrementCharacterSlots(accountId uint32, worldId world.Id) (int16, error) {
	result, err := requestIncrementCharacterSlots(p.ctx, accountId, byte(worldId))(p.l, p.ctx)
	if err != nil {
		return 0, err
	}
	return result.Slots, nil
