...
func TestKarmaUsedRoundTrip(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			m := NewBuilder(uuid.New(), tc.templateId).SetId(1).SetKarmaUsed(true).MustBuild()
			if !m.KarmaUsed() {
				t.Fatalf("KarmaUsed() = false after SetKarmaUsed(true) for template %d", tc.templateId)
			}
			cleared := Clone(m).SetKarmaUsed(false).MustBuild()
...
func TestKarmaUsedLeavesSpikesAlone(t *testing.T) {
	spiked := NewBuilder(uuid.New(), 1002357).SetId(1).AddFlag(af.FlagSpikes).SetKarmaUsed(true).MustBuild()
	if !spiked.Spikes() {
	...
	plain := NewBuilder(uuid.New(), 1002357).SetId(1).SetKarmaUsed(true).MustBuild()
	if plain.Spikes() {
