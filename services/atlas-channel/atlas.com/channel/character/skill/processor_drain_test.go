...
// /characters/{characterId}/skills is now paginated (task-117); the fixture
// server serves 300 skills across two pages of 250 -- only a genuine drain
// picks up skill id 300, which lives on page 2.
func TestByCharacterIdProviderDrainsBeyondOnePage(t *testing.T) {
	...
	ms, err := skill.NewProcessor(l, ctx).GetByCharacterId(characterId)
	if err != nil {
		t.Fatal(err)
	}
	if len(ms) != 300 {
