...
func TestProcessor_FirstGetPopulatesLazily_SecondGetIsZeroRest(t *testing.T) {
	tests := []struct {
		...
		{"first Get populates lazily, second Get performs zero REST", func(t *testing.T) {
...
func TestProcessor_ComposedMatchesDecoratorPath(t *testing.T) {
	tests := []struct {
		...
		{"snapshot-composed model matches today's decorator chain", func(t *testing.T) {
			resetRegistryForTest(t)
			p, _ := newTestProcessor(t)
...
func TestProcessor_PerComponentFallbackOnlyRefetchesInvalidComponent(t *testing.T) {
	tests := []struct {
		...
		{"fallback only refetches the invalidated component", func(t *testing.T) {
			resetRegistryForTest(t)
...
func TestProcessor_FallbackFailureSurfacesError(t *testing.T) {
	tests := []struct {
		...
		// stale-success.
		{"core fallback failure propagates rather than degrading", func(t *testing.T) {
			resetRegistryForTest(t)
			p, tm := newTestProcessor(t)
			counts := installFetchSeams(t, 7)
...
func TestProcessor_InventoryFallbackFailureServesDegradedModel(t *testing.T) {
	tests := []struct {
		...
		{"inventory fallback failure serves a degraded model, not an error", func(t *testing.T) {
			resetRegistryForTest(t)
			p, _ := newTestProcessor(t)
			restoreFetchSeams(t)
...
func TestProcessor_SkillsFallbackFailureServesDegradedModel(t *testing.T) {
	tests := []struct {
		...
		{"skills fallback failure serves a degraded model, not an error", func(t *testing.T) {
			resetRegistryForTest(t)
			p, _ := newTestProcessor(t)
			restoreFetchSeams(t)
...
func TestProcessor_StaleBackfillStillServesThisCaller(t *testing.T) {
	tests := []struct {
		...
		{"a gen bump mid-fetch discards the backfill but still serves this caller", func(t *testing.T) {
			resetRegistryForTest(t)
			p, tm := newTestProcessor(t)
			installFetchSeams(t, 7)
			if _, err := p.Get(7); err != nil {
...
func TestProcessor_PositionOverlayServesLocalFold(t *testing.T) {
	tests := []struct {
		...
		{"position overlay is served from the local fold", func(t *testing.T) {
			resetRegistryForTest(t)
			p, tm := newTestProcessor(t)
			installFetchSeams(t, 7)
			if _, err := p.Get(7); err != nil {
...
func TestProcessor_GetBuffs_LazySeedThenEventMaintained(t *testing.T) {
	tests := []struct {
		...
		{"buffs are lazily seeded via REST, then maintained by events with zero further REST", func(t *testing.T) {
			resetRegistryForTest(t)
			p, tm := newTestProcessor(t)
			counts := installFetchSeams(t, 7)
...
func TestProcessor_GetBuffs_SelfExpiresPastExpiresAt(t *testing.T) {
	tests := []struct {
		...
		{"reads self-filter buffs past their natural expiresAt", func(t *testing.T) {
			resetRegistryForTest(t)
			p, tm := newTestProcessor(t)
			installFetchSeams(t, 7)
