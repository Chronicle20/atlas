...
// Op is the unit of work emitted by ComputeOps and consumed by the
// apply loop. For OpDrain, Cfg is zero — only Key is needed.
type Op struct {
	Kind OpKind
	Key  server.Key
	Cfg  ListenerConfig
...
// reconcile prev → next. A PORT CHANGE on the same Key emits Drain
// then Add (two ops); the apply loop must execute them in order.
//
...
// per (tenant, world, channel) triple. Tenants without a matching
// tenant config are skipped — both topics must agree before we admit
// a listener.
func flatten(svc *configuration.RestModel, tenants map[uuid.UUID]tenant.RestModel) map[server.Key]desired {
	out := make(map[server.Key]desired)
	if svc == nil {
		return out
	}
	for _, st := range svc.Tenants {
		...
		for _, w := range st.Worlds {
			for _, c := range w.Channels {
				k := server.Key{
					TenantId:  tid,
					WorldId:   world.Id(w.Id),
					ChannelId: channel.Id(c.Id),
				}
				out[k] = desired{
					Key: k,
					Cfg: ListenerConfig{
