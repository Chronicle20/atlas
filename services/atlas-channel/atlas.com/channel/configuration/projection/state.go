...
// Snapshot's first return is nil. atlas-channel cannot run without a
// service config — the apply loop will respond by draining every
// listener if this transition is observed (handled in Phase G/J).
...
// (subscriber.go:112), so a non-nil service is by construction our own.
