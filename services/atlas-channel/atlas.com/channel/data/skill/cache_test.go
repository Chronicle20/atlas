...
// TestSkillCache is table-driven over the skill-template TTL cache's
// distinct behaviours (DOM-20). Each case's body is preserved verbatim from
// the original single-purpose test function, including its exact
// assertions, so no scenario's checking strength changed in the conversion.
