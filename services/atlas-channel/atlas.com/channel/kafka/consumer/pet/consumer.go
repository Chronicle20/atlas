	...
	return func(sc server.Model) func(wp writer.Producer) func(rf func(topic string, handler handler.Handler) (string, error)) error {
		return func(wp writer.Producer) func(rf func(topic string, handler handler.Handler) (string, error)) error {
			return func(rf func(topic string, handler handler.Handler) (string, error)) error {
				...
				if _, err := rf(t, message.AdaptHandler(message.PersistentConfig(handleSpawned(sc, wp)))); err != nil {
					return err
				}
				if _, err := rf(t, message.AdaptHandler(message.PersistentConfig(handleDespawned(sc, wp)))); err != nil {
					return err
				}
				if _, err := rf(t, message.AdaptHandler(message.PersistentConfig(handleCommandResponse(sc, wp)))); err != nil {
					return err
				}
				if _, err := rf(t, message.AdaptHandler(message.PersistentConfig(handleClosenessChanged(sc, wp)))); err != nil {
					return err
				}
				if _, err := rf(t, message.AdaptHandler(message.PersistentConfig(handleFullnessChanged(sc, wp)))); err != nil {
					return err
				}
				if _, err := rf(t, message.AdaptHandler(message.PersistentConfig(handleLevelChanged(sc, wp)))); err != nil {
					return err
				}
				if _, err := rf(t, message.AdaptHandler(message.PersistentConfig(handleSlotChanged(sc, wp)))); err != nil {
					return err
				}
				if _, err := rf(t, message.AdaptHandler(message.PersistentConfig(handleExcludeChanged(sc, wp)))); err != nil {
					return err
				}
				return nil
			}
		...
		return func(wp writer.Producer) func(p pet.Model) model.Operator[session.Model] {
			return func(p pet.Model) model.Operator[session.Model] {
				return func(s session.Model) error {
					...
					err = _map.NewProcessor(l, ctx).ForOtherSessionsInMap(s.Field(), s.CharacterId(), session.Announce(l)(ctx)(wp)(writer.PetActivated)(writer.PetSpawnBody(p)))
					if err != nil {
		...
		return func(wp writer.Producer) func(slot int8, reason string) model.Operator[session.Model] {
			return func(slot int8, reason string) model.Operator[session.Model] {
				return func(s session.Model) error {
					...
					err = session.Announce(l)(ctx)(wp)(writer.PetActivated)(writer.PetDespawnBody(s.CharacterId(), slot, reason))(s)
					if err != nil {
						l.WithError(err).Errorf("Unable to write pet despawned to character.")
					}
					err = _map.NewProcessor(l, ctx).ForOtherSessionsInMap(s.Field(), s.CharacterId(), session.Announce(l)(ctx)(wp)(writer.PetActivated)(writer.PetDespawnBody(s.CharacterId(), slot, reason)))
					if err != nil {
	...
	return func(l logrus.FieldLogger, ctx context.Context, e pet2.StatusEvent[pet2.CommandResponseStatusEventBody]) {
		...
		go func() {
			p := pet.NewModelBuilder(e.PetId, 0, 0, "").
				...
				SetFullness(e.Body.Fullness).
				MustBuild()
			_ = _map.NewProcessor(l, ctx).ForSessionsInMap(s.Field(), session.Announce(l)(ctx)(wp)(writer.PetCommandResponse)(writer.PetCommandResponseBody(p, e.Body.CommandId, e.Body.Success, false)))
		}()
	}
}
	...
	return func(ctx context.Context) func(wp writer.Producer) func(petId uint32, ownerId uint32) model.Operator[session.Model] {
		...
		return func(wp writer.Producer) func(petId uint32, ownerId uint32) model.Operator[session.Model] {
			return func(petId uint32, ownerId uint32) model.Operator[session.Model] {
				...
				return session.Announce(l)(ctx)(wp)(writer.CharacterInventoryChange)(writer.CharacterInventoryRefreshAsset(inventory2.TypeValueCash, *a))
			}
		}
	}
}
		...
		_ = session.NewProcessor(l, ctx).IfPresentByCharacterId(sc.Channel())(e.OwnerId, func(s session.Model) error {
			...
			return _map.NewProcessor(l, ctx).ForSessionsInMap(s.Field(), func(os session.Model) error {
				if s.CharacterId() == os.CharacterId() {
					err = session.Announce(l)(ctx)(wp)(writer.CharacterEffect)(writer.CharacterPetEffectBody(byte(e.Body.Slot), 0))(os)
					if err != nil {
						l.WithError(err).Errorf("Unable to issue pet [%d] level up.", p.Id())
					}
					return err
				} else {
					err = session.Announce(l)(ctx)(wp)(writer.CharacterEffectForeign)(writer.CharacterPetEffectForeignBody(s.CharacterId(), byte(e.Body.Slot), 0))(os)
					if err != nil {
						l.WithError(err).Errorf("Unable to issue pet [%d] level up.", p.Id())
					}
					return err
...
func handleSlotChanged(sc server.Model, wp writer.Producer) message.Handler[pet2.StatusEvent[pet2.SlotChangedStatusEventBody]] {
	return func(l logrus.FieldLogger, ctx context.Context, e pet2.StatusEvent[pet2.SlotChangedStatusEventBody]) {
		...
		_ = session.NewProcessor(l, ctx).IfPresentByCharacterId(sc.Channel())(e.OwnerId, func(s session.Model) error {
			...
			err := session.Announce(l)(ctx)(wp)(writer.StatChanged)(writer.StatChangedBody([]model2.StatUpdate{model2.NewStatUpdate(statType, sn)}, true))(s)
			if err != nil {
				return err
			}
