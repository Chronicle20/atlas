...
func InitConsumers(l logrus.FieldLogger) func(func(config consumer.Config, decorators ...model.Decorator[consumer.Config])) func(consumerGroupId string) {
	return func(rf func(config consumer.Config, decorators ...model.Decorator[consumer.Config])) func(consumerGroupId string) {
		return func(consumerGroupId string) {
			rf(consumer2.NewConfig(l)("storage_status_event")(storage2.EnvEventTopicStatus)(consumerGroupId), consumer.SetHeaderParsers(consumer.SpanHeaderParser, consumer.TenantHeaderParser, consumer.EnvHeaderParser), consumer.SetStartOffset(kafka.LastOffset))
			rf(consumer2.NewConfig(l)("storage_compartment_status_event")(storage2.EnvEventTopicStorageCompartmentStatus)(consumerGroupId), consumer.SetHeaderParsers(consumer.SpanHeaderParser, consumer.TenantHeaderParser, consumer.EnvHeaderParser), consumer.SetStartOffset(kafka.LastOffset))
		}
	}
}
	...
	return func(sc server.Model) func(wp writer.Producer) func(rf func(topic string, handler handler.Handler) (string, error)) ([]listener.HandlerHandle, error) {
		return func(wp writer.Producer) func(rf func(topic string, handler handler.Handler) (string, error)) ([]listener.HandlerHandle, error) {
			return func(rf func(topic string, handler handler.Handler) (string, error)) ([]listener.HandlerHandle, error) {
				...
				t, err = topic.EnvProvider(l)(storage2.EnvEventTopicStatus)()
				if err != nil {
					return nil, err
				}
				id, err := rf(t, message.AdaptHandler(message.PersistentConfig(handleMesosUpdatedEvent(sc, wp))))
				if err != nil {
					return nil, err
				}
				handles = append(handles, listener.HandlerHandle{Topic: t, Id: id})
				id, err = rf(t, message.AdaptHandler(message.PersistentConfig(handleArrangedEvent(sc, wp))))
				if err != nil {
					return nil, err
				}
				handles = append(handles, listener.HandlerHandle{Topic: t, Id: id})
				id, err = rf(t, message.AdaptHandler(message.PersistentConfig(handleStorageErrorEvent(sc, wp))))
				if err != nil {
					return nil, err
				}
				handles = append(handles, listener.HandlerHandle{Topic: t, Id: id})
				id, err = rf(t, message.AdaptHandler(message.PersistentConfig(handleProjectionCreatedEvent(sc, wp))))
				if err != nil {
					return nil, err
				}
				handles = append(handles, listener.HandlerHandle{Topic: t, Id: id})

				t, err = topic.EnvProvider(l)(storage2.EnvEventTopicStorageCompartmentStatus)()
				if err != nil {
					return nil, err
				}
				id, err = rf(t, message.AdaptHandler(message.PersistentConfig(handleStorageCompartmentAcceptedEvent(sc, wp))))
				if err != nil {
					return nil, err
				}
				handles = append(handles, listener.HandlerHandle{Topic: t, Id: id})
				id, err = rf(t, message.AdaptHandler(message.PersistentConfig(handleStorageCompartmentReleasedEvent(sc, wp))))
				if err != nil {
					return nil, err
				}
				handles = append(handles, listener.HandlerHandle{Topic: t, Id: id})
...
func handleStorageCompartmentAcceptedEvent(sc server.Model, wp writer.Producer) message.Handler[storage2.StorageCompartmentEvent[storage2.CompartmentAcceptedEventBody]] {
	return func(l logrus.FieldLogger, ctx context.Context, e storage2.StorageCompartmentEvent[storage2.CompartmentAcceptedEventBody]) {
		...
		if err != nil {
			...
			err = session.NewProcessor(l, ctx).IfPresentByCharacterId(sc.Channel())(e.CharacterId, session.Announce(l)(ctx)(wp)(storagecb.StorageOperationWriter)(storagepkt.StorageOperationStoreAssetsBody(storageData.Capacity, uint64(inventoryTypeToFlag(inventoryType)), toPacketAssets(inventoryType, storageData.Assets))))
			if err != nil {
		...
		err = session.NewProcessor(l, ctx).IfPresentByCharacterId(sc.Channel())(e.CharacterId, session.Announce(l)(ctx)(wp)(storagecb.StorageOperationWriter)(storagepkt.StorageOperationStoreAssetsBody(projData.Capacity, uint64(inventoryTypeToFlag(inventoryType)), toPacketAssets(inventoryType, assets))))
		if err != nil {
			l.WithError(err).Errorf("Unable to send storage update to character [%d].", e.CharacterId)
		}
	}
...
func handleStorageCompartmentReleasedEvent(sc server.Model, wp writer.Producer) message.Handler[storage2.StorageCompartmentEvent[storage2.CompartmentReleasedEventBody]] {
	return func(l logrus.FieldLogger, ctx context.Context, e storage2.StorageCompartmentEvent[storage2.CompartmentReleasedEventBody]) {
		...
		if err != nil {
			...
			err = session.NewProcessor(l, ctx).IfPresentByCharacterId(sc.Channel())(e.CharacterId, session.Announce(l)(ctx)(wp)(storagecb.StorageOperationWriter)(storagepkt.StorageOperationRetrieveAssetsBody(storageData.Capacity, uint64(inventoryTypeToFlag(inventoryType)), toPacketAssets(inventoryType, storageData.Assets))))
			if err != nil {
				l.WithError(err).Errorf("Unable to send storage update to character [%d].", e.CharacterId)
			}
			return
		...
		// Send updated storage assets for the affected compartment only
		err = session.NewProcessor(l, ctx).IfPresentByCharacterId(sc.Channel())(e.CharacterId, session.Announce(l)(ctx)(wp)(storagecb.StorageOperationWriter)(storagepkt.StorageOperationRetrieveAssetsBody(projData.Capacity, uint64(inventoryTypeToFlag(inventoryType)), toPacketAssets(inventoryType, assets))))
		if err != nil {
			l.WithError(err).Errorf("Unable to send storage update to character [%d].", e.CharacterId)
		}
	}
