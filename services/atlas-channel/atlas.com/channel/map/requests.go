...
// charactersInMapAllInstancesUrl returns the list URL for the characters
// currently in a map across every instance. Bare URL for the same reason as
// charactersInMapUrl.
func charactersInMapAllInstancesUrl(worldId world.Id, channelId channel.Id, mapId _map.Id) string {
	return fmt.Sprintf(getBaseRequest()+mapCharactersResource, worldId, channelId, mapId)
}
