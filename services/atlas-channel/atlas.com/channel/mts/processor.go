...
type Processor interface {
	CreateListing(transactionId uuid.UUID, worldId world.Id, sellerId uint32, sellerAccountId uint32, sellerName string, saleType string, sourceInventoryType byte, assetId uint32, quantity uint32, listValue uint32, buyNowPrice *uint32, durationHours int, minIncrement uint32, category string, subCategory string, offerWishSerial uint32, offerWishOwnerId uint32, itemId uint32, sellerLevel byte) error
	Buy(transactionId uuid.UUID, worldId world.Id, serial uint32, buyerId uint32, buyerAccountId uint32, buyNow bool, resultKind string) error
	...
	RegisterWish(worldId world.Id, characterId uint32, itemId uint32, listingSerial uint32, price uint32, count uint32, origin string) error
...
func (p *ProcessorImpl) CreateListing(transactionId uuid.UUID, worldId world.Id, sellerId uint32, sellerAccountId uint32, sellerName string, saleType string, sourceInventoryType byte, assetId uint32, quantity uint32, listValue uint32, buyNowPrice *uint32, durationHours int, minIncrement uint32, category string, subCategory string, offerWishSerial uint32, offerWishOwnerId uint32, itemId uint32, sellerLevel byte) error {
	...
	return producer.ProviderImpl(p.l)(p.ctx)(mtsmsg.EnvCommandTopic)(CreateListingCommandProvider(transactionId, worldId, sellerId, sellerAccountId, sellerName, saleType, sourceInventoryType, assetId, quantity, listValue, buyNowPrice, durationHours, minIncrement, category, subCategory, offerWishSerial, offerWishOwnerId, itemId, sellerLevel))
}

func (p *ProcessorImpl) Buy(transactionId uuid.UUID, worldId world.Id, serial uint32, buyerId uint32, buyerAccountId uint32, buyNow bool, resultKind string) error {
	p.l.Debugf("Character [%d] buying MTS listing serial [%d] (buyNow [%t], resultKind [%s]).", buyerId, serial, buyNow, resultKind)
	return producer.ProviderImpl(p.l)(p.ctx)(mtsmsg.EnvCommandTopic)(BuyCommandProvider(transactionId, worldId, serial, buyerId, buyerAccountId, buyNow, resultKind))
}
...
func (p *ProcessorImpl) RegisterWish(worldId world.Id, characterId uint32, itemId uint32, listingSerial uint32, price uint32, count uint32, origin string) error {
	p.l.Debugf("Character [%d] registering MTS wish for item [%d] (listingSerial [%d], price [%d], count [%d], origin [%s]).", characterId, itemId, listingSerial, price, count, origin)
	return producer.ProviderImpl(p.l)(p.ctx)(mtsmsg.EnvCommandTopic)(RegisterWishCommandProvider(worldId, characterId, itemId, listingSerial, price, count, origin))
}
