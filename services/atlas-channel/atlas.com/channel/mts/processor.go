...
type Processor interface {
	...
	RegisterWish(worldId world.Id, characterId uint32, itemId uint32, price uint32, count uint32, origin string) error
...
func (p *ProcessorImpl) RegisterWish(worldId world.Id, characterId uint32, itemId uint32, price uint32, count uint32, origin string) error {
	p.l.Debugf("Character [%d] registering MTS wish for item [%d] (price [%d], count [%d], origin [%s]).", characterId, itemId, price, count, origin)
	return producer.ProviderImpl(p.l)(p.ctx)(mtsmsg.EnvCommandTopic)(RegisterWishCommandProvider(worldId, characterId, itemId, price, count, origin))
}
