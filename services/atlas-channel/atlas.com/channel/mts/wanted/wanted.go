...
// display name into the sGameID column. The name lookup is best-effort: a failure
// yields "" (a blank seller column) and the want-ad is STILL included — a want-ad
// must never be dropped because its owner's name could not be resolved.
func item(l logrus.FieldLogger, ctx context.Context, w mtswish.Model) fieldcb.MtsItem {
	ownerName := ""
