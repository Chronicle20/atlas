...
// newSessionContext builds the base context for a channel socket session:
...
// connects to the PR's own channel service and the environment is
	...
	return func(t tenant.Model, wp writer.Producer) Adapter {
		return func(name string, v MessageValidator, h MessageHandler, readerOptions map[string]interface{}) request.Handler {
			return func(sessionId uuid.UUID, r request.Reader) {
				...
				sctx, span := otel.GetTracerProvider().Tracer("atlas-channel").Start(newSessionContext(t), name)
