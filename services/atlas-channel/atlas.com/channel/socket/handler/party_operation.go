	...
	return func(s session.Model, r *request.Reader, readerOptions map[string]interface{}) {
		...
		if isPartyOperation(l)(readerOptions, op, PartyOperationInvite) {
			...
			if err != nil {
				...
				err := session.Announce(l)(ctx)(wp)(party2.PartyOperationWriter)(writer.PartyErrorBody("UNABLE_TO_FIND_THE_CHARACTER", sp.Name()))(s)
				if err != nil {
					return
				}
			}
			...
			if err != nil || s.WorldId() != os.WorldId() || s.ChannelId() != os.ChannelId() {
				...
				err = session.Announce(l)(ctx)(wp)(party2.PartyOperationWriter)(writer.PartyErrorBody("UNABLE_TO_FIND_THE_REQUESTED_CHARACTER_IN_THIS_CHANNEL", sp.Name()))(s)
				if err != nil {
				}
				return
			}
