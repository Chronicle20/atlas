...
- Ring pair fields (couple/friendship): the MOVED appearance-update broadcast on equip/unequip re-derives the character's `RingSet` from `ring.Processor.GetRingSet` and announces it to every character already on the map (task-269), so an already-present partner or bystander sees an equipped/unequipped ring pair update within the map without either character changing maps. Residual limitation (PRD FR-12): if a partner already standing on your map buys a new ring while you are both present, `RING_PURCHASED` (`EVENT_TOPIC_CASH_SHOP_STATUS`) only invalidates both halves' cached ring pairs — it does not itself trigger an appearance-update broadcast. Your view of their new ring resolves when the partner equips it (their own subsequent EQUIP command re-derives their `RingSet` from the now-invalidated cache and broadcasts via the MOVED handling above), not from the purchase event alone.
