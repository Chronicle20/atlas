...
func (p *ProcessorImpl) Create(mb *message.Buffer) func(transactionId uuid.UUID, input Model, mapId _map.Id) (Model, error) {
	return func(transactionId uuid.UUID, input Model, mapId _map.Id) (Model, error) {
		...
		var res Model
		txErr := database.ExecuteTransaction(p.db.WithContext(p.ctx), func(tx *gorm.DB) error {
			res, err = create(tx, p.t.Id(), input.accountId, input.worldId, input.name, input.level, input.strength, input.dexterity, input.intelligence, input.luck, input.maxHp, input.maxMp, input.jobId, input.gender, input.hair, input.face, input.skinColor, input.gm, input.meso, input.ap, input.sp)
			if err != nil {
