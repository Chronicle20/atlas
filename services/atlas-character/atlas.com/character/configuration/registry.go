...
// fetcher resolves a tenant's imprint configuration. The default fetcher hits
...
// Registry is a lazy, per-tenant config cache. A fetch miss or error falls
// back to DefaultConfig so the service never hard-fails because a tenant has
// not configured a pending-change expiry — and the fallback (168h) is the
// documented default the configuration resource itself carries. Because the
// outcome is cached, the fallback notice is logged once per tenant.
...
// GetRegistry returns the process-wide imprint-config registry singleton.
...
// defaultFetcher fetches a tenant's configuration from atlas-tenants and
// folds it into the domain Model via Extract.
...
// thing that blocks a pending-change request (FR-2.6).
func (r *Registry) Get(l logrus.FieldLogger, ctx context.Context) Model {
	t, err := tenant.FromContext(ctx)()
	if err != nil {
		l.WithError(err).Warn("No tenant in context when resolving imprint configuration. Using defaults.")
...
// stays readable. Uses a read-locked fast path with a double-checked write
// lock.
func (r *Registry) getForTenant(l logrus.FieldLogger, ctx context.Context, tenantId uuid.UUID) Model {
	...
	if err != nil {
		l.WithError(err).Infof("Tenant [%s] has no imprint-configs resource. Using the shipped 168h default.", tenantId.String())
		cfg = DefaultConfig()
	}
