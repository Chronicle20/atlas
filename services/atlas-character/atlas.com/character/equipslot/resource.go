...
func InitResource(si jsonapi.ServerInformation) func(db *gorm.DB) server.RouteInitializer {
	return func(db *gorm.DB) server.RouteInitializer {
		return func(router *mux.Router, l logrus.FieldLogger) {
			...
			r.HandleFunc("", registerGet("get_equip_slot_extensions", handleGetEquipSlotExtensions(db))).Methods(http.MethodGet)
			r.HandleFunc("", rest.RegisterInputHandler[ExtendInputRestModel](l)(si)("extend_equip_slot", handleExtendEquipSlot(db))).Methods(http.MethodPost)
		}
	}
}

func handleGetEquipSlotExtensions(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				ms, err := NewProcessor(d.Logger(), d.Context(), db).GetActive(characterId)
				if err != nil {
...
func handleExtendEquipSlot(db *gorm.DB) rest.InputHandler[ExtendInputRestModel] {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext, input ExtendInputRestModel) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
