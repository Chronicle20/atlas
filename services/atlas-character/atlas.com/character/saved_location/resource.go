...
func InitResource(si jsonapi.ServerInformation) func(db *gorm.DB) server.RouteInitializer {
	return func(db *gorm.DB) server.RouteInitializer {
		return func(router *mux.Router, l logrus.FieldLogger) {
			...
			r.HandleFunc("/{type}", registerGet("get_saved_location", handleGetSavedLocation(db))).Methods(http.MethodGet)
			r.HandleFunc("/{type}", rest.RegisterInputHandler[RestModel](l)(si)("put_saved_location", handlePutSavedLocation(db))).Methods(http.MethodPut)
			r.HandleFunc("/{type}", registerGet("delete_saved_location", handleDeleteSavedLocation(db))).Methods(http.MethodDelete)
		}
	}
}
...
func handleGetSavedLocation(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return parseLocationType(d.Logger(), func(locationType string) http.HandlerFunc {
				return func(w http.ResponseWriter, r *http.Request) {
					m, err := NewProcessor(d.Logger(), d.Context(), db).Get(characterId, locationType)
					if err != nil {
...
func handlePutSavedLocation(db *gorm.DB) rest.InputHandler[RestModel] {
		...
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return parseLocationType(d.Logger(), func(locationType string) http.HandlerFunc {
				return func(w http.ResponseWriter, r *http.Request) {
					...
					result, err := NewProcessor(d.Logger(), d.Context(), db).Put(m)
					if err != nil {
						server.WriteErrorResponse(d.Logger())(w)(err)
						return
					}
...
func handleDeleteSavedLocation(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return parseLocationType(d.Logger(), func(locationType string) http.HandlerFunc {
				return func(w http.ResponseWriter, r *http.Request) {
