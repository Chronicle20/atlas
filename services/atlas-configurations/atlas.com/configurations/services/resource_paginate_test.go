...
func doGetServiceConfigs(t *testing.T, router *mux.Router, path string) *httptest.ResponseRecorder {
	t.Helper()
...
// TestGetServiceConfigurationsPaginates proves GET /configurations/services
// is now paginated. Entities are seeded directly with fixed, deliberately
// out-of-ascending-order ids ("...300", "...100", "...200") -
// database.PagedQuery's schema-derived primary-key ordering is what makes
// the paged response deterministic.
func TestGetServiceConfigurationsPaginates(t *testing.T) {
	db := setupTestDB(t)
	l := testLogger()

	for _, suffix := range []string{"300", "100", "200"} {
		data, err := json.Marshal(service.GenericRestModel{})
		if err != nil {
	...
	t.Run("FirstPageOfTwoInAscendingIdOrder", func(t *testing.T) {
		rr := doGetServiceConfigs(t, router, "/configurations/services?page[number]=1&page[size]=2")
	...
	t.Run("PageSizeZeroIsBadRequest", func(t *testing.T) {
		rr := doGetServiceConfigs(t, router, "/configurations/services?page[size]=0")
	...
	t.Run("LegacyLimitParamIsBadRequest", func(t *testing.T) {
		rr := doGetServiceConfigs(t, router, "/configurations/services?limit=5")
		if rr.Code != http.StatusBadRequest {
			t.Fatalf("status = %d, want 400", rr.Code)
		}
	})
	...
	t.Run("PastLastPageReturnsEmptyWithPrevAtLast", func(t *testing.T) {
		rr := doGetServiceConfigs(t, router, "/configurations/services?page[number]=99&page[size]=2")
		if rr.Code != http.StatusOK {
			t.Fatalf("status = %d, want 200, body=%s", rr.Code, rr.Body.String())
		}
		var doc struct {
