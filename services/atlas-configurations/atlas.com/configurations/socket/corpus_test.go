...
func TestValidate_AcceptsEverySeedTemplate(t *testing.T) {
	...
	if total != 3160 {
		t.Errorf("corpus size = %d entries, want 3160 (3052 before task-206, plus task-206's 10 CashShopCouponCodeHandle bindings — every template but gms_12 — plus task-207's 7 CashItemGachaponHandle handlers and 6 CashItemGachaponResult writers — plus task-210's 16 template bindings (CharacterUseDeathItemHandle handler and CharacterShowUpgradeTombEffect writer in 8 templates) and 2 v92 writers (CharacterEffect and CharacterEffectForeign) — plus task-212's 15 catch bindings — plus task-211's 30 kite writer bindings (SpawnKite, SpawnKiteError and DestroyKite on every template but gms_12) — plus task-213's 1 gms_92 CharacterSkillPrepareHandle binding, the only template that lacked it — plus task-217's 12 Aran combo bindings (AranComboCounterHandle handler and ShowCombo writer on gms_83/84/87/92/95 and jms_185) — plus task-218's 3 CharacterKeyMapChangeHandle bindings on gms_87/gms_92/jms_185, the three templates that lacked it and where keybinds therefore never saved — plus task-226's 6 skill-macro bindings (CharacterSkillMacroHandle handler on gms_61, gms_87, gms_92, gms_95 and jms_185 — gms_61 included because task-226 corrected SKILL_MACRO x gms_v61 off n-a, having located CMacroSysMan::FlushToSvr at 0x59746c sending opcode 101 — plus the CharacterSkillMacro writer on gms_92))", total)
	}
}
