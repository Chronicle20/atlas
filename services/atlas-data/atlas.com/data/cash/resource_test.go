...
// TestCashResourceIntegration tests the REST API endpoints for cash item functionality
func TestCashResourceIntegration(t *testing.T) {
	...
	t.Run("GetCashItemsEndpoint", func(t *testing.T) {
		testGetCashItemsEndpoint(t, testServer, tenantId)
	})

	t.Run("GetCashItemEndpoint", func(t *testing.T) {
		testGetCashItemEndpoint(t, testServer, tenantId)
	})
...
func setupTestCashData(t *testing.T, db *gorm.DB, tenantId uuid.UUID) {
	cashItems := []RestModel{
		{
			Id:      5000000,
			SlotMax: 1,
	...
	storage := document.NewStorage(l, db, GetModelRegistry(), "CASH")
	for _, c := range cashItems {
...
func testGetCashItemsEndpoint(t *testing.T, testServer *httptest.Server, tenantId uuid.UUID) {
	t.Run("GetAllCashItems", func(t *testing.T) {
		...
		for _, item := range data {
			cashItem := item.(map[string]interface{})
			assert.Contains(t, cashItem, "type")
			assert.Contains(t, cashItem, "id")
			assert.Contains(t, cashItem, "attributes")
		}
	})
}
...
func testGetCashItemEndpoint(t *testing.T, testServer *httptest.Server, tenantId uuid.UUID) {
	t.Run("GetCashItemById", func(t *testing.T) {
		...
		assert.Equal(t, "cash_items", data["type"])
		assert.Equal(t, "5000000", data["id"])

		attributes := data["attributes"].(map[string]interface{})
		assert.Equal(t, float64(1), attributes["slotMax"])
	})
...
func testJSONAPICompliance(t *testing.T, testServer *httptest.Server, tenantId uuid.UUID) {
	...
	t.Run("CollectionResourceStructure", func(t *testing.T) {
		...
		for _, item := range data {
			...
			assert.Equal(t, "cash_items", resource["type"])
		}
	})
}
