...
// setupTestHairData seeds 3 hair styles.
func setupTestHairData(t *testing.T, db *gorm.DB, tenantId uuid.UUID) {
	t.Helper()
	l := logrus.New()
	l.SetLevel(logrus.ErrorLevel)
	...
	storage := document.NewStorage(l, db, GetModelRegistry(), "HAIR")
	rows := []RestModel{
		{Id: 30000, Cash: false},
		{Id: 30010, Cash: false},
		{Id: 37000, Cash: true},
...
func TestHairsBareList_PaginationEnvelope(t *testing.T) {
	db := setupResourceTestDB(t)
	tenantId := uuid.New()
	setupTestHairData(t, db, tenantId)

	router := setupTestRouter(db)
	ts := httptest.NewServer(router)
	defer ts.Close()

	url := fmt.Sprintf("%s/data/cosmetics/hairs?page[number]=1&page[size]=2", ts.URL)
...
func TestHairsBareList_RejectsBadPageSize(t *testing.T) {
	db := setupResourceTestDB(t)
	tenantId := uuid.New()
	setupTestHairData(t, db, tenantId)
	...
	url := fmt.Sprintf("%s/data/cosmetics/hairs?page[size]=abc", ts.URL)
	resp, err := http.DefaultClient.Do(createRequestWithTenant("GET", url, tenantId))
	require.NoError(t, err)
	defer resp.Body.Close()
...
func TestHairById_StillWorks(t *testing.T) {
	db := setupResourceTestDB(t)
	tenantId := uuid.New()
	setupTestHairData(t, db, tenantId)
	...
	url := fmt.Sprintf("%s/data/cosmetics/hairs/30000", ts.URL)
	resp, err := http.DefaultClient.Do(createRequestWithTenant("GET", url, tenantId))
	require.NoError(t, err)
	defer resp.Body.Close()
