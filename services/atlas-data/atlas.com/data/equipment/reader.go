...
func Read(l logrus.FieldLogger) func(np model.Provider[xml.Node]) model.Provider[RestModel] {
	return func(np model.Provider[xml.Node]) model.Provider[RestModel] {
		...
		m := RestModel{
			...
			ReqLevel:       info.GetShort("reqLevel", 0),
			ReqJob:         info.GetShort("reqJob", 0),
			ReqStr:         info.GetShort("reqSTR", 0),
			ReqDex:         info.GetShort("reqDEX", 0),
			ReqInt:         info.GetShort("reqINT", 0),
			ReqLuk:         info.GetShort("reqLUK", 0),
			ReqPop:         info.GetShort("reqPOP", 0),
			ReqFame:        info.GetShort("reqFame", 0),
