...
// seedMobSkills stores 3 mob skills: two share SkillId 200, one has SkillId 300.
func seedMobSkills(t *testing.T, db *gorm.DB, tenantId uuid.UUID) {
	t.Helper()
	l := logrus.New()
	l.SetLevel(logrus.ErrorLevel)
	...
	rows := []RestModel{
		{SkillId: 200, Level: 1, Name: "Weapon Attack Up"},
		{SkillId: 200, Level: 2, Name: "Weapon Attack Up"},
		{SkillId: 300, Level: 1, Name: "Summon"},
	}
	for _, r := range rows {
...
func TestMobSkillsBareList_PaginationEnvelope(t *testing.T) {
	db := setupResourceTestDB(t)
	tenantId := uuid.New()
	seedMobSkills(t, db, tenantId)

	router := setupTestRouter(db)
	ts := httptest.NewServer(router)
	defer ts.Close()

	url := fmt.Sprintf("%s/data/mob-skills?page[number]=1&page[size]=2", ts.URL)
...
func TestMobSkillsBareList_RejectsBadPageSize(t *testing.T) {
	db := setupResourceTestDB(t)
	tenantId := uuid.New()
	seedMobSkills(t, db, tenantId)
	...
	url := fmt.Sprintf("%s/data/mob-skills?page[size]=-1", ts.URL)
	resp, err := http.DefaultClient.Do(createRequestWithTenant("GET", url, tenantId))
	require.NoError(t, err)
	defer resp.Body.Close()
...
// TestMobSkillsByType_PaginationEnvelope covers the by-type derived-filter
// route: skillId 200 has 2 levels in the fixture, so page[size]=1 must
// report total=2 with a next link.
func TestMobSkillsByType_PaginationEnvelope(t *testing.T) {
	db := setupResourceTestDB(t)
	tenantId := uuid.New()
	seedMobSkills(t, db, tenantId)
	...
	url := fmt.Sprintf("%s/data/mob-skills/200?page[number]=1&page[size]=1", ts.URL)
	resp, err := http.DefaultClient.Do(createRequestWithTenant("GET", url, tenantId))
	require.NoError(t, err)
	defer resp.Body.Close()
...
func TestMobSkillsByType_RejectsBadPageNumber(t *testing.T) {
	db := setupResourceTestDB(t)
	tenantId := uuid.New()
	seedMobSkills(t, db, tenantId)
	...
	url := fmt.Sprintf("%s/data/mob-skills/200?page[number]=0", ts.URL)
	resp, err := http.DefaultClient.Do(createRequestWithTenant("GET", url, tenantId))
	require.NoError(t, err)
	defer resp.Body.Close()
