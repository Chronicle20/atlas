...
func NewStorage(l logrus.FieldLogger, db *gorm.DB) *Storage {
	return &Storage{
		l:   l,
		db:  db,
		doc: document.NewStorage[string, RestModel](l, db, GetModelRegistry(), "REACTOR"),
	}
}
...
func (s *Storage) Add(ctx context.Context) func(m RestModel) model.Provider[RestModel] {
	return func(m RestModel) model.Provider[RestModel] {
		...
		txErr := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			scoped := document.NewStorage[string, RestModel](s.l, tx, GetModelRegistry(), "REACTOR")
...
func (s *Storage) Clear(ctx context.Context) error {
	...
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("type = ?", "REACTOR").Delete(&document.Entity{}).Error; err != nil {
			return err
		}
