...
func TestExtractPreservesMemberOrder(t *testing.T) {
	...
	assert.Equal(t, uint32(100), m.LeaderId())

	members := m.Members()
	require.Len(t, members, 2)
	assert.Equal(t, uint32(100), members[0], "member[0] must be the first in join order (leader)")
	assert.Equal(t, uint32(200), members[1], "member[1] must be the second in join order")
}
