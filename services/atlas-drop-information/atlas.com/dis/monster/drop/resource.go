...
func InitResource(si jsonapi.ServerInformation) func(db *gorm.DB) server.RouteInitializer {
	return func(db *gorm.DB) server.RouteInitializer {
		return func(router *mux.Router, l logrus.FieldLogger) {
			...
			r.HandleFunc("", registerGet("get_all_drops", handleGetAllDrops(db))).Methods(http.MethodGet)

			ir := router.PathPrefix("/items/{itemId}/drops").Subrouter()
			ir.HandleFunc("", registerGet("get_item_drops", handleGetItemDrops(db))).Methods(http.MethodGet)
		}
	}
}

func handleGetAllDrops(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseMonsterId(d.Logger(), func(monsterId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				...
				paged, err := NewProcessor(d.Logger(), d.Context(), db).GetForMonster(monsterId, page)()
				if err != nil {
...
func handleGetItemDrops(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseItemId(d.Logger(), func(itemId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				...
				paged, err := NewProcessor(d.Logger(), d.Context(), db).GetForItem(itemId, page)()
				if err != nil {
					if errors.Is(err, ErrNotFound) {
						w.WriteHeader(http.StatusNotFound)
						return
