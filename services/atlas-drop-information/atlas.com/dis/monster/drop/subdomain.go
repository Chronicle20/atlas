...
// JSONModel is the decoded shape of the "attributes" field in a monster-drop catalog file.
...
// Subdomain implements seeder.Subdomain for the monster_drops table.
type Subdomain struct{}

func (Subdomain) Name() string { return "monster-drop" }
func (Subdomain) Path() string { return "drops/monsters" }
func (Subdomain) Type() string { return "monster-drop" }
func (Subdomain) EntityIDPattern() *regexp.Regexp {
	return regexp.MustCompile(`^monster-(\d+)\.json$`)
}
...
func (Subdomain) Build(t tenant.Model, entityID string, attrs JSONModel) ([]Model, error) {
	monsterId64, err := strconv.ParseUint(entityID, 10, 32)
	if err != nil {
		return nil, fmt.Errorf("monster-drop: parse entityID %q: %w", entityID, err)
	}
	monsterId := uint32(monsterId64)
	...
	for _, d := range attrs.Drops {
		m, err := NewMonsterDropBuilder(t.Id(), 0).
			...
			Build()
		if err != nil {
			return nil, fmt.Errorf("monster-drop: build model for monster %d: %w", monsterId, err)
		}
...
func (Subdomain) BulkCreate(db *gorm.DB, models []Model) error {
	return BulkCreateMonsterDrop(db, models)
}
