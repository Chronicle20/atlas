...
func handleGetEffectiveStats(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
	return rest.ParseWorldChannel(d.Logger(), func(worldId world.Id, channelId channel.Id) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				ch := channel.NewModel(worldId, channelId)
				computed, bonuses, err := NewProcessor(d.Logger(), d.Context()).GetEffectiveStats(ch, characterId)
				if err != nil {
