...
//
// Trailing-slash handling is per-client because external/*/requests.go
// concatenates the base URL with the path template directly:
//
//   - external/character/requests.go    : <base>+"characters/%d"          → trailing slash
//   - external/inventory/requests.go    : <base>+"characters/%d/.../..."  → trailing slash
//   - external/buffs/requests.go        : <base>+"characters/%d/buffs"    → trailing slash
//   - external/skills/requests.go       : <base>+"characters/%d/skills"   → trailing slash
//   - external/data/equipment/requests.go: <base>+"/"+"data/equipment/%d" → NO trailing slash
//   - external/data/skill/requests.go   : <base>+"data/skills/%d"         → trailing slash
//
// This service uses both DATA forms inconsistently. Fortunately the data
// stub handler is path-agnostic (lastSegment), and our skills handler
// returns an empty list so external/data/skill is never reached.
