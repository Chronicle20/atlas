...
// setupCharacterTest wires a fresh miniredis-backed character registry for
// the duration of the test. Mirrors the model-package helper of the same
// shape so consumer tests can drive the same Get/Put paths the initializer
...
// matching the model-package helper. Consumer tests need both because
// handleStatChanged threads ctx through the registry and the processor.
