...
func TestGlobalProvider_GetByTier_FiltersByTenant(t *testing.T) {
	...
	rowsA, err := getByTier("common")(db.WithContext(database.TenantContext(tidA)))()
	...
	rowsB, err := getByTier("common")(db.WithContext(database.TenantContext(tidB)))()
	require.NoError(t, err)
...
func TestGlobalAdministrator_DeleteItem_ScopedToTenant(t *testing.T) {
	...
	require.NoError(t, DeleteItem(db.WithContext(database.TenantContext(tidA)), 2))
	...
	require.NoError(t, DeleteItem(db.WithContext(database.TenantContext(tidA)), 1))
	require.NoError(t, db.Unscoped().Order("tenant_id").Find(&rows).Error)
