...
func setupKeyRouter(db *gorm.DB) *mux.Router {
	r := mux.NewRouter()
	l := logrus.New()
...
// TestGetKeyMapPaginates drives GET /characters/{characterId}/keys through
// the real resource router, verifying the JSON:API paginated envelope, 400
// on invalid paging params, and empty-page handling past the last page.
func TestGetKeyMapPaginates(t *testing.T) {
	db := databasetest.NewInMemoryTenantDB(t, key.Migration)
	tenantId := uuid.New()
	characterId := uint32(42)
	otherCharacterId := uint32(99)
	seedKey(t, db, tenantId, characterId, 1)
	seedKey(t, db, tenantId, characterId, 2)
	seedKey(t, db, tenantId, characterId, 3)
	seedKey(t, db, tenantId, otherCharacterId, 1)

	srv := httptest.NewServer(setupKeyRouter(db))
	defer srv.Close()

	t.Run("FirstPageOfTwo", func(t *testing.T) {
		url := fmt.Sprintf("%s/characters/42/keys?page[number]=1&page[size]=2", srv.URL)
		...
		assert.EqualValues(t, 3, doc.Meta["total"], "must exclude the other character's keys")
	...
	t.Run("PageSizeZeroIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/characters/42/keys?page[size]=0", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("LegacyLimitParamIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/characters/42/keys?limit=5", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("PastLastPageReturnsEmptyWithPrevAtLast", func(t *testing.T) {
		url := fmt.Sprintf("%s/characters/42/keys?page[number]=99&page[size]=2", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
