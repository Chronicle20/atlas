...
func TestKeyProvider_ByCharacterId_FiltersByTenant(t *testing.T) {
	...
	rowsA, err := byCharacterIdEntityProvider(1001)(db.WithContext(databasetest.TenantContext(tidA)))()
	...
	rowsB, err := byCharacterIdEntityProvider(1001)(db.WithContext(databasetest.TenantContext(tidB)))()
	require.NoError(t, err)
...
func TestKeyAdministrator_DeleteByCharacter_ScopedToTenant(t *testing.T) {
	...
	require.NoError(t, deleteByCharacter(db.WithContext(databasetest.TenantContext(tidA)), 1001))
