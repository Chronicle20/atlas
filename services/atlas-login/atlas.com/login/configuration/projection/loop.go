...
// AddBody is the per-tenant startup callback main.go provides. It runs
...
type AddBody func(parent context.Context, key listener.Key, cfg ListenerConfig, h *listener.Handle) ([]listener.HandlerHandle, error)

// ServerModelFn builds the listener.ServerModel that the listener tracks.
// Passed in (rather than inlined) so test code can inject a stub without
// touching tenant.Register's process-global side effect.
type ServerModelFn func(key listener.Key, cfg ListenerConfig) listener.ServerModel
