...
func TestComputeOps_AddRemovePortChangeUnchanged(t *testing.T) {
	...
	mk := func(port int) *configuration.RestModel {
		return &configuration.RestModel{
			Tenants: []configuration.TenantRestModel{
				{Id: tid.String(), Port: port},
			},
		}
	}
	key := listener.Key{TenantId: tid}

	// ADD: empty → one tenant
	ops := projection.ComputeOps(nil, nil, mk(8484), tcfg)
	require.Len(t, ops, 1)
	require.Equal(t, projection.OpAdd, ops[0].Kind)
	require.Equal(t, key, ops[0].Key)
	require.Equal(t, 8484, ops[0].Cfg.Port)

	// UNCHANGED: same → no ops
	ops = projection.ComputeOps(mk(8484), tcfg, mk(8484), tcfg)
	require.Empty(t, ops)

	// PORT CHANGE: drain then add
	ops = projection.ComputeOps(mk(8484), tcfg, mk(9090), tcfg)
	require.Len(t, ops, 2)
	...
	ops = projection.ComputeOps(mk(8484), tcfg, nil, nil)
	require.Len(t, ops, 1)
...
// returns early on env.Id != s.ServiceId.String() (subscriber.go:111-113),
...
func TestHasServiceIsTrueAfterTheMatchingServiceIsApplied(t *testing.T) {
	...
	require.NoError(t, s.ApplyService(projection.ServiceEnvelope{
		SchemaVersion: 1,
		Id:            "6439ca9c-d28d-5db9-821b-8dd93d318a25",
		Config:        cfgBts,
	}))
