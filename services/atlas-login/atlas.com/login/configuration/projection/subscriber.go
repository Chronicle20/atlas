...
type Subscriber struct {
	...
	// ServiceId is this atlas-login instance's service id; service
	// envelopes with a different id are skipped.
	ServiceId uuid.UUID
}

// Start snapshots end offsets for both topics, then registers a consumer
// + handler for each on the shared atlas-kafka manager. Returns the first
// error encountered; partial registration is tolerated by the caller (the
// gate will not flip and the apply loop will see no ops).
...
func (s *Subscriber) handleService(l logrus.FieldLogger) handler.Handler {
	return func(_ logrus.FieldLogger, _ context.Context, msg kafka.Message) (bool, error) {
		...
		if IsTombstone(msg.Value) {
			// A service tombstone matching our id means atlas-login
			// should drop the entire config. Non-matching tombstones are
			// ignored.
			key := string(msg.Key)
