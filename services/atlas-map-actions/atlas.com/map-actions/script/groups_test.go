...
func newGroupsTestRouter(t *testing.T, db *gorm.DB) *mux.Router {
	...
	routeInit := InitSeedResource(testSrvInfo{})(db)
	if routeInit == nil {
...
// TestInitSeedResource_SeedRouteAccepted verifies that POST /maps/actions/seed
...
func TestInitSeedResource_SeedRouteAccepted(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/maps/actions/seed", nil)
	...
	if w.Code != http.StatusAccepted {
		t.Errorf("POST /maps/actions/seed: got %d, want %d (body: %s)",
			w.Code, http.StatusAccepted, w.Body.String())
	}
}

// TestInitSeedResource_StatusRouteOK verifies that GET /maps/actions/seed/status
...
func TestInitSeedResource_StatusRouteOK(t *testing.T) {
	...
	if w.Code != http.StatusOK {
		t.Errorf("GET /maps/actions/seed/status: got %d, want %d (body: %s)",
			w.Code, http.StatusOK, w.Body.String())
	}
	body := w.Body.String()
	if body == "" {
		t.Error("GET /maps/actions/seed/status: empty body")
	}
	if !containsStr(body, "catalogRevision") {
		t.Errorf("GET /maps/actions/seed/status: body missing 'catalogRevision': %s", body)
	}
}
