...
func seedStatusReq(tenantId uuid.UUID) *http.Request {
	req := httptest.NewRequest(http.MethodGet, "/maps/actions/seed/status", nil)
...
func insertSeedStatusScript(t *testing.T, db *gorm.DB, tenantId uuid.UUID, scriptName, scriptType string) {
	t.Helper()
...
func TestSeedStatusHandler_Empty(t *testing.T) {
	...
	if env.Data.Type != "mapActionScriptsSeedStatus" {
		t.Errorf("type = %q, want mapActionScriptsSeedStatus", env.Data.Type)
	}
...
func TestSeedStatusHandler_Populated(t *testing.T) {
	...
	tenantId := uuid.New()
	insertSeedStatusScript(t, db, tenantId, "map_a", "onEnter")
	insertSeedStatusScript(t, db, tenantId, "map_b", "onEnter")
	insertSeedStatusScript(t, db, tenantId, "map_c", "onFirstEnter")
...
func TestSeedStatusHandler_TenantIsolation(t *testing.T) {
	...
	insertSeedStatusScript(t, db, tenant1, "t1_map_a", "onEnter")
	insertSeedStatusScript(t, db, tenant2, "t2_map_a", "onEnter")
	insertSeedStatusScript(t, db, tenant2, "t2_map_b", "onEnter")
