...
// - When a monster is spawned, NextSpawnAt is set to time.Now() + MobTime seconds (default 5s if MobTime == 0)
// - During spawn filtering, only spawn points with NextSpawnAt.Before(now) are considered eligible
// - This prevents immediate re-spawning and enforces MobTime-based cooldown periods
// - Boss spawn points (MobTime > 0) get their full MobTime delay; normal monsters get 5 seconds
