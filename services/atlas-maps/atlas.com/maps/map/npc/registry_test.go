...
func TestRegistryAddAndGetAll(t *testing.T) {
	tests := []struct {
		...
		{name: "adds and retrieves all entries for a field key"},
	}
...
func TestRegistryGetAllReturnsCopy(t *testing.T) {
	tests := []struct {
		...
		{name: "GetAll snapshot is unaffected by a later Add"},
	}
...
func TestRegistryReset(t *testing.T) {
	tests := []struct {
		...
		{name: "Reset empties the registry"},
	}
...
func TestRegistryNextIdIsUnique(t *testing.T) {
	tests := []struct {
		...
		{name: "NextId returns distinct values"},
	}
