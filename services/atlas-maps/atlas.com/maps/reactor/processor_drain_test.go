...
// .../instances/{id}/reactors is now paginated (task-117); Spawn's dedupe
// logic (doesNotExist) needs the COMPLETE existing set to avoid
// double-spawning a reactor already present -- the fixture server serves
// 300 reactors across two pages of 250, so only a genuine drain picks up
// reactor id 300, which lives on page 2.
func TestInMapModelProviderDrainsBeyondOnePage(t *testing.T) {
	...
	ms, err := reactor.NewProcessor(l, ctx, nil).InMapModelProvider(uuid.New(), f)()
	if err != nil {
		t.Fatal(err)
	}
	if len(ms) != 300 {
