...
func setupVisitRouter(db *gorm.DB) *mux.Router {
	r := mux.NewRouter()
	l := logrus.New()
	l.SetLevel(logrus.ErrorLevel)
	ri := visit.InitResource(&testServerInformation{})(db)
	ri(r, l)
	return r
}
...
func seedVisit(t *testing.T, db *gorm.DB, tenantId uuid.UUID, characterId uint32, mapId uint32) {
	t.Helper()
	require.NoError(t, db.Create(&visit.Entity{
		ID:             uuid.New(),
		TenantId:       tenantId,
...
// TestGetCharacterVisitsPaginates drives GET /characters/{characterId}/visits
// through the real resource router, verifying the JSON:API paginated
// envelope AND that the default page size is 50 (growing-log default), not
// the 250 game cap used by bounded registry/DB lists.
func TestGetCharacterVisitsPaginates(t *testing.T) {
	db := databasetest.NewInMemoryTenantDB(t, visit.MigrateTable)
	tenantId := uuid.New()
	characterId := uint32(900)

	seedVisit(t, db, tenantId, characterId, 100000000)
	seedVisit(t, db, tenantId, characterId, 100000001)
	seedVisit(t, db, tenantId, characterId, 100000002)

	srv := httptest.NewServer(setupVisitRouter(db))
	defer srv.Close()

	t.Run("FirstPageOfTwo", func(t *testing.T) {
		url := fmt.Sprintf("%s/characters/900/visits?page[number]=1&page[size]=2", srv.URL)
	...
	t.Run("PageSizeZeroIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/characters/900/visits?page[size]=0", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("LegacyLimitParamIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/characters/900/visits?limit=5", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("PastLastPageReturnsEmptyWithPrevAtLast", func(t *testing.T) {
		url := fmt.Sprintf("%s/characters/900/visits?page[number]=99&page[size]=2", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("DefaultPageSizeIs50NotGameCap250", func(t *testing.T) {
		url := fmt.Sprintf("%s/characters/900/visits", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
