...
type MessengerIdHandler func(messengerId uint32) http.HandlerFunc

func ParseMessengerId(l logrus.FieldLogger, next MessengerIdHandler) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		messengerId, err := strconv.Atoi(mux.Vars(r)["messengerId"])
		if err != nil {
			l.WithError(err).Errorf("Unable to properly parse messengerId from path.")
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		next(uint32(messengerId))(w, r)
	}
}
