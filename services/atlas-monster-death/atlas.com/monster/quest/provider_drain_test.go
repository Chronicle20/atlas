...
// so mob-kill quest matching doesn't silently miss quests started while a
// character has more than 250 active quests).
func TestGetStartedQuestIdsDrainsBeyondOnePage(t *testing.T) {
	...
	ids, err := quest.GetStartedQuestIds(l)(ctx)(characterId)
	if err != nil {
		t.Fatal(err)
	}
	if len(ids) != 300 {
