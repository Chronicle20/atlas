...
// TestCreateSpawnIfAbsent covers the SpawnIfAbsent guard on Create (design
// D5/F6, and task A5's F3 field-scoping fix):
//   - "suppresses when present": a second Create for the same template on an
//     occupied field is suppressed — no new monster, no error, a zero Model.
//   - "creates when absent": SpawnIfAbsent does not suppress the spawn when
//     the field has no monster of that template yet.
//   - "matches on template not unique id": the guard compares MonsterId (the
//     template), not UniqueId — a different template spawns even though the
//     field is already occupied.
//   - "stacks without SpawnIfAbsent": existing behavior is unchanged when
//     SpawnIfAbsent is absent (its zero value) — repeated Create calls for
//     the same template stack.
//   - "is field scoped": the same template spawns independently in two
//     different instances of the same map.
func TestCreateSpawnIfAbsent(t *testing.T) {
	tests := []struct {
		name string
		run  func(t *testing.T, p *ProcessorImpl)
	}{
		{
			name: "suppresses when present",
			run: func(t *testing.T, p *ProcessorImpl) {
				f := testField()
		...
		{
			name: "matches on template not unique id",
			run: func(t *testing.T, p *ProcessorImpl) {
				f := testField()
		...
		{
			name: "stacks without SpawnIfAbsent",
			run: func(t *testing.T, p *ProcessorImpl) {
				f := testField()
