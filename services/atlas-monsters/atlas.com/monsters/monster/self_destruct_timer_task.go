...
func NewSelfDestructTimerTask(l logrus.FieldLogger, interval time.Duration) *SelfDestructTimerTask {
	l.Infof("Initializing self-destruct timer task to run every %dms.", interval.Milliseconds())
	return &SelfDestructTimerTask{l: l, interval: interval}
}

func (t *SelfDestructTimerTask) Run(ctx context.Context) {
	now := time.Now()
	for key, entry := range GetSelfDestructTimerRegistry().GetAll(ctx) {
		t.processEntry(ctx, now, key.Tenant, key.MonsterId, entry)
	}
}

func (t *SelfDestructTimerTask) processEntry(ctx context.Context, now time.Time, ten tenant.Model, uniqueId uint32, e SelfDestructTimerEntry) {
	...
	if err != nil || !m.Alive() {
		GetSelfDestructTimerRegistry().Unregister(ctx, ten, uniqueId)
		return
	}
