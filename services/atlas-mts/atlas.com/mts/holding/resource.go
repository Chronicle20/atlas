...
func InitResource(si jsonapi.ServerInformation) func(db *gorm.DB) server.RouteInitializer {
	return func(db *gorm.DB) server.RouteInitializer {
		return func(router *mux.Router, l logrus.FieldLogger) {
			...
			registerInput := rest.RegisterInputHandler[TakeHomeRestModel](l)(si)

			r := router.PathPrefix("/characters/{characterId}/mts/holding").Subrouter()
			r.HandleFunc("", registerGet("get_character_holdings", handleGetCharacterHoldings(db))).Methods(http.MethodGet)
			r.HandleFunc("/{holdingId}/take-home", registerInput("take_home_holding", handleTakeHome(db))).Methods(http.MethodPost)
		}
	}
}
...
func handleTakeHome(db *gorm.DB) rest.InputHandler[TakeHomeRestModel] {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext, rm TakeHomeRestModel) http.HandlerFunc {
...
// (task-117).
func handleGetCharacterHoldings(db *gorm.DB) rest.GetHandler {
