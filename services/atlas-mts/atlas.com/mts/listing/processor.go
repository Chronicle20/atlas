...
type Processor interface {
	...
	Browse(worldId world.Id, state State, category string) ([]Model, error)
...
// Browse returns the listings for a world filtered by state and category. The
// signature mirrors the getBrowse provider exactly.
func (p *ProcessorImpl) Browse(worldId world.Id, state State, category string) ([]Model, error) {
	return model.SliceMap(modelFromEntity)(getBrowse(worldId, state, category)(p.db.WithContext(p.ctx)))()()
}
