...
func InitResource(si jsonapi.ServerInformation) func(db *gorm.DB) server.RouteInitializer {
	return func(db *gorm.DB) server.RouteInitializer {
		return func(router *mux.Router, l logrus.FieldLogger) {
			...
			r.HandleFunc("", registerGet("get_character_wishlist", handleGetCharacterWishlist(db))).Methods(http.MethodGet)
			r.HandleFunc("", registerInput("create_wish", handleCreateWish(db))).Methods(http.MethodPost)
			r.HandleFunc("/{wishId}", registerGet("delete_wish", handleDeleteWish(db))).Methods(http.MethodDelete)

			wr := router.PathPrefix("/worlds/{worldId}/mts/wishlist").Subrouter()
			wr.HandleFunc("", registerGet("get_world_wishlist", handleGetWorldWishlist(db))).Methods(http.MethodGet)
		}
	}
}
...
func handleGetCharacterWishlist(db *gorm.DB) rest.GetHandler {
...
func handleGetWorldWishlist(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseWorldId(d.Logger(), func(worldId world.Id) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				...
				paged, err := NewProcessor(d.Logger(), d.Context(), db).WantedByWorldPagedProvider(worldId, page)()
				if err != nil {
...
func handleCreateWish(db *gorm.DB) rest.InputHandler[RestModel] {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext, rm RestModel) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				...
				created, err := NewProcessor(d.Logger(), d.Context(), db).Create(m)
				if err != nil {
...
func handleDeleteWish(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			ok, err := NewProcessor(d.Logger(), d.Context(), db).Delete(wishId)
			if err != nil {
