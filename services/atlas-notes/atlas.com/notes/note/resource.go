...
func InitializeRoutes(si jsonapi.ServerInformation) func(db *gorm.DB) server.RouteInitializer {
	return func(db *gorm.DB) server.RouteInitializer {
		return func(router *mux.Router, l logrus.FieldLogger) {
			...
			router.HandleFunc("/notes", registerHandler("get_all_notes", GetAllNotesHandler(db))).Methods(http.MethodGet)

			// ByIdProvider all notes for a character
			router.HandleFunc(
				"/characters/{"+characterIdPattern+"}/notes",
				registerHandler("get_character_notes", GetCharacterNotesHandler(db)),
			).Methods(http.MethodGet)

			// ByIdProvider a specific note
			router.HandleFunc(
				"/notes/{"+noteIdPattern+"}",
				registerHandler("get_note", GetNoteHandler(db)),
			).Methods(http.MethodGet)

			// Create a note
			router.HandleFunc("/notes", registerInputHandler("create_note", CreateNoteHandler(db))).Methods(http.MethodPost)

			// Update a note
			router.HandleFunc(
				"/notes/{"+noteIdPattern+"}",
				registerInputHandler("update_note", UpdateNoteHandler(db)),
			).Methods(http.MethodPatch)

			// Delete a note
			router.HandleFunc(
				"/notes/{"+noteIdPattern+"}",
				registerHandler("delete_note", DeleteNoteHandler(db)),
			).Methods(http.MethodDelete)

			// Delete all notes for a character
			router.HandleFunc(
				"/characters/{"+characterIdPattern+"}/notes",
				registerHandler("delete_character_notes", DeleteCharacterNotesHandler(db)),
			).Methods(http.MethodDelete)
		}
	}
}
...
func GetAllNotesHandler(db *gorm.DB) rest.GetHandler {
...
func GetCharacterNotesHandler(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				...
				paged, err := NewProcessor(d.Logger(), d.Context(), db).ByCharacterProvider(characterId, page)()
				if err != nil {
...
func GetNoteHandler(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseNoteId(d.Logger(), func(noteId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				mp := NewProcessor(d.Logger(), d.Context(), db).ByIdProvider(noteId)
...
func CreateNoteHandler(db *gorm.DB) rest.InputHandler[RestModel] {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext, i RestModel) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			m, err := NewProcessor(d.Logger(), d.Context(), db).CreateAndEmit(uuid.Nil, im.CharacterId(), im.SenderId(), im.Message(), im.Flag(), false)
			if err != nil {
...
func UpdateNoteHandler(db *gorm.DB) rest.InputHandler[RestModel] {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext, i RestModel) http.HandlerFunc {
		return rest.ParseNoteId(d.Logger(), func(noteId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				im, err := Extract(i)
				...
				m, err := NewProcessor(d.Logger(), d.Context(), db).UpdateAndEmit(im.Id(), im.CharacterId(), im.SenderId(), im.Message(), im.Flag())
				if err != nil {
...
func DeleteNoteHandler(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, _ *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseNoteId(d.Logger(), func(noteId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				err := NewProcessor(d.Logger(), d.Context(), db).DeleteAndEmit(noteId)
				if err != nil {
...
func DeleteCharacterNotesHandler(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, _ *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
