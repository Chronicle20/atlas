...
func newGroupsTestDB(t *testing.T) *gorm.DB {
	...
	if err := recipe.MigrateTable(db); err != nil {
		t.Fatalf("recipe.MigrateTable: %v", err)
	}
...
// TestInitSeedResource_SeedRouteAccepted verifies that POST /npcs/conversations/seed
...
func TestInitSeedResource_SeedRouteAccepted(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/npcs/conversations/seed", nil)
	req = req.WithContext(tenant.WithContext(req.Context(), te))
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)

	if w.Code != http.StatusAccepted {
		t.Errorf("POST /npcs/conversations/seed: got %d, want %d (body: %s)",
			w.Code, http.StatusAccepted, w.Body.String())
	}
}

// TestInitSeedResource_StatusRouteOK verifies that GET /npcs/conversations/seed/status
...
func TestInitSeedResource_StatusRouteOK(t *testing.T) {
	...
	if w.Code != http.StatusOK {
		t.Errorf("GET /npcs/conversations/seed/status: got %d, want %d (body: %s)",
			w.Code, http.StatusOK, w.Body.String())
	}
	body := w.Body.String()
	if body == "" {
		t.Error("GET /npcs/conversations/seed/status: empty body")
	}
	if !containsNpcStr(body, "catalogRevision") {
		t.Errorf("GET /npcs/conversations/seed/status: body missing 'catalogRevision': %s", body)
	}
}

func containsNpcStr(s, sub string) bool {
