...
func setupConversationRouter(db *gorm.DB) *mux.Router {
	r := mux.NewRouter()
	l := logrus.New()
...
func seedConversation(t *testing.T, db *gorm.DB, tenantId uuid.UUID, npcId uint32) uuid.UUID {
	t.Helper()
...
// TestGetAllConversationsPaginates drives GET /npcs/conversations through the
...
func TestGetAllConversationsPaginates(t *testing.T) {
	db := databasetest.NewInMemoryTenantDB(t, MigrateTable)
	tenantId := uuid.New()
	seedConversation(t, db, tenantId, 9001001)
	seedConversation(t, db, tenantId, 9001002)
	seedConversation(t, db, tenantId, 9001003)

	srv := httptest.NewServer(setupConversationRouter(db))
	defer srv.Close()

	t.Run("FirstPageOfTwo", func(t *testing.T) {
		url := fmt.Sprintf("%s/npcs/conversations?page[number]=1&page[size]=2", srv.URL)
	...
	t.Run("PageSizeZeroIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/npcs/conversations?page[size]=0", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("LegacyLimitParamIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/npcs/conversations?limit=5", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("PastLastPageReturnsEmptyWithPrevAtLast", func(t *testing.T) {
		url := fmt.Sprintf("%s/npcs/conversations?page[number]=99&page[size]=2", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
...
// TestGetConversationsByNpcPaginates drives GET /npcs/{npcId}/conversations,
// verifying the filtered-by-npcId arm also carries the pagination envelope
// and excludes other NPCs from the total.
func TestGetConversationsByNpcPaginates(t *testing.T) {
	db := databasetest.NewInMemoryTenantDB(t, MigrateTable)
	tenantId := uuid.New()
	seedConversation(t, db, tenantId, 9002001)
	seedConversation(t, db, tenantId, 9002001)
	seedConversation(t, db, tenantId, 9002002)

	srv := httptest.NewServer(setupConversationRouter(db))
	defer srv.Close()

	t.Run("FirstPageOfTwo", func(t *testing.T) {
		url := fmt.Sprintf("%s/npcs/9002001/conversations?page[number]=1&page[size]=2", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
		...
		require.NotNil(t, doc.Meta)
		assert.EqualValues(t, 2, doc.Meta["total"], "must exclude the other NPC")
	})

	t.Run("PageSizeZeroIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/npcs/9002001/conversations?page[size]=0", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
