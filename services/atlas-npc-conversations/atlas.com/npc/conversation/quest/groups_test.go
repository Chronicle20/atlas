...
// TestInitSeedResource_SeedRouteAccepted verifies that POST /quests/conversations/seed
...
func TestInitSeedResource_SeedRouteAccepted(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/quests/conversations/seed", nil)
	req = req.WithContext(tenant.WithContext(req.Context(), te))
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)

	if w.Code != http.StatusAccepted {
		t.Errorf("POST /quests/conversations/seed: got %d, want %d (body: %s)",
			w.Code, http.StatusAccepted, w.Body.String())
	}
}

// TestInitSeedResource_StatusRouteOK verifies that GET /quests/conversations/seed/status
...
func TestInitSeedResource_StatusRouteOK(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/quests/conversations/seed/status", nil)
	req = req.WithContext(tenant.WithContext(req.Context(), te))
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)

	if w.Code != http.StatusOK {
		t.Errorf("GET /quests/conversations/seed/status: got %d, want %d (body: %s)",
			w.Code, http.StatusOK, w.Body.String())
	}
	body := w.Body.String()
	if body == "" {
		t.Error("GET /quests/conversations/seed/status: empty body")
	}
	if !containsQuestStr(body, "catalogRevision") {
		t.Errorf("GET /quests/conversations/seed/status: body missing 'catalogRevision': %s", body)
	}
}

func containsQuestStr(s, sub string) bool {
