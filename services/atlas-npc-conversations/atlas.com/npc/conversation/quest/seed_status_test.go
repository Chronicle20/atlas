...
func newSeedStatusTestDB(t *testing.T) *gorm.DB {
	...
	if err := MigrateTable(db); err != nil {
		t.Fatalf("Failed to migrate quest conversations: %v", err)
	}
...
func insertSeedStatusRow(t *testing.T, db *gorm.DB, tenantId uuid.UUID, questId uint32) {
	t.Helper()
	...
	m := createTestModel(t, questId)
	if _, err := p.Create(m); err != nil {
		t.Fatalf("Create quest conversation %d: %v", questId, err)
	}
}
...
func TestSeedStatusHandler_Empty(t *testing.T) {
	...
	if env.Data.Type != "questConversationsSeedStatus" {
		t.Errorf("type = %q, want questConversationsSeedStatus", env.Data.Type)
	}
