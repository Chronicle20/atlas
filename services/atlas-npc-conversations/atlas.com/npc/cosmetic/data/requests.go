...
func RequestFaceById(faceId uint32) requests.Request[FaceRestModel] {
	return requests.GetRequest[FaceRestModel](fmt.Sprintf(getBaseRequest()+"/data/cosmetics/faces/%d", faceId))
}
...
func RequestHairById(hairId uint32) requests.Request[HairRestModel] {
	return requests.GetRequest[HairRestModel](fmt.Sprintf(getBaseRequest()+"/data/cosmetics/hairs/%d", hairId))
}
