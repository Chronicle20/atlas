...
func newGroupsTestDB(t *testing.T) *gorm.DB {
	...
	if err := shops.Migration(db); err != nil {
		t.Fatalf("shops.Migration: %v", err)
	}
	if err := commodities.Migration(db); err != nil {
		t.Fatalf("commodities.Migration: %v", err)
	}
...
func newGroupsTestRouter(t *testing.T, db *gorm.DB) *mux.Router {
	...
	routeInit := InitResource(testGroupsSrvInfo{})(db)
	if routeInit == nil {
...
// TestInitResource_SeedRouteAccepted verifies that POST /shops/seed
...
func TestInitResource_SeedRouteAccepted(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/shops/seed", nil)
	...
	if w.Code != http.StatusAccepted {
		t.Errorf("POST /shops/seed: got %d, want %d (body: %s)",
			w.Code, http.StatusAccepted, w.Body.String())
	}
}

// TestInitResource_StatusRouteOK verifies that GET /shops/seed/status
...
func TestInitResource_StatusRouteOK(t *testing.T) {
	...
	if w.Code != http.StatusOK {
		t.Errorf("GET /shops/seed/status: got %d, want %d (body: %s)",
			w.Code, http.StatusOK, w.Body.String())
	}
	body := w.Body.String()
	if body == "" {
		t.Error("GET /shops/seed/status: empty body")
	}
	if !containsStr(body, "catalogRevision") {
		t.Errorf("GET /shops/seed/status: body missing 'catalogRevision': %s", body)
	}
}
