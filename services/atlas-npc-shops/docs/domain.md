...
- Buy operations require the character to be in a shop, the commodity to exist, sufficient currency (mesos for meso-priced commodities, the configured token item for token-priced commodities), and a free inventory slot
...
- Processes buy operations: validates shop membership, commodity existence, and inventory capacity; emits currency-change and create-asset commands. `Buy()` tries three ordered branches per commodity: rechargeable items purchase a full stack (slotMax quantity) priced by the commodity's mesoPrice if set, otherwise by unitPrice × slotMax, charged in mesos; non-rechargeable commodities with mesoPrice > 0 are charged mesoPrice × quantity in mesos; every other commodity is token-priced, charging tokenPrice × quantity units of the commodity's tokenTemplateId (resolved from configuration, never hardcoded, so any token vendor works without code changes) — the client-supplied discount price does not affect this cost. Token consumption can span multiple inventory stacks: draws are taken in ascending slot order, one DESTROY command per drawn slot, because the compartment command contract is slot-based; a free-slot probe on the destination compartment precedes any consumption, so tokens are never spent on an item that cannot be received. Refusals emit a status event and consume nothing: NOT_ENOUGH_MONEY/NEED_MORE_ITEMS when the character holds insufficient mesos/tokens, INVENTORY_FULL when the destination has no free slot, and GENERIC_ERROR when the commodity is misconfigured (no price, a token price with no token item, an invalid item id) or the cost overflows.
