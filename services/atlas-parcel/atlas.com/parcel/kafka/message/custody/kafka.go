...
// orchestrator cannot import this module, so its
// kafka/message/parcel/custody/kafka.go mirrors this file byte-for-byte
// (identical JSON tags + Type discriminator strings) — every field here must
// stay in lockstep, tag-for-tag, with that file. Mirrors the MTS custody twin
...
const (
	...
	// Idempotent: 0 rows affected is success. Fire-and-forget from the
	// orchestrator's compensator — no consumer awaits a success ack, so the
	// handler emits ERROR on failure only.
