...
// TestExpirySweep is table-driven over run: each row exercises a distinct
// sweep scenario (seed shape, batch size, or a direct ClaimExpired call)
// with a distinct assertion set, so the scenario itself is carried as a
// closure rather than shared data fields.
func TestExpirySweep(t *testing.T) {
	tests := []struct {
		name string
		run  func(t *testing.T)
	}{
		{name: "expires and returns", run: func(t *testing.T) {
			db := newSweepTestDB(t)
			tid := uuid.New()
		...
		{name: "return leg expires into nothing", run: func(t *testing.T) {
			db := newSweepTestDB(t)
			tid := uuid.New()
		...
		{name: "not yet expired", run: func(t *testing.T) {
			db := newSweepTestDB(t)
			tid := uuid.New()
			id := seedSweepParcel(t, db, tid, func(b *Builder) {
		...
		{name: "already resolved", run: func(t *testing.T) {
			db := newSweepTestDB(t)
			tid := uuid.New()
			id := seedSweepParcel(t, db, tid, func(b *Builder) {
		...
		{name: "meso-only return", run: func(t *testing.T) {
			db := newSweepTestDB(t)
			tid := uuid.New()
		...
		{name: "batch bound", run: func(t *testing.T) {
			db := newSweepTestDB(t)
			tid := uuid.New()
			for i := 0; i < 5; i++ {
