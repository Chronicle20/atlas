...
// /tenants is now paginated (task-117); main.go's startup PQ-timer ticker
// and graceful-shutdown sweep are genuine semantic-all consumers that must
// see every tenant. The fixture server serves 260 tenants across two pages
// of 250 - only a genuine drain picks up the tenant on page 2.
