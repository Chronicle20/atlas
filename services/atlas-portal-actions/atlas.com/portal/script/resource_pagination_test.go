...
func setupPortalScriptRouter(db *gorm.DB) *mux.Router {
	r := mux.NewRouter()
	l := logrus.New()
...
func seedPortalScript(t *testing.T, db *gorm.DB, tenantId uuid.UUID, portalId string) {
	t.Helper()
...
// TestGetAllScriptsPaginates drives GET /portals/scripts through the real
...
func TestGetAllScriptsPaginates(t *testing.T) {
	...
	tenantId := uuid.New()
	seedPortalScript(t, db, tenantId, "portal1")
	seedPortalScript(t, db, tenantId, "portal2")
	seedPortalScript(t, db, tenantId, "portal3")

	srv := httptest.NewServer(setupPortalScriptRouter(db))
	defer srv.Close()

	t.Run("FirstPageOfTwo", func(t *testing.T) {
		url := fmt.Sprintf("%s/portals/scripts?page[number]=1&page[size]=2", srv.URL)
	...
	t.Run("PageSizeZeroIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/portals/scripts?page[size]=0", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("LegacyLimitParamIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/portals/scripts?limit=5", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("PastLastPageReturnsEmptyWithPrevAtLast", func(t *testing.T) {
		url := fmt.Sprintf("%s/portals/scripts?page[number]=99&page[size]=2", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
