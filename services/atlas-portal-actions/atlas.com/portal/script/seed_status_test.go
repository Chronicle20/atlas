...
func newSeedStatusTestDB(t *testing.T) *gorm.DB {
	...
	if err := MigrateTable(db); err != nil {
		t.Fatalf("Failed to migrate portal_scripts: %v", err)
	}
...
func insertSeedStatusScript(t *testing.T, db *gorm.DB, tenantId uuid.UUID, portalId string) {
	t.Helper()
	...
	if _, err := p.Create(m); err != nil {
		t.Fatalf("Create portal script %s: %v", portalId, err)
	}
}
...
func TestSeedStatusHandler_Empty(t *testing.T) {
	...
	if env.Data.Type != "portalScriptsSeedStatus" {
		t.Errorf("type = %q, want portalScriptsSeedStatus", env.Data.Type)
	}
...
func TestSeedStatusHandler_Populated(t *testing.T) {
	...
	tenantId := uuid.New()
	insertSeedStatusScript(t, db, tenantId, "portal_a")
	insertSeedStatusScript(t, db, tenantId, "portal_b")
	insertSeedStatusScript(t, db, tenantId, "portal_c")
...
func TestSeedStatusHandler_TenantIsolation(t *testing.T) {
	...
	insertSeedStatusScript(t, db, tenant1, "t1_portal_a")
	insertSeedStatusScript(t, db, tenant2, "t2_portal_a")
	insertSeedStatusScript(t, db, tenant2, "t2_portal_b")
