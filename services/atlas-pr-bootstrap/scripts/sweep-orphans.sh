...
#   sweep-orphans.sh --sweep-tenant TENANT_UUID [--apply]
#
...
# sweep_tenant <tenant-uuid> — deletes every row for <tenant-uuid> across
# the shared databases sparse-mode teardown cannot DROP wholesale (design
# §7.5, D1: sparse mode's databases are main's — do_drop_dbs skips
# entirely there). Driven by tenant-tables.txt, a checked-in "<database>
# <table>" list GENERATED by tools/gen-tenant-tables.sh from the FR-8.1
# query-scope audit filtered to Plane == Data — one mechanism, not 85
# per-service cleanup scripts. Control-plane rows are NOT in this list;
# cleanup.sh's do_drop_control_plane already owns those.
#
