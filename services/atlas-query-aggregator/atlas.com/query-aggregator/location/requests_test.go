...
func TestGetField_HappyPath(t *testing.T) {
	...
	if uint8(f.WorldId()) != 2 {
		t.Fatalf("worldId = %d, want 2", f.WorldId())
	}
	if uint8(f.ChannelId()) != 3 {
		t.Fatalf("channelId = %d, want 3", f.ChannelId())
	}
	if uint32(f.MapId()) != 100000000 {
		t.Fatalf("mapId = %d, want 100000000", f.MapId())
	}
	if f.Instance() != instanceUUID {
		t.Fatalf("instance = %v, want %v", f.Instance(), instanceUUID)
	}
}
...
func TestGetField_InfrastructureError(t *testing.T) {
	...
	if errors.Is(err, ErrNotFound) {
		t.Fatalf("expected non-ErrNotFound, got ErrNotFound")
	}
}
