...
func QuestCompletedEventProvider(transactionId uuid.UUID, characterId uint32, worldId world.Id, questId uint32, completedAt time.Time, items []quest2.ItemReward) model.Provider[[]kafka.Message] {
	key := producer.CreateKey(int(characterId))
...
func QuestForfeitedEventProvider(transactionId uuid.UUID, characterId uint32, worldId world.Id, questId uint32) model.Provider[[]kafka.Message] {
	key := producer.CreateKey(int(characterId))
...
func QuestProgressUpdatedEventProvider(transactionId uuid.UUID, characterId uint32, worldId world.Id, questId uint32, infoNumber uint32, progress string) model.Provider[[]kafka.Message] {
	key := producer.CreateKey(int(characterId))
...
func ErrorStatusEventProvider(characterId uint32, worldId world.Id, questId uint32, errorType string) model.Provider[[]kafka.Message] {
	key := producer.CreateKey(int(characterId))
...
func EmitQuestCompleted(l logrus.FieldLogger, ctx context.Context, transactionId uuid.UUID, characterId uint32, worldId world.Id, questId uint32, completedAt time.Time, items []quest2.ItemReward) error {
	return emitEvent(l, ctx, QuestCompletedEventProvider(transactionId, characterId, worldId, questId, completedAt, items))
}

// EmitQuestForfeited emits a quest forfeited event
func EmitQuestForfeited(l logrus.FieldLogger, ctx context.Context, transactionId uuid.UUID, characterId uint32, worldId world.Id, questId uint32) error {
...
func EmitProgressUpdated(l logrus.FieldLogger, ctx context.Context, transactionId uuid.UUID, characterId uint32, worldId world.Id, questId uint32, infoNumber uint32, progress string) error {
