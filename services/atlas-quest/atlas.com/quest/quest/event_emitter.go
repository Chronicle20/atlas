...
type EventEmitter interface {
	...
	EmitQuestCompleted(transactionId uuid.UUID, characterId uint32, worldId byte, questId uint32, completedAt time.Time, items []questmessage.ItemReward) error
...
func (e *KafkaEventEmitter) EmitQuestCompleted(transactionId uuid.UUID, characterId uint32, worldId byte, questId uint32, completedAt time.Time, items []questmessage.ItemReward) error {
