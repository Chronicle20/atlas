...
func InitResource(si jsonapi.ServerInformation) func(db *gorm.DB) server.RouteInitializer {
	return func(db *gorm.DB) server.RouteInitializer {
		return func(router *mux.Router, l logrus.FieldLogger) {
			...
			r.HandleFunc("/{questId}/start", rest.RegisterInputHandler[StartQuestInputRestModel](l)(si)(StartQuest, handleStartQuest(db))).Methods(http.MethodPost)
			r.HandleFunc("/{questId}/complete", rest.RegisterInputHandler[CompleteQuestInputRestModel](l)(si)(CompleteQuest, handleCompleteQuest(db))).Methods(http.MethodPost)
			...
			r.HandleFunc("/{questId}/progress", rest.RegisterInputHandler[progress.RestModel](l)(si)(UpdateQuestProgress, handleUpdateQuestProgress(db))).Methods(http.MethodPatch)
		}
	}
}
...
func handleStartQuest(db *gorm.DB) rest.InputHandler[StartQuestInputRestModel] {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext, i StartQuestInputRestModel) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return rest.ParseQuestId(d.Logger(), func(questId uint32) http.HandlerFunc {
				return func(w http.ResponseWriter, r *http.Request) {
					f := field.NewBuilder(i.WorldId, i.ChannelId, i.MapId).Build()
					// REST endpoints use uuid.Nil since they're not saga-initiated
					q, failedConditions, err := NewProcessor(d.Logger(), d.Context(), db).Start(uuid.Nil, characterId, questId, f, i.SkipValidation, nil)
...
func handleCompleteQuest(db *gorm.DB) rest.InputHandler[CompleteQuestInputRestModel] {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext, i CompleteQuestInputRestModel) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return rest.ParseQuestId(d.Logger(), func(questId uint32) http.HandlerFunc {
				return func(w http.ResponseWriter, r *http.Request) {
					f := field.NewBuilder(i.WorldId, i.ChannelId, i.MapId).Build()
					// REST endpoints use uuid.Nil since they're not saga-initiated
					nextQuestId, err := NewProcessor(d.Logger(), d.Context(), db).Complete(uuid.Nil, characterId, questId, f, i.SkipValidation, nil)
					if errors.Is(err, gorm.ErrRecordNotFound) {
						w.WriteHeader(http.StatusNotFound)
						return
					}
...
func handleUpdateQuestProgress(db *gorm.DB) rest.InputHandler[progress.RestModel] {
	return func(d *rest.HandlerDependency, _ *rest.HandlerContext, i progress.RestModel) http.HandlerFunc {
		return rest.ParseCharacterId(d.Logger(), func(characterId uint32) http.HandlerFunc {
			return rest.ParseQuestId(d.Logger(), func(questId uint32) http.HandlerFunc {
				return func(w http.ResponseWriter, r *http.Request) {
					...
					// REST endpoints use uuid.Nil since they're not saga-initiated
					err := NewProcessor(d.Logger(), d.Context(), db).SetProgress(uuid.Nil, characterId, questId, infoNumber, i.Progress)
					if errors.Is(err, gorm.ErrRecordNotFound) {
						w.WriteHeader(http.StatusNotFound)
						return
					}
