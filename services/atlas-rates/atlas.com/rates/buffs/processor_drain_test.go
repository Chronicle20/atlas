...
// TestGetActiveBuffsDrainsBeyondOnePage proves GetActiveBuffs (via
// requests.DrainProvider) fetches every page of a character's buffs rather
// than stopping after the first response. atlas-buffs' GET
...
// picks up buff sourceId 300, which lives on page 2. initializeActiveBuffs
// (the sole caller) needs every buff to compute rate-affecting stat changes.
func TestGetActiveBuffsDrainsBeyondOnePage(t *testing.T) {
