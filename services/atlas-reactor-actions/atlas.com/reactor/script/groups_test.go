...
// TestInitSeedResource_SeedRouteAccepted verifies that POST /reactors/actions/seed
...
func TestInitSeedResource_SeedRouteAccepted(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/reactors/actions/seed", nil)
	req = req.WithContext(tenant.WithContext(req.Context(), te))
	w := httptest.NewRecorder()
	router.ServeHTTP(w, req)

	if w.Code != http.StatusAccepted {
		t.Errorf("POST /reactors/actions/seed: got %d, want %d (body: %s)",
			w.Code, http.StatusAccepted, w.Body.String())
	}
}

// TestInitSeedResource_StatusRouteOK verifies that GET /reactors/actions/seed/status
...
func TestInitSeedResource_StatusRouteOK(t *testing.T) {
	...
	if w.Code != http.StatusOK {
		t.Errorf("GET /reactors/actions/seed/status: got %d, want %d (body: %s)",
			w.Code, http.StatusOK, w.Body.String())
	}
	body := w.Body.String()
	if body == "" {
		t.Error("GET /reactors/actions/seed/status: empty body")
	}
	if !containsReactorStr(body, "catalogRevision") {
		t.Errorf("GET /reactors/actions/seed/status: body missing 'catalogRevision': %s", body)
	}
}

func containsReactorStr(s, sub string) bool {
