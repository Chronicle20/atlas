...
type paginationTestServerInformation struct{}

func (t *paginationTestServerInformation) GetBaseURL() string { return "http://localhost:8080" }
func (t *paginationTestServerInformation) GetPrefix() string  { return "/api/" }

var _ jsonapi.ServerInformation = &paginationTestServerInformation{}

func setupReactorScriptRouter(db *gorm.DB) *mux.Router {
	r := mux.NewRouter()
	l := logrus.New()
	l.SetLevel(logrus.ErrorLevel)
	ri := InitResource(&paginationTestServerInformation{})(db)
	ri(r, l)
	return r
}
...
func seedReactorScript(t *testing.T, db *gorm.DB, tenantId uuid.UUID, reactorId string) {
	t.Helper()
...
// TestGetAllScriptsPaginates drives GET /reactors/actions through the real
...
func TestGetAllScriptsPaginates(t *testing.T) {
	...
	tenantId := uuid.New()
	seedReactorScript(t, db, tenantId, "reactor1")
	seedReactorScript(t, db, tenantId, "reactor2")
	seedReactorScript(t, db, tenantId, "reactor3")

	srv := httptest.NewServer(setupReactorScriptRouter(db))
	defer srv.Close()

	t.Run("FirstPageOfTwo", func(t *testing.T) {
		url := fmt.Sprintf("%s/reactors/actions?page[number]=1&page[size]=2", srv.URL)
	...
	t.Run("PageSizeZeroIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/reactors/actions?page[size]=0", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("LegacyLimitParamIsBadRequest", func(t *testing.T) {
		url := fmt.Sprintf("%s/reactors/actions?limit=5", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
	...
	t.Run("PastLastPageReturnsEmptyWithPrevAtLast", func(t *testing.T) {
		url := fmt.Sprintf("%s/reactors/actions?page[number]=99&page[size]=2", srv.URL)
		req := requestWithTenant(http.MethodGet, url, tenantId)
