...
const (
	Resource = "reactor-scripts"
)

// RestModel represents the REST model for reactor scripts
...
func Transform(m ReactorScript) (RestModel, error) {
	restHitRules := make([]RestRuleModel, 0, len(m.HitRules()))
	for _, rule := range m.HitRules() {
		restRule := transformRule(rule)
		restHitRules = append(restHitRules, restRule)
	}

	restActRules := make([]RestRuleModel, 0, len(m.ActRules()))
...
func Extract(r RestModel) (ReactorScript, error) {
	if r.ReactorId == "" {
		return ReactorScript{}, fmt.Errorf("reactorId is required")
	}

	builder := NewReactorScriptBuilder().
		SetReactorId(r.ReactorId).
		SetDescription(r.Description)

	for _, restRule := range r.HitRules {
	...
	for _, restRule := range r.ActRules {
		rule, err := extractRule(restRule)
		if err != nil {
			return ReactorScript{}, err
		}
