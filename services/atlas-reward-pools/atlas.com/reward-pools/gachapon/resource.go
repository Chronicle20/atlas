...
func InitResource(si jsonapi.ServerInformation) func(db *gorm.DB) server.RouteInitializer {
	return func(db *gorm.DB) server.RouteInitializer {
		return func(router *mux.Router, l logrus.FieldLogger) {
			...
			r.HandleFunc("", registerGet("get_all_gachapons", handleGetAllGachapons(db))).Methods(http.MethodGet)
			r.HandleFunc("", registerInput("create_gachapon", handleCreateGachapon(db))).Methods(http.MethodPost)
			r.HandleFunc("/{gachaponId}", registerGet("get_gachapon", handleGetGachapon(db))).Methods(http.MethodGet)
			r.HandleFunc("/{gachaponId}", registerInput("update_gachapon", handleUpdateGachapon(db))).Methods(http.MethodPatch)
			r.HandleFunc("/{gachaponId}", registerGet("delete_gachapon", handleDeleteGachapon(db))).Methods(http.MethodDelete)
		}
	}
}

func handleGetAllGachapons(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			paged, err := NewProcessor(d.Logger(), d.Context(), db).GetAll(page)()
			if err != nil {
...
func handleGetGachapon(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseGachaponId(d.Logger(), func(gachaponId string) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				m, err := NewProcessor(d.Logger(), d.Context(), db).GetById(gachaponId)
				if err != nil {
...
func handleCreateGachapon(db *gorm.DB) rest.InputHandler[RestModel] {
...
func handleUpdateGachapon(db *gorm.DB) rest.InputHandler[RestModel] {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext, rm RestModel) http.HandlerFunc {
		return rest.ParseGachaponId(d.Logger(), func(gachaponId string) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				err := NewProcessor(d.Logger(), d.Context(), db).Update(gachaponId, rm.Name, rm.NpcIds, rm.CommonWeight, rm.UncommonWeight, rm.RareWeight)
				if err != nil {
					if errors.Is(err, gorm.ErrRecordNotFound) {
						w.WriteHeader(http.StatusNotFound)
						return
...
func handleDeleteGachapon(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return rest.ParseGachaponId(d.Logger(), func(gachaponId string) http.HandlerFunc {
			return func(w http.ResponseWriter, r *http.Request) {
				err := NewProcessor(d.Logger(), d.Context(), db).Delete(gachaponId)
				if err != nil {
