...
func InitResource(si jsonapi.ServerInformation) func(db *gorm.DB) server.RouteInitializer {
	return func(db *gorm.DB) server.RouteInitializer {
		return func(router *mux.Router, l logrus.FieldLogger) {
			...
			r.HandleFunc("", registerGet("get_all_global_items", handleGetAllGlobalItems(db))).Methods(http.MethodGet)
			r.HandleFunc("", registerInput("create_global_item", handleCreateGlobalItem(db))).Methods(http.MethodPost)
			r.HandleFunc("/{itemId}", registerInput("update_global_item", handleUpdateGlobalItem(db))).Methods(http.MethodPatch)
			r.HandleFunc("/{itemId}", registerGet("delete_global_item", handleDeleteGlobalItem(db))).Methods(http.MethodDelete)
		}
	}
}

func handleGetAllGlobalItems(db *gorm.DB) rest.GetHandler {
...
func handleCreateGlobalItem(db *gorm.DB) rest.InputHandler[RestModel] {
...
func handleUpdateGlobalItem(db *gorm.DB) rest.InputHandler[RestModel] {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext, rm RestModel) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			...
			err = NewProcessor(d.Logger(), d.Context(), db).Update(uint32(itemId), rm.ItemId, rm.Quantity, rm.Tier)
			if err != nil {
...
func handleDeleteGlobalItem(db *gorm.DB) rest.GetHandler {
	return func(d *rest.HandlerDependency, c *rest.HandlerContext) http.HandlerFunc {
		return func(w http.ResponseWriter, r *http.Request) {
			itemIdStr := mux.Vars(r)["itemId"]
			itemId, err := strconv.ParseUint(itemIdStr, 10, 32)
			...
			err = NewProcessor(d.Logger(), d.Context(), db).Delete(uint32(itemId))
			if err != nil {
