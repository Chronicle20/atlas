...
// TestHandleAcceptedEvent_CompletesAcceptToStorageStep verifies that a storage
// compartment ACCEPTED event completes an AcceptToStorage step.
func TestHandleAcceptedEvent_CompletesAcceptToStorageStep(t *testing.T) {
	...
	s, err := saga.NewBuilder().
		...
		SetInitiatedBy("test").
		AddStep("s1", saga.Pending, saga.AcceptToStorage, saga.AcceptToStoragePayload{TransactionId: tx, AccountId: 1, CharacterId: 1, TemplateId: 2070015}).
		AddStep("s2", saga.Pending, saga.SendMessage, saga.SendMessagePayload{CharacterId: 1, Message: "pinned"}).
		Build()
	require.NoError(t, err)
	putTestSaga(t, ctx, s)

	handleAcceptedEvent(logger, ctx, storageCompartment.StatusEvent[storageCompartment.StatusEventAcceptedBody]{
		Type:      storageCompartment.StatusEventTypeAccepted,
		AccountId: 1,
		Body:      storageCompartment.StatusEventAcceptedBody{TransactionId: tx},
	})

	got, err := saga.NewProcessor(logger, ctx).GetById(tx)
	require.NoError(t, err)
	assert.Equal(t, saga.Completed, got.Steps()[0].Status(), "AcceptToStorage step must be completed by storage compartment ACCEPTED")
}

// TestHandleAcceptedEvent_DoesNotCompleteAwardAssetStep verifies anti-match:
// a storage compartment ACCEPTED event must NOT complete an AwardAsset step.
func TestHandleAcceptedEvent_DoesNotCompleteAwardAssetStep(t *testing.T) {
	...
	got, err := saga.NewProcessor(logger, ctx).GetById(tx)
	require.NoError(t, err)
	assert.Equal(t, saga.Pending, got.Steps()[0].Status(), "AwardAsset step must not be completed by storage compartment ACCEPTED")
