...
// CreateAssetCommandBody. It is duplicated verbatim in atlas-inventory's copy
// of this package (kafka/message/compartment/kafka_test.go) because the two
// live in separate Go modules with no shared compile-time contract. If you
// add, rename, or retag a field on CreateAssetCommandBody, update the struct
// AND this literal in BOTH modules together.
...
// TestCreateAssetCommandBody_GoldenJSON_AgreesWithInventory pins this
// module's CreateAssetCommandBody to the shared golden literal, so a field
// rename or json tag drift against atlas-inventory's copy is caught here
// rather than silently dropping stats on the wire at runtime.
func TestCreateAssetCommandBody_GoldenJSON_AgreesWithInventory(t *testing.T) {
