...
type Processor interface {
	...
	AcceptAndEmit(transactionId uuid.UUID, worldId byte, accountId uint32, slot int16, templateId uint32, referenceId uint32, referenceType string, referenceData []byte) error
	Accept(mb *message.Buffer) func(transactionId uuid.UUID, worldId byte, accountId uint32, slot int16, templateId uint32, referenceId uint32, referenceType string, referenceData []byte) error
	ReleaseAndEmit(transactionId uuid.UUID, worldId byte, accountId uint32, assetId uint32) error
	Release(mb *message.Buffer) func(transactionId uuid.UUID, worldId byte, accountId uint32, assetId uint32) error
}
...
func (p *ProcessorImpl) AcceptAndEmit(transactionId uuid.UUID, worldId byte, accountId uint32, slot int16, templateId uint32, referenceId uint32, referenceType string, referenceData []byte) error {
	return message.Emit(p.p)(func(mb *message.Buffer) error {
		return p.Accept(mb)(transactionId, worldId, accountId, slot, templateId, referenceId, referenceType, referenceData)
	})
}

func (p *ProcessorImpl) Accept(mb *message.Buffer) func(transactionId uuid.UUID, worldId byte, accountId uint32, slot int16, templateId uint32, referenceId uint32, referenceType string, referenceData []byte) error {
	return func(transactionId uuid.UUID, worldId byte, accountId uint32, slot int16, templateId uint32, referenceId uint32, referenceType string, referenceData []byte) error {
		return mb.Put(storageCompartment.EnvCommandTopic, AcceptCommandProvider(transactionId, worldId, accountId, slot, templateId, referenceId, referenceType, referenceData))
	}
}

func (p *ProcessorImpl) ReleaseAndEmit(transactionId uuid.UUID, worldId byte, accountId uint32, assetId uint32) error {
	return message.Emit(p.p)(func(mb *message.Buffer) error {
		return p.Release(mb)(transactionId, worldId, accountId, assetId)
	})
}

func (p *ProcessorImpl) Release(mb *message.Buffer) func(transactionId uuid.UUID, worldId byte, accountId uint32, assetId uint32) error {
	return func(transactionId uuid.UUID, worldId byte, accountId uint32, assetId uint32) error {
		return mb.Put(storageCompartment.EnvCommandTopic, ReleaseCommandProvider(transactionId, worldId, accountId, assetId))
	}
}
