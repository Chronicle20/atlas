...
// CompartmentRestModel mirrors the atlas-inventory equip-compartment resource
// (mirrors services/atlas-effective-stats/.../external/inventory/rest.go). The
// summon damage ceiling only needs the equipped weapon's templateId, so the
// asset model is reduced to slot+templateId.
...
// SetReferencedStructs hydrates the stub assets created by SetToManyReferenceIDs
// from the `included` resources, populating slot+templateId. Without it the
// asset Slot/TemplateId stay zero and the weapon lookup would never match
// slot -11.
...
// AssetRestModel is the minimal equipped-asset view: slot identifies the equip
// position (weapon = -11) and templateId classifies the weapon type.
