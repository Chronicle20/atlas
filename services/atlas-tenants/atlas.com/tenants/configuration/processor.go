...
func (p *ProcessorImpl) CreateIncubatorRewardAndEmit(tenantId uuid.UUID, reward map[string]interface{}) (Model, error) {
	...
	txErr := database.ExecuteTransaction(p.db.WithContext(p.ctx), func(tx *gorm.DB) error {
		...
		result, err = message.EmitWithResult[Model, uuid.UUID](outbox.EmitProvider(p.l, p.ctx, tx))(func(mb *message.Buffer) func(uuid.UUID) (Model, error) {
			return func(tenantId uuid.UUID) (Model, error) {
				return NewProcessor(p.l, p.ctx, tx).CreateIncubatorReward(mb)(tenantId)(reward)
			}
		})(tenantId)
		return err
	})
...
func (p *ProcessorImpl) UpdateIncubatorRewardAndEmit(tenantId uuid.UUID, incubatorRewardID string, reward map[string]interface{}) (Model, error) {
	...
	txErr := database.ExecuteTransaction(p.db.WithContext(p.ctx), func(tx *gorm.DB) error {
		...
		result, err = message.EmitWithResult[Model, uuid.UUID](outbox.EmitProvider(p.l, p.ctx, tx))(func(mb *message.Buffer) func(uuid.UUID) (Model, error) {
			return func(tenantId uuid.UUID) (Model, error) {
				return NewProcessor(p.l, p.ctx, tx).UpdateIncubatorReward(mb)(tenantId)(incubatorRewardID)(reward)
			}
		})(tenantId)
		return err
	})
...
func (p *ProcessorImpl) DeleteIncubatorRewardAndEmit(tenantId uuid.UUID, incubatorRewardID string) error {
	return database.ExecuteTransaction(p.db.WithContext(p.ctx), func(tx *gorm.DB) error {
		return message.Emit(outbox.EmitProvider(p.l, p.ctx, tx))(func(mb *message.Buffer) error {
			return NewProcessor(p.l, p.ctx, tx).DeleteIncubatorReward(mb)(tenantId)(incubatorRewardID)
		})
	})
}
