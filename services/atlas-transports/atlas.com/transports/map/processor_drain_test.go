...
// TestCharacterIdsInMapProviderDrainsBeyondOnePage proves
// CharacterIdsInMapProvider (via requests.DrainProvider) fetches every page
// of characters-in-a-map-instance rather than stopping after the first
// response. atlas-maps' GET
// /worlds/{w}/channels/{c}/maps/{m}/instances/{i}/characters is now
// paginated (task-117); this consumer needs the complete set to identify
// transport passengers -- the fixture server serves 300 characters across
// two pages of 250, so only a genuine drain picks up character id 300,
// which lives on page 2.
func TestCharacterIdsInMapProviderDrainsBeyondOnePage(t *testing.T) {
	...
	ids, err := _map.NewProcessor(l, ctx).CharacterIdsInMapProvider(f)()
	if err != nil {
		t.Fatal(err)
	}
	if len(ids) != 300 {
