...
export function InventoryCard({
  ...
  // Read the active tenant defensively (undefined outside a TenantProvider, e.g.
  // in unit tests) so the seal badge can resolve the game lock asset; SealIcon
  // falls back to a lucide Lock when the tenant is absent.
  const sealTenant = useContext(TenantContext)?.activeTenant ?? null;
  ...
  const { itemData, isLoading, hasError, errorMessage } = useItemData(
  ...
  useEffect(() => {
    if (
      ...
      img.onload = () => {
        console.log(
          `[InventoryCard] Preloaded image for item ${asset.attributes.templateId}`,
        );
      };
      ...
      img.onerror = () => {
        console.warn(
          `[InventoryCard] Failed to preload image for item ${asset.attributes.templateId}`,
        );
      };
  ...
  const handleImageError = () => {
    ...
    setImageLoaded(true);
    console.warn(
      `Failed to load item icon for item ${asset.attributes.templateId}`,
    );
  };
  ...
  const displayError =
    hasError || errorMessage || (!isLoading && !hasIcon && !hasName);
  ...
  return (
    <Card
      ref={lazyRef}
      className={cn(
        "overflow-hidden relative py-0 w-[100px] h-[120px]",
        isSealed(asset) && "ring-1 ring-amber-400/60",
        className,
      )}
    >
      {/* Delete Button */}
      ...
      <CardContent className="p-2 pt-0 flex flex-col items-center justify-center min-h-[70px]">
        {!shouldLoad && !shouldPreload ? (
          ...
          <div
            ...
            data-testid="inventory-card-loading"
          >
          ...
          <div className="flex flex-col items-center text-center space-y-1">
            <div className="relative h-8 w-8 flex-shrink-0 flex items-center justify-center">
              <Package
                ...
                data-testid="inventory-card-package-icon"
              />
            </div>
            ...
            <div className="relative h-8 w-8 flex-shrink-0">
              {hasIcon && itemData?.iconUrl ? (
                <>
                  <img
                    ...
                    width={32}
                    height={32}
                    loading={shouldLoad || shouldPreload ? "eager" : "lazy"}
                    className={cn(
              ...
              {isTagged(asset) && (
                <Tag
                  ...
                  className="absolute top-0 right-0 h-3 w-3 text-amber-500"
              ...
              {isSealed(asset) && (
                <SealIcon
                  tenant={sealTenant}
                  className="absolute bottom-0 right-0 h-3 w-3 text-amber-500"
                />
              )}
            </div>
