...
function CharacterRenderErrorFallback({
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center p-4 border-2 border-dashed border-red-300 rounded-lg bg-red-50 ${className}`}
    >
...
const DynamicCharacterRenderer = lazy(() =>
  import("./CharacterRenderer").then((mod) => ({
    default: mod.CharacterRenderer,
  })),
);
 ...
 */
const OptimizedCharacterRendererComponent =
  memo<OptimizedCharacterRendererProps>(
    ({
      character,
      inventory = [],
      scale = 2,
      ...
      useMemo(() => {
        if (preloadSiblings.length > 0 && activeTenant) {
          ...
          const siblingData = preloadSiblings.map((sibling) => {
            const loadout = characterToLoadout(
              sibling.character,
              sibling.inventory || [],
            );
            return {
 ...
 */
export function OptimizedCharacterRenderer(
  props: OptimizedCharacterRendererProps,
) {
...
export function CharacterGallery({
  ...
  useMemo(() => {
    if (priority && characters.length > 0 && activeTenant) {
      const characterData = characters.map(
        ({ character, inventory = [], scale = 2 }) => {
    ...
    <div className={`grid gap-4 ${className}`}>
      {characters.map(({ character, inventory, scale }, index) => (
        <div
          key={character.id}
          className={
            onCharacterClick ? "cursor-pointer hover:opacity-80" : undefined
          }
