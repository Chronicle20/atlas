...
export function SkillTooltipContent({ definition, learnedLevel }: Props) {
  ...
  return (
    <div className="space-y-1 max-w-sm text-xs">
      ...
      {definition.element && (
        <div>
          <strong>Element:</strong> {definition.element}
        </div>
      )}
      {definition.animationTime > 0 && (
        <div>
          <strong>Animation:</strong> {definition.animationTime}ms
        </div>
      )}
      {effect?.cooldown != null && effect.cooldown > 0 && (
        <div>
          <strong>Cooldown:</strong> {effect.cooldown}s
        </div>
      )}
      {effect?.MPConsume != null && effect.MPConsume > 0 && (
        <div>
          <strong>MP Cost:</strong> {effect.MPConsume}
        </div>
      )}
      {effect?.HPConsume != null && effect.HPConsume > 0 && (
        <div>
          <strong>HP Cost:</strong> {effect.HPConsume}
        </div>
      )}
