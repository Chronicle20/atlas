...
describe("HpMpBar", () => {
  it("renders cur / max text", () => {
    const { getByText } = render(
      <HpMpBar label="HP" cur={50} max={100} colorClass="bg-red-500" />,
    );
  ...
  it("computes width as a percentage", () => {
    const { container } = render(
      <HpMpBar label="HP" cur={25} max={100} colorClass="bg-red-500" />,
    );
  ...
  it("clamps to 100% when cur > max", () => {
    const { container } = render(
      <HpMpBar label="HP" cur={500} max={100} colorClass="bg-red-500" />,
    );
    const fill = container.querySelector(".bg-red-500") as HTMLElement;
  ...
  it("renders 0% when max is 0", () => {
    const { container } = render(
      <HpMpBar label="MP" cur={10} max={0} colorClass="bg-blue-500" />,
    );
