...
function Harness({
  ...
  return (
    <InventorySection
      ...
      onAdd={(templateId) => dispatch({ type: "addInventory", key: preset.key, templateId })}
      onRemove={(index) => dispatch({ type: "removeInventory", key: preset.key, index })}
      onSetQty={(index, value) => dispatch({ type: "setInventoryQty", key: preset.key, index, value })}
    />
  );
}
...
describe("InventorySection", () => {
  ...
  it("reducer round-trip: clearing and retyping commits exactly the typed value, not a prepend", async () => {
    const seedPresets: CharacterPreset[] = [
      {
        id: "p1",
        attributes: { ...DEFAULT_PRESET_ATTRIBUTES, inventory: [{ templateId: 2000000, quantity: 5 }] },
      },
    ];
  ...
  it("resyncs the draft from a new value when NOT focused (preset switch)", async () => {
    const seedPresets: CharacterPreset[] = [
      ...
      {
        id: "p2",
        attributes: { ...DEFAULT_PRESET_ATTRIBUTES, inventory: [{ templateId: 2000000, quantity: 20 }] },
      },
    ];
    const loaded = presetReducer(initialPresetEditorState(), { type: "load", presets: seedPresets });
    const initialState = presetReducer(loaded, { type: "select", key: "p1" });
