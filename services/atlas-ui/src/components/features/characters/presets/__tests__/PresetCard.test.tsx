...
describe("PresetCard", () => {
  ...
  it("opening (click/Enter) fires onOpen; Duplicate fires onDuplicate", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /open preset Test Warrior/i }),
    );
  ...
  it("hides Apply quick-action when onApply is absent", () => {
    ...
    expect(
      screen.queryByRole("button", { name: /apply to account/i }),
    ).toBeNull();
  });
  ...
  it("clicking Apply fires onApply", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: /apply to account/i }),
    );
