...
describe("RecipesByItemCard", () => {
  it("renders the loading message while data loads", () => {
    render(
      wrap(
        <RecipesByItemCard recipes={undefined} isLoading={true} error={null} />,
      ),
    );
  ...
  it("renders the empty-state copy when no recipes match", () => {
    render(
      wrap(<RecipesByItemCard recipes={[]} isLoading={false} error={null} />),
    );
  ...
  it("renders one widget per recipe and shows the count in the title", () => {
    render(
      wrap(
        <RecipesByItemCard
          recipes={[sample, { ...sample, id: "r2", stateId: "craft1" }]}
          isLoading={false}
          error={null}
        />,
      ),
