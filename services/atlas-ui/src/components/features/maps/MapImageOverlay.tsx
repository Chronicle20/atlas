...
function computeMarkers<
  T extends { id: string; attributes: { x: number; y: number } },
>(entities: T[] | undefined, bounds: MapBounds): ComputedMarker<T>[] {
  if (!entities) return [];
  return entities.map((e, i) => {
    ...
    const outOfBounds =
      leftNum < 0 || leftNum > 100 || topNum < 0 || topNum > 100;
...
export function MapImageOverlay({
  ...
  const monsterMarkers = useMemo(
    () => computeMarkers(monsters, bounds),
    [monsters, bounds],
  );
  const reactorMarkers = useMemo(
    () => computeMarkers(reactors, bounds),
    [reactors, bounds],
  );
  const npcMarkers = useMemo(
    () => computeMarkers(npcs, bounds),
    [npcs, bounds],
  );
  const portalMarkers = useMemo(
  ...
  useEffect(() => {
    ...
    const oob: string[] = [];
    for (const m of monsterMarkers)
      if (m.outOfBounds) oob.push(`monster:${m.entity.id}`);
    for (const m of reactorMarkers)
      if (m.outOfBounds) oob.push(`reactor:${m.entity.id}`);
...
function PortalMarker({
  ...
  return (
    <MarkerShell
      ...
      className={cn(
        sizing.primary,
        "rotate-45 bg-emerald-500/70 border-2 border-white",
      )}
...
function NpcMarker({
  ...
  return (
    <MarkerShell
      ...
      className={cn(
        sizing.primary,
        "rounded-full bg-sky-500/70 border-2 border-white",
      )}
      highlightRingColor="ring-yellow-400"
    />
  );
...
function ReactorMarker({
  ...
  return (
    <MarkerShell
      ...
      tooltip={
        reactor.attributes.name || String(reactor.attributes.classification)
      }
...
function MonsterMarker({
  ...
  return (
    <MarkerShell
      ...
      target={{
        kind: "monster",
        template: monster.attributes.template,
        spawnIndex,
      }}
