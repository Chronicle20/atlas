    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <Badge
            variant="outline"
            className="px-2.5 py-1 text-xs font-medium cursor-help"
          >
            {label}
          </Badge>
        </TooltipTrigger>
...
function summarizeEffect(
  ...
  if (a.hp > 0 && a.hp < 100)
    rows.push({ label: "HP trigger", value: `≤ ${a.hp}%` });
  if (a.interval > 0) rows.push({ label: "Interval", value: `${a.interval}s` });
  if (a.count > 0)
    rows.push({ label: "Count", value: a.count.toLocaleString() });
  if (a.limit > 0)
    rows.push({ label: "Limit", value: a.limit.toLocaleString() });
  if (a.summons && a.summons.length > 0) {
    rows.push({
      label: "Summons",
      value: `${a.summons.length} mob${a.summons.length === 1 ? "" : "s"}`,
    });
  }
  ...
  if (
    aoeW > 0 &&
    aoeH > 0 &&
    (a.lt_x !== 0 || a.lt_y !== 0 || a.rb_x !== 0 || a.rb_y !== 0)
  ) {
