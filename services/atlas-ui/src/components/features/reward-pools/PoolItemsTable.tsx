...
function tierBadgeVariant(tier: string): "destructive" | "secondary" | "outline" {
  ...
  function renderItem(itemId: number) {
    ...
    return (
      <span className="inline-flex items-center gap-2">
        {iconUrl && <img src={iconUrl} alt="" width={24} height={24} loading="lazy" />}
            ...
            <TableBody>
              {items.map((it) => (
                <TableRow key={it.id}>
                  ...
                  <TableCell>{it.attributes.weight > 0 ? it.attributes.weight : "—"}</TableCell>
                  <TableCell>{pct(chances.get(it.id) ?? 0)}</TableCell>
                  <TableCell className="text-right">
                    <PoolItemActions onEdit={() => onEdit(it)} onDelete={() => onDelete(it)} />
                  </TableCell>
                </TableRow>
              ))}
              {items.length === 0 && (
                <TableRow>
                  <TableCell colSpan={5} className="text-muted-foreground">No items in this pool.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
      ...
      <div className="shrink-0 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {mixedTiers.length > 0 && (
          <Alert variant="destructive" className="sm:max-w-md">
            <AlertDescription>
              {mixedTiers.join(", ")} {mixedTiers.length === 1 ? "tier mixes" : "tiers mix"} weighted and unweighted
              items — the weighted roll excludes every zero-weight item (including all global items) in{" "}
        ...
        <Table>
          ...
          <TableBody>
            {visibleRows.map((r) => {
              ...
              return (
                <TableRow key={r.key} className={c?.excluded ? "opacity-60" : undefined}>
                  ...
                  <TableCell><Badge variant={tierBadgeVariant(r.tier)}>{r.tier}</Badge></TableCell>
                  ...
                  <TableCell className="text-right">
                    {r.source === "machine" && r.item && (
                      <PoolItemActions onEdit={() => onEdit(r.item as RewardPoolItemData)} onDelete={() => onDelete(r.item as RewardPoolItemData)} />
                    )}
                  </TableCell>
                </TableRow>
              );
            ...
            {visibleRows.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-muted-foreground">No items in this pool.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
