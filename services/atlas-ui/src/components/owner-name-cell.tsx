...
export function OwnerNameCell({ characterId, tenant }: { characterId: string; tenant: Tenant | null }) {
    ...
    useEffect(() => {
        ...
        charactersService.getById(characterId)
            .then((character) => {
                ...
                if (!cancelled) setResolved((prev) => ({ ...prev, [characterId]: characterName }))
            })
            .catch(() => {
                // Fall back to the id on failure.
                if (!cancelled) setResolved((prev) => ({ ...prev, [characterId]: characterId }))
            })
        return () => {
            cancelled = true
        }
    ...
    const name = characterNameCache.get(characterId) ?? resolved[characterId] ?? characterId
    return <>{name}</>
}
