...
export function OptionsField<T extends FieldValues>({
  form,
  path,
}: OptionsFieldProps<T>) {
  const options = useWatch({ control: form.control, name: path }) as
  ...
  const handleAddOption = (key: string) => {
    ...
    setNewValue("");
    form.setValue(path, { ...currentOptions, [key]: "" } as PathValue<
      T,
      Path<T>
    >);
  };
  ...
  const handleAddOptionChild = (opt: string, key: string, value: string) => {
    ...
    if (isComplex) {
      parsedValue = complexProperties.reduce(
        (obj, prop) => {
        ...
        <Dialog open={isOptionsDialogOpen} onOpenChange={setDialogOptionsOpen}>
          <DialogContent>
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setDialogOptionsOpen(false)}
              >
                Cancel
              </Button>
              <Button
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <Dialog
        open={isOptionChildDialogOpen}
        onOpenChange={setDialogOptionChildOpen}
      >
        <DialogContent>
          <DialogHeader>
              ...
              {complexProperties.map((prop, index) => (
                <div key={index} className="flex gap-2 items-center">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setComplexProperties(
                        complexProperties.filter((_, i) => i !== index),
                      )
                    }
                  >
                    <X />
