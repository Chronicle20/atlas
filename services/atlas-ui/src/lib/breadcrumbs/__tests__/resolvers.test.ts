...
describe("Entity Type Utilities", () => {
  describe("getEntityTypeFromRoute", () => {
    it("should identify account routes", () => {
      ...
      expect(getEntityTypeFromRoute("/accounts/uuid-123/settings")).toBe(
        EntityType.ACCOUNT,
      );
    });

    it("should identify character routes", () => {
      expect(getEntityTypeFromRoute("/characters/456")).toBe(
        EntityType.CHARACTER,
      );
      expect(getEntityTypeFromRoute("/characters/456/inventory")).toBe(
        EntityType.CHARACTER,
      );
    });
    ...
    it("should identify guild routes", () => {
      ...
      expect(getEntityTypeFromRoute("/guilds/789/members")).toBe(
        EntityType.GUILD,
      );
    });
    ...
    it("should identify template routes", () => {
      expect(getEntityTypeFromRoute("/templates/template-id")).toBe(
        EntityType.TEMPLATE,
      );
      expect(getEntityTypeFromRoute("/templates/template-id/properties")).toBe(
        EntityType.TEMPLATE,
      );
    });

    it("should identify tenant routes", () => {
      expect(getEntityTypeFromRoute("/tenants/tenant-123")).toBe(
        EntityType.TENANT,
      );
      expect(getEntityTypeFromRoute("/tenants/tenant-123/settings")).toBe(
        EntityType.TENANT,
      );
    });
...
describe("Entity Label Resolution", () => {
  ...
  describe("Preloading", () => {
    it("should preload entities not in cache", async () => {
      ...
      // Preload
      await preloadEntityLabels(EntityType.CHARACTER, ["123"], mockTenant);
