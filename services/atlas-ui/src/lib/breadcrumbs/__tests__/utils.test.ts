...
describe("Breadcrumb Utils", () => {
  ...
  describe("getEntityType", () => {
    it("should return correct entity types", () => {
      ...
      expect(getEntityType("template-id", "/templates/template-id")).toBe(
        "template",
      );
    });
  ...
  describe("buildBreadcrumbPath", () => {
    const longPath: BreadcrumbSegment[] = [
      ...
      },
      {
        segment: "templates",
        label: "Templates",
        href: "/tenants/uuid-1/character/templates",
        dynamic: false,
        isCurrentPage: true,
      },
    ];
    ...
    it("should truncate items when over limit", () => {
      const result = buildBreadcrumbPath(longPath, {
        maxItems: 3,
        showEllipsis: false,
      });
      expect(result).toHaveLength(3);
