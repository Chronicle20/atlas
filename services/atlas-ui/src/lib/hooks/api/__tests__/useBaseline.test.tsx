...
describe("useRestoreBaseline", () => {
  ...
  it("rejects when tenant is null", async () => {
    ...
    const { result } = renderHook(() => useRestoreBaseline(null), {
  ...
  it("invalidates dataStatus query on success", async () => {
    (baselineService.restore as ReturnType<typeof vi.fn>).mockResolvedValue(
      undefined,
    );
    const qc = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    ...
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: dataStatusKey(mockTenant.id),
    });
  });
