...
const mockConversationsService = conversationsService as Mocked<
...
describe("useConversations hooks", () => {
  ...
  describe("useConversations", () => {
    ...
    it("should not fetch when tenant is not provided", () => {
      ...
      const { result } = renderHook(
        () => useConversations(null as unknown as Tenant),
        {
          wrapper: createWrapper(),
        },
      );
  ...
  describe("useConversation", () => {
    it("should fetch specific conversation successfully", async () => {
      ...
      const { result } = renderHook(
        () => useConversation(mockTenant, "conversation-1"),
        {
          wrapper: createWrapper(),
        },
      );
      ...
      expect(mockConversationsService.getById).toHaveBeenCalledWith(
        "conversation-1",
        { useCache: false },
      );
    });
  ...
  describe("useConversationExists", () => {
    it("should check if conversation exists successfully", async () => {
      ...
      const { result } = renderHook(
        () => useConversationExists(mockTenant, "conversation-1"),
        {
          wrapper: createWrapper(),
        },
      );
      ...
      expect(mockConversationsService.exists).toHaveBeenCalledWith(
        "conversation-1",
        { useCache: false },
      );
    });
  ...
  describe("useConversationByNpc", () => {
    it("should fetch conversation by NPC ID successfully", async () => {
      ...
      const { result } = renderHook(
        () => useConversationByNpc(mockTenant, 100),
        {
          wrapper: createWrapper(),
        },
      );
      ...
      expect(result.current.data).toEqual(mockConversation);
      expect(mockConversationsService.getByNpcId).toHaveBeenCalledWith(100, {
        useCache: false,
      });
    });
    ...
    it("should return null when no conversation exists for NPC", async () => {
      ...
      const { result } = renderHook(
        () => useConversationByNpc(mockTenant, 999),
        {
          wrapper: createWrapper(),
        },
      );
  ...
  describe("useConversationSearch", () => {
    it("should search conversations by text successfully", async () => {
      mockConversationsService.searchByText.mockResolvedValue([
        mockConversation,
      ]);

      const { result } = renderHook(
        () => useConversationSearch(mockTenant, "shop"),
        {
          wrapper: createWrapper(),
        },
      );
      ...
      expect(result.current.data).toEqual([mockConversation]);
      expect(mockConversationsService.searchByText).toHaveBeenCalledWith(
        "shop",
        { useCache: false },
      );
    });
    ...
    it("should not search when search text is too short", () => {
      const { result } = renderHook(
        () => useConversationSearch(mockTenant, "ab"),
        {
          wrapper: createWrapper(),
        },
      );
  ...
  describe("useDeleteConversation", () => {
    it("should delete conversation successfully", async () => {
      ...
      expect(mockConversationsService.delete).toHaveBeenCalledWith(
        "conversation-1",
        undefined,
      );
    });
  ...
  describe("conversationKeys", () => {
    it("should generate correct query keys", () => {
      ...
      expect(conversationKeys.list(mockTenant)).toEqual([
        "conversations",
        "list",
        "tenant-1",
        undefined,
      ...
      expect(conversationKeys.detail(mockTenant, "conversation-1")).toEqual([
        "conversations",
        "detail",
        "tenant-1",
        "conversation-1",
      ...
      expect(conversationKeys.npcConversation(mockTenant, 100)).toEqual([
        "conversations",
        "byNpc",
        "tenant-1",
        100,
      ...
      expect(conversationKeys.search(mockTenant, "test")).toEqual([
        "conversations",
        "search",
        "tenant-1",
        "test",
      ]);
      expect(conversationKeys.validation()).toEqual([
        "conversations",
        "validation",
      ]);
      expect(
        conversationKeys.stateConsistency(mockTenant, "conversation-1"),
      ).toEqual(["conversations", "validation", "tenant-1", "conversation-1"]);
      expect(conversationKeys.exports()).toEqual(["conversations", "export"]);
      expect(conversationKeys.export(mockTenant, "json")).toEqual([
        "conversations",
        "export",
        "tenant-1",
        "json",
    ...
    it("should handle null tenant in query keys", () => {
      expect(conversationKeys.list(null)).toEqual([
        "conversations",
        "list",
        "no-tenant",
        undefined,
      ]);
      expect(conversationKeys.detail(null, "conversation-1")).toEqual([
        "conversations",
        "detail",
        "no-tenant",
        "conversation-1",
      ]);
      expect(conversationKeys.npcConversation(null, 100)).toEqual([
        "conversations",
        "byNpc",
        "no-tenant",
        100,
