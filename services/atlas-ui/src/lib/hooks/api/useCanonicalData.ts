...
): UseQueryResult<WzInputStatus, Error> {
  return useQuery({
...
export function useCanonicalDataStatus(
  sel: CanonicalSelection | null,
): UseQueryResult<DataStatus, Error> {
  return useQuery({
    queryKey: sel
      ? canonicalDataStatusKey(sel)
      : ["canonical", "dataStatus", "none"],
...
export function useUploadCanonicalWz(
  sel: CanonicalSelection | null,
): UseMutationResult<void, Error, File> {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({
        queryKey: canonicalWzInputKey(sel),
      });
    },
...
export function useRunCanonicalProcessing(
  sel: CanonicalSelection | null,
  ...
  return useMutation({
    mutationFn: () => {
      if (!sel) {
        throw new Error(
          "useRunCanonicalProcessing: no region/version selected",
        );
      }
    ...
    onSuccess: () => {
      if (!sel) return;
      void queryClient.invalidateQueries({
        queryKey: canonicalDataStatusKey(sel),
      });
    },
  });
}
...
export function usePublishCanonicalBaseline(
  sel: CanonicalSelection | null,
): UseMutationResult<void, Error, void> {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: () => {
      if (!sel) {
        throw new Error(
          "usePublishCanonicalBaseline: no region/version selected",
        );
      }
