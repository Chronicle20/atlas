...
export function useGachapons(
  options?: QueryOptions,
): UseQueryResult<GachaponData[], Error> {
  const { activeTenant } = useTenant();
  return useQuery({
    queryKey: gachaponKeys.list(options),
    queryFn: () =>
      gachaponsService.getAllGachapons({ ...options, useCache: false }),
...
export function useGachaponPrizePool(
  gachaponId: string,
): UseQueryResult<GachaponRewardData[], Error> {
  const { activeTenant } = useTenant();
  return useQuery({
