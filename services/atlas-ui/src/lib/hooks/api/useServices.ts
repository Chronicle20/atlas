...
export function useServices(
  ...
  return useQuery({
    ...
    queryFn: () =>
      servicesService.getAllServices({ ...options, useCache: false }),
...
export function useService(
  ...
  return useQuery({
    ...
    queryFn: () =>
      servicesService.getServiceById(id, { ...options, useCache: false }),
    enabled: !!id,
    gcTime: 10 * 60 * 1000,
  });
}
...
> {
  ...
  return useMutation({
    mutationFn: (input: CreateServiceInput) =>
      servicesService.createService(input),
...
> {
  ...
  return useMutation({
    ...
    onMutate: async ({ id }) => {
      ...
      const previousList = queryClient.getQueryData<Service[]>(
        serviceKeys.lists(),
      );
...
export type { Service, CreateServiceInput, UpdateServiceInput };
