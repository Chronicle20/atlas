 ...
 */
function compactRenderOptions(o: {
...
const DEFAULT_OPTIONS: Required<
  Omit<
    ...
    "onSuccess" | "onError" | "region" | "majorVersion"
  >
> = {
  enabled: true,
  priority: false,
...
export function useCharacterImage(
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<CharacterImageResult> => {
      const url = generateCharacterUrl(
        ...
        character.minorVersion,
        {
          ...
          equipment: Object.fromEntries(
            Object.entries(character.equipment).map(([k, v]) => [
              k,
              v as number,
            ]),
          ),
      ...
      if (renderOptions?.stance !== undefined)
        mergedOptions.stance = renderOptions.stance;
      if (renderOptions?.frame !== undefined)
        mergedOptions.frame = renderOptions.frame;
      if (renderOptions?.resize !== undefined)
        mergedOptions.resize = renderOptions.resize;
      if (renderOptions?.renderMode !== undefined)
        mergedOptions.renderMode = renderOptions.renderMode;
      if (renderOptions?.flipX !== undefined)
        mergedOptions.flipX = renderOptions.flipX;
  ...
  const prefetchVariants = useCallback(
    (variants: Array<Partial<CharacterRenderOptions>>) => {
      ...
      const preloadPromises = characters.map(({ character, options }) => {
        ...
        return queryClient.prefetchQuery({
          ...
          queryFn: () => {
            ...
            if (options?.stance !== undefined)
              preloadOpts.stance = options.stance;
            if (options?.frame !== undefined) preloadOpts.frame = options.frame;
            if (options?.resize !== undefined)
              preloadOpts.resize = options.resize;
            if (options?.renderMode !== undefined)
              preloadOpts.renderMode = options.renderMode;
...
export function useCharacterImageCache() {
  ...
  const getCacheStats = useCallback(() => {
    ...
    const characterImageQueries = cache.findAll({
      queryKey: ["character-image"],
    });

    return {
      totalQueries: characterImageQueries.length,
      activeQueries: characterImageQueries.filter(
        (q) => q.state.status === "success",
      ).length,
      errorQueries: characterImageQueries.filter(
        (q) => q.state.status === "error",
      ).length,
      loadingQueries: characterImageQueries.filter(
      ...
      const warmupPromises = characters.map((character) => {
        ...
        return queryClient.prefetchQuery({
          ...
          queryFn: () => {
            ...
            if (options?.renderMode !== undefined)
              warmOpts.renderMode = options.renderMode;
