...
export function useNpcErrorHandler(options: NpcErrorHandlerOptions = {}) {
  ...
  const classifyError = useCallback(
    (error: Error | string, npcId: number): NpcError => {
      ...
      let userFriendlyMessage =
        "An unexpected error occurred while loading NPC data.";
      ...
      if (
        ...
        type = "network";
        userFriendlyMessage =
          "Unable to connect to the MapleStory API. Please check your internet connection and try again.";
        retryable = true;
      }
      // API errors
        ...
        type = "api";
        userFriendlyMessage =
          "The MapleStory API is temporarily unavailable. Please try again later.";
        retryable = true;
      } else if (
        ...
        type = "api";
        userFriendlyMessage =
          "Access to NPC data is currently restricted. Please try again later.";
        retryable = false;
      } else if (
      ...
      // Parsing errors
        ...
        type = "parsing";
        userFriendlyMessage =
          "Received invalid data from the MapleStory API. Please try again.";
        retryable = true;
      }
      // Rendering errors
      else if (
        ...
        type = "rendering";
        userFriendlyMessage =
          "There was an error displaying the NPC card. Please refresh the page.";
        retryable = true;
      }
  ...
  const handleError = useCallback(
    (
      ...
      if (config.logErrors) {
        const errorToLog =
          npcError.originalError || new Error(npcError.message);
  ...
  const handleErrors = useCallback(
    (
      ...
      if (
        ...
        toastCountRef.current < config.maxToastsPerMinute
      ) {
        const retryableCount = processedErrors.filter(
          (e) => e.retryable,
        ).length;
   ...
   */
  const createNpcErrorHandler = useCallback(
    (npcId: number) => {
