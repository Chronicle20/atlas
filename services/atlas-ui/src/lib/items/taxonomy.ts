export type Compartment =
  "equipment" | "use" | "setup" | "etc" | "cash" | "unknown";
...
export const COMPARTMENT_TAXONOMY: Record<
  Exclude<Compartment, "unknown">,
  string[]
> = {
  equipment: [
    "hat",
  ...
  cash: [
    ...
    "avatar-megaphone",
    "character-imprint",
    "cosmetic-membership-coupon",
    "character-creation",
    "remote-merchant",
    "pet-multi-consumable",
    "remote-store",
    "other-cash",
  ],
};
...
export function serializeClassFilter(
  selected: Set<ClassOption>,
  allClasses: boolean,
): string {
...
export function parseClassFilter(raw: string | null): {
  selected: Set<ClassOption>;
  allClasses: boolean;
} {
  ...
  const tokens = raw
    .split(",")
    .filter((t): t is ClassOption =>
      (CLASS_OPTIONS as readonly string[]).includes(t),
    );
