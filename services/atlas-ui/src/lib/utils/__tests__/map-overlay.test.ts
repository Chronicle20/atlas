...
describe("worldToOverlayPercent", () => {
  it("maps positive-origin bounds to percentages", () => {
    ...
    expect(worldToOverlayPercent(500, 250, bounds)).toEqual({
      left: "50%",
      top: "50%",
    });
  });
  ...
  it("maps negative-origin bounds (miniMap case)", () => {
    ...
    expect(worldToOverlayPercent(0, 0, bounds)).toEqual({
      left: "50%",
      top: "50%",
    });
  });
  ...
  it("places entity at exact origin at 0%,0%", () => {
    ...
    expect(worldToOverlayPercent(-100, -200, bounds)).toEqual({
      left: "0%",
      top: "0%",
    });
  });
  ...
  it("places entity at far corner at 100%,100%", () => {
    ...
    expect(worldToOverlayPercent(100, 100, bounds)).toEqual({
      left: "100%",
      top: "100%",
    });
  });
