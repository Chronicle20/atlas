...
beforeEach(() => {
  ...
  mockErrorUtils.transformApiError.mockReturnValue(
    "Transformed API error message",
  );
  mockErrorUtils.transformValidationError.mockReturnValue(
    "Transformed validation error message",
  );
});
...
describe("Toast Notification Utilities", () => {
  ...
  describe("promise", () => {
    ...
    it("should handle promise with function error message", async () => {
      const failedPromise = Promise.reject(new Error("Test error")).catch(
        () => {},
      ); // Handle rejection
      const messages = {
        loading: "Loading...",
