...
export function GuildDetailPage() {
  ...
  const error =
    guildQuery.error?.message ?? tenantConfigQuery.error?.message ?? null;

  if (loading) return <PageLoader />;
  if (error || !guild || !tenantConfig)
    return (
      <ErrorDisplay
        error={error || "Guild or tenant configuration not found"}
        className="p-4"
      />
    );
        ...
        <Card className="w-[80%]">
          ...
          <CardContent className="grid grid-cols-4 gap-2 text-sm text-muted-foreground">
            <div>
              ...
              {guild.attributes.members.find(
                (m) => m.characterId === guild.attributes.leaderId,
              )?.name ?? "Unknown"}
            </div>
            <div>
              <strong>World:</strong>{" "}
              {tenantConfig.attributes.worlds[guild.attributes.worldId]?.name ||
                "Unknown"}
            </div>
            <div>
              <strong>Members:</strong> {guild.attributes.members.length}
            </div>
            <div>
              <strong>Capacity:</strong> {guild.attributes.capacity}
            </div>
            <div className="col-span-4">
              <strong>Notice:</strong> {guild.attributes.notice}
            </div>
            <div className="col-span-4">
              <strong>Points:</strong> {guild.attributes.points}
            </div>
            <div>
              <strong>Logo:</strong> {guild.attributes.logo}
            </div>
            <div>
              <strong>Logo Color:</strong> {guild.attributes.logoColor}
            </div>
            <div>
              <strong>Background:</strong> {guild.attributes.logoBackground}
            </div>
            <div>
              <strong>Background Color:</strong>{" "}
              {guild.attributes.logoBackgroundColor}
            </div>
          </CardContent>
        </Card>
              ...
              <TooltipProvider key={title.index}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Badge title={`Index: ${title.index}`}>{title.name}</Badge>
                  </TooltipTrigger>
  ...
  return [
    {
      ...
      cell: ({ row }) => (
        <Link
          to={"/characters/" + row.original.characterId}
