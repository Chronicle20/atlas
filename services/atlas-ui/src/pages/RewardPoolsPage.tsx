...
export function RewardPoolsPage() {
  ...
  const [globalDialog, setGlobalDialog] = useState<{
    open: boolean;
    item?: GlobalRewardItemData;
  }>({ open: false });
  const [globalDelete, setGlobalDelete] = useState<GlobalRewardItemData | null>(
    null,
  );
  ...
  return (
    <div className="flex flex-col flex-1 space-y-6 p-10 pb-16">
      ...
      <Tabs defaultValue="all">
        <div className="flex items-center justify-between">
          <TabsList>
            ...
            <TabsTrigger value="gachapons">
              Gachapons ({gachapons.length})
            </TabsTrigger>
        ...
        <TabsContent value="all" className="mt-4">
          {poolTable(
            ...
            "Seed defaults from Setup, or create a pool.",
          )}
        </TabsContent>
        ...
        <TabsContent value="global" className="mt-4 space-y-4">
          ...
          {globalQuery.isLoading ? (
            ...
            <ErrorDisplay
              ...
              retry={() => void globalQuery.refetch()}
            />
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {globalItems.map((gi) => (
                  <TableRow key={gi.id}>
                    <TableCell>
                      <ItemNameCell
                        itemId={String(gi.attributes.itemId)}
                        tenant={activeTenant}
                      />
                    </TableCell>
                    <TableCell>{gi.attributes.quantity}</TableCell>
                    <TableCell>
                      <Badge variant="outline">{gi.attributes.tier}</Badge>
                    </TableCell>
                    <TableCell className="space-x-2 text-right">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setGlobalDialog({ open: true, item: gi })
                        }
                      >
                        Edit
                      </Button>
      ...
      <AlertDialog
        open={!!globalDelete}
        onOpenChange={(open) => !open && setGlobalDelete(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete global item?</AlertDialogTitle>
            <AlertDialogDescription>
              Item {globalDelete?.attributes.itemId} will stop appearing in
              every gachapon's {globalDelete?.attributes.tier} rolls.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
