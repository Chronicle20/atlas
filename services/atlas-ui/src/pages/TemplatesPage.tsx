...
export function TemplatesPage() {
  ...
  const templatesQuery = useTemplatesPage({
    number: pageNumber,
    size: PAGE_SIZE,
  });
  ...
  const [templateForTenant, setTemplateForTenant] = useState<Template | null>(
    null,
  );
  ...
  const handleCreateTenantFromTemplate = async (data: TenantNameFormData) => {
    ...
    try {
      ...
      if (err instanceof ConfigurationCreationError) {
        toast.error(
          `Tenant created but configuration failed. Tenant ID: ${err.tenantId}. Please retry configuration manually.`,
        );
      } else {
  ...
  return (
    <div className="flex flex-col flex-1 space-y-6 p-10 pb-16">
      ...
      <Dialog open={cloneDialogOpen} onOpenChange={setCloneDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new template based on the selected template. Please
          ...
          <Form {...form}>
            <form
              onSubmit={form.handleSubmit(handleCloneTemplate)}
              className="space-y-4"
            >
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="Enter region (3 characters)"
                        {...field}
                      />
                    </FormControl>
                    <FormDescription>
      ...
      <Dialog
        ...
        onOpenChange={handleCreateTenantDialogOpenChange}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Create Tenant from Template</DialogTitle>
            <DialogDescription>
              Create a new tenant based on the selected template. All
              information from the template will be used.
            </DialogDescription>
          </DialogHeader>
          <Form {...tenantForm}>
            <form
