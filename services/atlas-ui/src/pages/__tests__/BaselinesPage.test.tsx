...
vi.mock("@/components/features/baselines/BaselineTargetPicker", () => ({
  BaselineTargetPicker: ({
    ...
    <button
      onClick={() =>
        onChange({ region: "GMS", majorVersion: 83, minorVersion: 1 })
      }
    >
      pick-gms-83
    </button>
...
let wzStatus:
  | { fileCount: number; totalBytes: number; updatedAt: string | null }
  | undefined;
let dataStatus:
  | {
...
describe("BaselinesPage", () => {
  it("renders the empty state when no baselines are published", () => {
    ...
    expect(
      screen.getByText(/no canonical baselines published yet/i),
    ).toBeInTheDocument();
  });
  ...
  it("disables all workflow rows until a selection exists", () => {
    ...
    expect(
      screen.getByRole("button", { name: /process data/i }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: /publish baseline/i }),
    ).toBeDisabled();
  });
  ...
  it("requires confirmation when re-publishing over an existing baseline", () => {
    ...
    expect(
      screen.getByText(
        /this will replace the shared canonical baseline for GMS v83\.1/i,
      ),
    ).toBeInTheDocument();
