...
vi.mock("@/services/api/items.service", async () => {
  return {
    ...
    buildItemSearchQuery: (
      await vi.importActual<typeof import("@/services/api/items.service")>(
        "@/services/api/items.service",
      )
    ).buildItemSearchQuery,
  };
});
...
describe("ItemsPage", () => {
  ...
  it("fires the default browse query on mount with no params", async () => {
    ...
    const lastCall = vi
      .mocked(itemsService.searchItems)
      .mock.calls.at(-1)![0] as ItemSearchFilters;
  ...
  it("hydrates compartment + class from URL params", async () => {
    ...
    expect(screen.getByRole("button", { name: "Warrior" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    expect(screen.getByRole("button", { name: "Bowman" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
  });
  ...
  it("'All Classes' is mutually exclusive with per-class toggles", async () => {
    ...
    expect(screen.getByRole("button", { name: "All Classes" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    expect(screen.getByRole("button", { name: "Warrior" })).toHaveAttribute(
