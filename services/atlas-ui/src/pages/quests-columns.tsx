  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const itemCount =
          (startReqs?.items?.length || 0) + (endReqs?.items?.length || 0);
        const questCount = startReqs?.quests?.length || 0;

        if (mobCount > 0)
          counts.push(`${mobCount} mob${mobCount > 1 ? "s" : ""}`);
        if (itemCount > 0)
          counts.push(`${itemCount} item${itemCount > 1 ? "s" : ""}`);
        if (questCount > 0)
          counts.push(`${questCount} quest${questCount > 1 ? "s" : ""}`);
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (endActions?.items && endActions.items.length > 0) {
          rewards.push(
            `${endActions.items.length} item${endActions.items.length > 1 ? "s" : ""}`,
          );
        }
