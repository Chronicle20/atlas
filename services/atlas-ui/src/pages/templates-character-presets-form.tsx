...
export function TemplatesPresetsForm() {
  const { id } = useParams();
  ...
  const onSubmit: SubmitHandler<PresetsFormValues> = (data) => {
    ...
    updateTemplate.mutate(
      ...
      },
      {
        ...
        onError: (err: unknown) => {
          const apiErr = err as {
            errors?: { meta?: { path?: string }; detail?: string }[];
            message?: string;
          };
            ...
            apiErr.errors.forEach((e) => {
              ...
              if (path) {
                ...
                if (m) {
                  ...
                  if (idx >= 0) {
                    form.setError(
                      `presets.${idx}.attributes.${field}` as Parameters<
  ...
  if (templateQuery.isLoading)
    return <FormSkeleton fields={6} showLabels showSubmitButton />;
  if (templateQuery.error)
    return <ErrorDisplay error={templateQuery.error.message} />;
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <div className="flex justify-end">
          <Button
            type="button"
            onClick={() =>
              append({ id: undefined, attributes: DEFAULT_PRESET_ATTRIBUTES })
            }
          >
      ...
      {open && (
        <div className="px-4 pb-4 space-y-4 border-t pt-4">
          ...
          <section>
            ...
            <TagsField
              form={form}
              name={`presets.${index}.attributes.tags`}
              title="Tags"
            />
          </section>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        {...field}
                        onChange={(e) =>
                          field.onChange(Number(e.target.value) as 0 | 1)
                        }
                      />
                    </FormControl>
                    <FormMessage />
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              />
              {(["str", "dex", "int", "luk", "hp", "mp"] as const).map(
                (stat) => (
                  <FormField
                    key={stat}
                    control={form.control}
