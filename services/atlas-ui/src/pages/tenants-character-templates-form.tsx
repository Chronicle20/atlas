      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        {fields.map((field, index) => (
          ...
          >
            ...
            <NumbersField
              form={form}
              name={`templates.${index}.faces`}
              title="Faces"
            />
            <NumbersField
              form={form}
              name={`templates.${index}.hairs`}
              title="Hairs"
            />
            <NumbersField
              form={form}
              name={`templates.${index}.hairColors`}
              title="Hair Colors"
            />
            <NumbersField
              form={form}
              name={`templates.${index}.skinColors`}
              title="Skin Colors"
            />
            <NumbersField
              form={form}
              name={`templates.${index}.tops`}
              title="Tops"
            />
            <NumbersField
              form={form}
              name={`templates.${index}.bottoms`}
              title="Bottoms"
            />
            <NumbersField
              form={form}
              name={`templates.${index}.shoes`}
              title="Shoes"
            />
            <NumbersField
              form={form}
              name={`templates.${index}.weapons`}
              title="Weapons"
            />
            <NumbersField
              form={form}
              name={`templates.${index}.items`}
              title="Items"
            />
            <NumbersField
              form={form}
              name={`templates.${index}.skills`}
              title="Skills"
            />
            <Button
              type="button"
...
function NumbersField<T extends FieldValues>({
  form,
  name,
  title,
}: NumbersFieldProps<T>) {
  ...
  const handleAdd = () => {
    if (newValue.trim() !== "" && !isNaN(Number(newValue))) {
      form.setValue(name, [...values, Number(newValue)] as PathValue<
        T,
        typeof name
      >);
      setNewValue("");
  ...
  const handleRemove = (index: number) => {
    form.setValue(
      name,
      values.filter((_, i) => i !== index) as PathValue<T, typeof name>,
    );
  };
