...
describe("rewardPoolsService", () => {
  ...
  it("item CRUD targets the nested collection", async () => {
    ...
    await rewardPoolsService.createItem("4170001", { itemId: 2000000, quantity: 1, tier: "common", weight: 50 });
    expect(api.post).toHaveBeenCalledWith("/api/gachapons/4170001/items", {
      data: { type: "gachapon-items", attributes: { itemId: 2000000, quantity: 1, tier: "common", weight: 50 } },
    });

    await rewardPoolsService.updateItem("4170001", "12", { itemId: 2000001, quantity: 2, tier: "common", weight: 75 });
    expect(api.patch).toHaveBeenCalledWith("/api/gachapons/4170001/items/12", {
      data: { id: "12", type: "gachapon-items", attributes: { itemId: 2000001, quantity: 2, tier: "common", weight: 75 } },
    });
