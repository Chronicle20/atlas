...
function transformBan(data: Ban): Ban {
  return {
    ...
    attributes: {
      ...data.attributes,
      banType: Number(
        data.attributes.banType,
      ) as typeof data.attributes.banType,
      reasonCode: Number(
        data.attributes.reasonCode,
      ) as typeof data.attributes.reasonCode,
...
function validateCreateBan(data: CreateBanRequest): ValidationError[] {
  ...
  if (
    ...
    errors.push({
      field: "expiresAt",
      message: "Expiration date must be in the future for non-permanent bans",
    });
  }
...
export const bansService = {
  ...
  async checkBan(
    params: CheckBanParams,
    options?: ServiceOptions,
  ): Promise<CheckBanResult> {
