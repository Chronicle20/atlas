...
class DropsService {
  async getMonsterDrops(monsterId: string, _tenant: Tenant): Promise<DropData[]> {
    return api.getList<DropData>(`/api/monsters/${monsterId}/drops`);
  }

  async getReactorDrops(reactorId: string, _tenant: Tenant): Promise<ReactorDropData[]> {
    return api.getList<ReactorDropData>(`/api/reactors/${reactorId}/drops`);
  }

  async getItemDrops(itemId: string, _tenant: Tenant): Promise<DropData[]> {
