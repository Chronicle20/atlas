...
function applyFilters(
  ...
  if (options.autoStart !== undefined) {
    filtered = filtered.filter(
      (q) => q.attributes.autoStart === options.autoStart,
    );
  }
  if (options.autoComplete !== undefined) {
    filtered = filtered.filter(
      (q) => q.attributes.autoComplete === options.autoComplete,
    );
  }
...
export const questsService = {
  ...
  async getAllQuests(options?: QuestQueryOptions): Promise<QuestDefinition[]> {
    const quests = await fetchAll<QuestDefinition>(
      BASE_PATH,
      undefined,
      options,
    );
    return applyFilters(quests, options).sort(
  ...
  async getQuestChain(
    ...
    while (currentQuestId) {
      try {
        ...
        if (chain.length > 100) {
          console.warn(
            "Quest chain exceeded 100 quests, stopping to prevent infinite loop",
          );
          break;
        }
      } catch {
