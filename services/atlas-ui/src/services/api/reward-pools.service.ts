...
export const rewardPoolsService = {
  ...
  async getAllPools(options?: QueryOptions): Promise<RewardPoolData[]> {
    return fetchAll<RewardPoolData>(
  ...
  async updatePool(
    ...
    await api.patch(`${BASE_PATH}/${id}`, {
      data: { id, type: REWARD_POOL_TYPE, attributes },
    });
  },
  ...
  async createItem(
    ...
    await api.post(`${BASE_PATH}/${poolId}/items`, {
      data: { type: REWARD_POOL_ITEM_TYPE, attributes },
    });
  },
  ...
  async createGlobalItem(
    ...
    await api.post(GLOBAL_PATH, {
      data: { type: GLOBAL_REWARD_ITEM_TYPE, attributes },
    });
  },
