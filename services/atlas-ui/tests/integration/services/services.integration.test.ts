...
describe("Services Integration Tests", () => {
  ...
  describe("Tenants Service Integration", () => {
    ...
    it("should handle tenant configuration management", async () => {
      ...
      const config =
        await tenantsService.getTenantConfigurationById("tenant-123");
  ...
  describe("Accounts Service Integration", () => {
    it("should handle account management with tenant context", async () => {
      ...
      const exists = await accountsService.accountExists(
        mockTenant,
        "account-1",
      );
    ...
    it("should handle batch session termination", async () => {
      ...
      const result = await accountsService.terminateMultipleSessions(
        mockTenant,
        accountIds,
      );
    ...
    it("should calculate account statistics", async () => {
      const accounts = [
        ...
        },
        {
          ...mockAccount,
          id: "account-2",
          attributes: { ...mockAccount.attributes },
        },
        {
          ...mockAccount,
          id: "account-3",
  ...
  describe("Characters Service Integration", () => {
    it("should handle character operations with tenant context", async () => {
      ...
      mockApi.getOne.mockResolvedValueOnce(mockCharacter);
      const fetchedCharacter = await charactersService.getById(
        mockTenant,
        "char-1",
      );
  ...
  describe("Error Handling Integration", () => {
    it("should handle API errors gracefully", async () => {
      ...
      await expect(
        tenantsService.getTenantById("non-existent"),
      ...
      await expect(accountsService.getAllAccounts(mockTenant)).rejects.toThrow(
    ...
    it("should handle partial failures in batch operations", async () => {
      ...
      expect(result.failed).toEqual([
        { id: "account-2", error: "Session not found" },
      ]);
    });
  });
