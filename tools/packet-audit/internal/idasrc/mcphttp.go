...
// GetBinaryInfo fetches the underlying IDB's module (binary) name and md5 via
// the survey_binary tool (detail_level "minimal" — this path only needs the
// metadata block, not the expensive per-function xref ranking the default
// detail level computes). It implements BinaryInfoProvider so exportRun can
// populate the export's top-level "binary"/"md5" provenance fields, which
// task-27 found were being silently left empty (no prior code path ever set
// them — every previously-committed export's values were hand-seeded once and
// preserved via --splice, never regenerated by a full harvest).
func (c *MCPHTTPClient) GetBinaryInfo(ctx context.Context) (string, string, error) {
	raw, err := c.callStructured(ctx, "survey_binary", map[string]any{"detail_level": "minimal"})
	if err != nil {
		return "", "", err
	}
	var payload struct {
	...
	if err := json.Unmarshal(raw, &payload); err != nil {
		return "", "", fmt.Errorf("idasrc: parse survey_binary result: %w", err)
	}
	return payload.Metadata.Module, payload.Metadata.MD5, nil
}
