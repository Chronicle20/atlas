...
// partialGlyphs disambiguates the three distinct StatePartial Note values into
// distinct render glyphs (render-only; status.json still carries State=partial
// + the Note, so the grading contract is unchanged — FR-4.2 / task-169 T2.2).
var partialGlyphs = map[string]string{
	"tier-1: needs byte-fixture test to verify": "🟡ᶠ", // tier-1 needs byte-fixture
	"tool ✅ without byte-test":                  "🟡ᵈ", // tool-pass diff-only (no byte-test)
	"evidence-pinned deferral":                  "🟡ᵖ", // evidence-pinned deferral
}

// cellSymbol renders a cell's glyph, disambiguating the three known partial
// notes into distinct superscripted glyphs. Any other state (or an
// unrecognized partial note) renders the base state symbol.
func cellSymbol(c Cell) string {
	if c.State == StatePartial {
		if g, ok := partialGlyphs[c.Note]; ok {
			return g
		}
	}
	return c.State.Symbol()
}
...
func RenderMarkdown(m Matrix, versionKeys []string) string {
	...
	b.WriteString("> Partial (🟡) sub-glyphs: 🟡ᶠ tier-1 needs byte-fixture · 🟡ᵈ tool-pass diff-only (no byte-test) · 🟡ᵖ evidence-pinned deferral\n\n")
...
func renderDirection(b *strings.Builder, m Matrix, versionKeys []string, dir opregistry.Direction, title string) {
	...
	for _, r := range m.Rows {
		...
		for _, vk := range versionKeys {
			fmt.Fprintf(b, " %s | %s |", opcodeCell(r, vk), cellSymbol(r.Cells[vk]))
		}
		b.WriteString("\n")
	}
	b.WriteString("\n")
